...
lemma sq_eq_sq_of_charZero {R : Type*} [CommRing R] [IsDomain R] [CharZero R]
    {x c : R} (h : x ^ 2 = c ^ 2) : x = c ∨ x = -c :=
  eq_or_eq_neg_of_sq_eq_sq _ _ h
...
private lemma f_poly_separable : f_poly.Separable := by
  ...
  have h_deriv : f_poly' = 5 * Polynomial.X ^ 4 + 20 := by
    simp +zetaDelta at *
    have h5 : (C 4 + 1 : Polynomial ℚ) = 5 := by
  ...
  · intros z hz hz' hz'' hz'''
    ...
    obtain ⟨r, hr⟩ := Complex.exists_root hdeg
    use r
    have := Polynomial.eval_eq_zero_of_dvd_of_eval_eq_zero
      (Polynomial.map_dvd (algebraMap ℚ ℂ) hz'') hr
    have := Polynomial.eval_eq_zero_of_dvd_of_eval_eq_zero
      (Polynomial.map_dvd (algebraMap ℚ ℂ) hz''') hr
    simp_all only [derivative_X_pow_succ, Nat.cast_ofNat, map_add, map_one, derivative_mul,
      derivative_C, zero_mul, derivative_X, mul_one, zero_add, add_zero, mem_nonunits_iff, ne_eq,
      IsRoot.def, eval_map_algebraMap, Polynomial.map_add, Polynomial.map_pow, map_X,
      Polynomial.map_mul, map_C, eq_ratCast, Rat.cast_ofNat, eval_add, eval_pow, eval_X, eval_mul,
      eval_C, Polynomial.map_ofNat, eval_ofNat, and_self, f_poly']
...
private lemma disc_value (v : Fin 5 ≃ (f_poly.rootSet f_poly.SplittingField)) :
    ...
    algebraMap ℚ _ (32000 ^ 2) := by
      ...
      have hv : (f_poly.map (algebraMap ℚ (f_poly.SplittingField))).roots =
        ...
        have hv_roots : (f_poly.map (algebraMap ℚ (f_poly.SplittingField))).roots =
        ...
        have hv_map : Multiset.map (fun x ↦ (x : f_poly.SplittingField))
      ...
      have h_vieta_sum : ∑ i : Fin 5, (v i : f_poly.SplittingField) ^ 0 = 5 ∧
          ...
          ∑ i : Fin 5, (v i : f_poly.SplittingField) ^ 3 = 0 ∧
          ∑ i : Fin 5, (v i : f_poly.SplittingField) ^ 4 = -80 := by
      ...
      have h_vieta_sum2 : ∑ i : Fin 5, (v i : f_poly.SplittingField) ^ 5 = -80 ∧
          ...
          ∑ i : Fin 5, (v i : f_poly.SplittingField) ^ 8 = 1600 := by
        have h_pow5 : ∀ i : Fin 5,
            (v i : f_poly.SplittingField) ^ 5 = -20 * (v i : f_poly.SplittingField) - 16 := by
          intro i
          have h_root : (v i : f_poly.SplittingField) ^ 5 + 20 * (v i : f_poly.SplittingField) + 16 = 0 := by
            replace h_factor := congr_arg (Polynomial.eval (v i : f_poly.SplittingField)) h_factor
...
theorem card_gal_f_poly_ne_120 :
  ...
  have h_perm : ∀ σ : f_poly.Gal, ∃ π : Equiv.Perm (Fin 5),
      ∀ i : Fin 5, σ (v i : f_poly.SplittingField) = v (π i) := by
    intro σ
    have h_mem : ∀ i : Fin 5,
        σ (v i : f_poly.SplittingField) ∈ f_poly.rootSet f_poly.SplittingField := by
      ...
      have h_root : (v i : f_poly.SplittingField) ^ 5 + 20 * (v i : f_poly.SplittingField) + 16 = 0 := by
        ...
        norm_num [f_poly]
      convert congr_arg (σ : f_poly.SplittingField → f_poly.SplittingField) h_root using 1
      norm_num [f_poly]
      erw [σ.commutes, σ.commutes]
      norm_num
      intro _
      exact ne_of_apply_ne (Polynomial.eval 0) (by norm_num)
    refine ⟨Equiv.ofBijective (fun i ↦ v.symm ⟨σ (v i), h_mem i⟩) ?_, ?_⟩
    · apply Finite.injective_iff_bijective.mp
      intro i j hij
      simpa [v.injective.eq_iff] using hij
    · intro i
      simp
  ...
  have h_sign : ∀ σ : f_poly.Gal, Equiv.Perm.sign (π σ) = 1 := by
    ...
    · simp_all
      erw [map_natCast] at h_sigma_delta
      norm_num at h_sigma_delta
      rcases Int.units_eq_one_or (Equiv.Perm.sign (π σ)) with h | h <;> simp_all
  ...
  apply ne_of_lt
  apply lt_of_le_of_lt h_image_card
