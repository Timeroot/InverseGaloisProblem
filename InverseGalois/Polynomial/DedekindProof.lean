...
lemma expand_eq_pow_zmod (f : Polynomial (ZMod p)) :
  ...
  have := expChar_zmod (p := p)
  have h := Polynomial.map_frobenius_expand (R := ZMod p) p f
  have frob_eq : frobenius (ZMod p) p = RingHom.id (ZMod p) := by
    ext x
    simp
  rwa [frob_eq, Polynomial.map_id] at h
...
lemma eval₂_pow_char (f : Polynomial (ZMod p)) (α : AlgebraicClosure (ZMod p)) :
  ...
  suffices h : (Polynomial.expand _ p) g = g ^ p by
    rw [h]
...
lemma frobenius_on_irred_is_cycle
  ...
  · -- Let α be a root of g in the algebraic closure.
    obtain ⟨α, hα⟩ : ∃ α : AlgebraicClosure (ZMod p), α ∈ g.rootSet (AlgebraicClosure (ZMod p)) := by
      obtain ⟨α, hα⟩ : ∃ α : AlgebraicClosure (ZMod p),
          g.eval₂ (algebraMap (ZMod p) (AlgebraicClosure (ZMod p))) α = 0 := by
        have := @IsAlgClosed.exists_root (AlgebraicClosure (ZMod p)) _
        specialize this (g.map (algebraMap (ZMod p) (AlgebraicClosure (ZMod p))))
        simp_all [Polynomial.degree_map]
        exact this (ne_of_gt (Polynomial.natDegree_pos_iff_degree_pos.mp (pos_of_gt h_deg)))
          |> fun ⟨x, hx⟩ ↦ ⟨x, by simpa [Polynomial.eval₂_eq_eval_map] using hx⟩
      refine ⟨α, ?_⟩
      rw [Polynomial.mem_rootSet]
      trivial
    -- The orbit of α under Frobenius has size exactly d.
    have h_orbit_size :
        Finset.card (Finset.image (fun k : ℕ ↦ α ^ (p ^ k)) (Finset.range g.natDegree)) = g.natDegree := by
      ...
      have h_contra : ∀ k < g.natDegree, k ≠ 0 → α ^ (p ^ k) ≠ α := by
        ...
        have h_contra : Module.finrank (ZMod p) (IntermediateField.adjoin (ZMod p) {α}) ≤ k := by
          have h_contra : ∀ x ∈ IntermediateField.adjoin (ZMod p) {α}, x ^ (p ^ k) = x := by
            intro x hx
            induction hx using IntermediateField.adjoin_induction <;> simp_all
            · simp [← map_pow, ZMod.pow_card_pow]
            · simp_all [add_pow_char_pow]
            · rw [mul_pow, ‹ (_ : AlgebraicClosure (ZMod p)) ^ p ^ k = _ ›,
                ‹ (_ : AlgebraicClosure (ZMod p)) ^ p ^ k = _ ›]
          have h_contra : ∀ x ∈ IntermediateField.adjoin (ZMod p) {α},
          ...
          have h_contra :
            ...
            have h_contra :
                Set.ncard (IntermediateField.adjoin (ZMod p) {α} : Set (AlgebraicClosure (ZMod p))) ≤
                  Multiset.card (Polynomial.roots
                    (Polynomial.X ^ (p ^ k) - Polynomial.X : Polynomial (AlgebraicClosure (ZMod p)))) := by
              have h_contra :
                  (IntermediateField.adjoin (ZMod p) {α} : Set (AlgebraicClosure (ZMod p))) ⊆
                    Multiset.toFinset (Polynomial.roots
                      (Polynomial.X ^ (p ^ k) - Polynomial.X : Polynomial (AlgebraicClosure (ZMod p)))) := by
          ...
          have h_contra :
            ...
            have h_contra : ∀ (F : IntermediateField (ZMod p) (AlgebraicClosure (ZMod p))),
                FiniteDimensional (ZMod p) F →
                  Set.ncard (F : Set (AlgebraicClosure (ZMod p))) = p ^ (Module.finrank (ZMod p) F) := by
      ...
      · exact this hl hk (by simpa only [eq_comm] using hkl) (Ne.symm hkl_ne)
          (lt_of_le_of_ne (le_of_not_gt hkl_lt) (Ne.symm hkl_ne))
      · -- Since `k < l`, `α ^ (p ^ l) = α ^ (p ^ k)` implies `α ^ (p ^ (l - k)) = α`.
    ...
    have h_orbit_eq_rootSet :
        Finset.image (fun k : ℕ ↦ α ^ (p ^ k)) (Finset.range g.natDegree) =
          Finset.image (fun x : g.rootSet (AlgebraicClosure (ZMod p)) ↦ x.val)
            (Finset.univ : Finset (g.rootSet (AlgebraicClosure (ZMod p)))) := by
      refine Finset.eq_of_subset_of_card_le (Finset.image_subset_iff.mpr ?_) ?_
      · intro k hk
        induction' k with k ih <;> simp_all [pow_succ, pow_mul]
      ...
      · rw [h_orbit_size, Finset.card_image_of_injective]
        · simp [Polynomial.rootSet_def]
          exact le_trans (Multiset.toFinset_card_le _) (Polynomial.card_roots' _)
            |> le_trans <| by simp [Polynomial.natDegree_map]
        · exact Subtype.coe_injective
    refine Or.inl ⟨⟨α, hα⟩, ?_, ?_⟩
    · intro h
      have := congr_arg Subtype.val h
      simp_all
      replace h_orbit_eq_rootSet := Finset.card_image_iff.mp
        (by simp_all : Finset.card (Finset.image (fun k ↦ α ^ p ^ k) (Finset.range g.natDegree)) = _)
      simp_all [Finset.card_image_of_injective, Function.Injective]
      have := @h_orbit_eq_rootSet 0
        (by norm_num
            linarith)
        1
        (by norm_num
    ...
    · intro y hy
      ...
      obtain ⟨k, hk₁, hk₂⟩ := h_orbit_eq_rootSet
      use k
      simp_all
      apply Subtype.ext
      convert hk₂ using 1
      refine Nat.recOn k ?_ ?_ <;> simp_all [pow_succ, pow_mul]
      intro n hn
      have hpow : (frobeniusPermOnRoots g hg ^ n) (frobeniusPermOnRoots g hg ⟨α, hα⟩) =
          frobeniusPermOnRoots g hg ((frobeniusPermOnRoots g hg ^ n) ⟨α, hα⟩) :=
        Nat.recOn n rfl fun n ihn ↦ by simp [*, pow_succ']
      erw [hpow]
...
lemma frobenius_on_irred_support_card
  ...
  -- Since `g` is irreducible and has degree `d ≥ 2`, the Frobenius permutation on its roots is a
  -- single cycle of length `d`, so its support is exactly the set of roots of `g`.
  have h_support : (frobeniusPermOnRoots g hg).support = Finset.univ := by
    apply Finset.eq_univ_of_forall
    intro x
    simp [frobeniusPermOnRoots]
    -- Since `x` is a root of `g`, `g x = 0`. If `x ^ p = x`, then `x` would be a root of
    -- `x ^ p - x`, which contradicts the irreducibility of `g`.
    have h_contradiction : ¬(x.val ^ p = x.val) := by
      intro h_contra
      have h_root : x.val ∈ Set.range (algebraMap (ZMod p) (AlgebraicClosure (ZMod p))) := by
        have h_root : x.val ∈ Set.range (algebraMap (ZMod p) (AlgebraicClosure (ZMod p))) := by
          have h_poly : (Polynomial.X ^ p - Polynomial.X : Polynomial (AlgebraicClosure (ZMod p))) =
              ∏ a ∈ Finset.univ,
                (Polynomial.X - Polynomial.C (algebraMap (ZMod p) (AlgebraicClosure (ZMod p)) a)) := by
            refine Polynomial.eq_of_degree_sub_lt_of_eval_finset_eq
              (Finset.image (fun a : ZMod p ↦ algebraMap (ZMod p) (AlgebraicClosure (ZMod p)) a)
                Finset.univ) ?_ ?_
            · convert Polynomial.degree_sub_lt _ _ ?_ <;>
              ...
              · rw [Finset.card_image_of_injective _ fun a b h ↦ by simpa using h, Finset.card_univ]
                norm_num [Polynomial.degree_sub_eq_left_of_degree_lt, hp.1.one_lt]
              · rw [Polynomial.degree_sub_eq_left_of_degree_lt] <;> norm_num [hp.1.one_lt]
              · exact ne_of_apply_ne (Polynomial.derivative) (by simp [Polynomial.derivative_pow])
              · rw [Polynomial.leadingCoeff_prod]
                rw [Polynomial.leadingCoeff_sub_of_degree_lt] <;> norm_num [hp.1.one_lt]
            · simp [Polynomial.eval_prod]
              intro a
              rw [Finset.prod_eq_prod_diff_singleton_mul <| Finset.mem_univ a]
      ...
      have hgy : g.eval y = 0 := by
        ...
        subst hy
        simp_all only [aeval_algebraMap_eq_zero_iff, coe_aeval_eq_eval]
      have := Polynomial.degree_eq_one_of_irreducible_of_root hirr hgy
      rw [Polynomial.degree_eq_natDegree hirr.ne_zero] at this
      norm_cast at this
      linarith
...
lemma frobenius_cycleType_irred
  ...
  · -- Since g is irreducible, the Frobenius permutation on its roots is a single cycle of length g.natDegree.
    ...
    rw [Polynomial.natDegree_mul'] <;> simp_all
  · -- Since the degree of g is less than 2, the permutation is the identity.
    have h_identity : frobeniusPermOnRoots g hg = 1 := by
      have h_root_set_card : (g.rootSet (AlgebraicClosure (ZMod p))).toFinset.card ≤ 1 := by
        have hsub : (g.rootSet (AlgebraicClosure (ZMod p))).toFinset ⊆
            (g.map (algebraMap (ZMod p) (AlgebraicClosure (ZMod p)))
              |> Polynomial.roots |> Multiset.toFinset) :=
          fun x hx ↦ by simp_all [Polynomial.mem_rootSet]
        refine le_trans (Finset.card_le_card hsub) ?_
        refine le_trans (Multiset.toFinset_card_le _) ?_
        refine le_trans (Polynomial.card_roots' _) ?_
        erw [Polynomial.natDegree_map]
      ...
      · have : Subsingleton ↑(g.rootSet (AlgebraicClosure (ZMod p))) :=
          Fintype.card_le_one_iff_subsingleton.mp (by omega)
        exact fun a b ↦ Subsingleton.elim _ _
    simp_all [factorizationType]
    interval_cases _ : g.natDegree <;> simp_all [normalize]
    all_goals rw [Polynomial.natDegree_mul']
    · simp_all only [natDegree_C, add_zero]
      rfl
    · simp_all only [leadingCoeff_C, ne_eq, leadingCoeff_eq_zero, not_false_eq_true, mul_inv_cancel₀, one_ne_zero]
    · simp_all only [natDegree_C, add_zero]
      rfl
    · simp_all only [leadingCoeff_C, ne_eq, leadingCoeff_eq_zero, not_false_eq_true, mul_inv_cancel₀, one_ne_zero]
...
private lemma frobeniusPermOnRoots_unit_mul'
  ...
  · ext
    ...
    grind only [Equiv.Perm.subtypeCongr.apply, Equiv.permCongr_def, frobeniusPermOnRoots_val,
      rootSet_unit_mul', Equiv.permCongr_apply, = Equiv.trans_apply, Equiv.coe_fn_mk,
...
private lemma perm_cycleType_decompose {α : Type*} [Fintype α] [DecidableEq α]
  ...
  · intro x
    by_cases hx : P x <;> by_cases hx' : Q x <;> simp at hdisj ⊢
    · exact False.elim (hdisj x hx hx')
    · simp [Equiv.Perm.extendDomain]
      simp [Equiv.Perm.subtypeCongr]
      simp [Equiv.sumCompl]
      grind
...
private lemma frobenius_cycleType_coprime_mul'
  ...
  have hP : ∀ x, P (frobeniusPermOnRoots (g * h) hgh x) ↔ P x := by
    simp +zetaDelta at *
    intro a ha
    have := frobeniusPermOnRoots_val (g * h) hgh ⟨a, ha⟩
    simp_all [Polynomial.mem_rootSet]
    have := eval₂_pow_char g a
  ...
  have h_disjoint : ∀ x, ¬(P x ∧ Q x) := by
    ...
    simp_all [Polynomial.mem_rootSet]
    obtain ⟨u, v, h⟩ := hc
    replace h := congr_arg (Polynomial.aeval a) h
    simp_all
...
theorem frobenius_cycleType_eq_factorizationType
  ...
  revert f hf hsf
  intro f hf hsf
  have h_segment : ∀ (g : Polynomial (ZMod p)) (hg : g ≠ 0) (hirr : Irreducible g),
      (frobeniusPermOnRoots g hg).cycleType = factorizationType g := by
  ...
  have h_segment_mul : ∀ (g : Polynomial (ZMod p)) (hg : g ≠ 0) (hsf : Squarefree g),
    ...
    · rw [Polynomial.isUnit_iff] at *
      obtain ⟨r, hr, rfl⟩ := ‹∃ r, IsUnit r ∧ C r = g›
      simp_all [factorizationType]
      rw [Multiset.filter_eq_nil.mpr] <;> norm_num
      · ext ⟨x, hx⟩
        simp at hx ⊢
      · intro a ha
        have := Polynomial.natDegree_le_of_dvd (dvd_of_mem_normalizedFactors ha)
        simp_all
    · rename_i g hg₁ hg₂ hg₃
...
private lemma natDegree_map_eq_of_monic (f : ℤ[X]) (hf : f.Monic) :
  ...
  rw [Polynomial.natDegree_map_of_leadingCoeff_ne_zero,
    Polynomial.natDegree_map_of_leadingCoeff_ne_zero] <;> norm_num [hf]
...
private lemma reduction_injective_on_roots_aux (f : ℤ[X]) (hf_monic : f.Monic)
    (_hf_irr : Irreducible (f.map (Int.castRingHom ℚ)))
  ...
  by_contra h_neq
  have h_common_root : (f.map (Int.castRingHom (𝓞 L ⧸ Q))).IsRoot (Ideal.Quotient.mk Q α) ∧
      (Polynomial.derivative (f.map (Int.castRingHom (𝓞 L ⧸ Q)))).IsRoot (Ideal.Quotient.mk Q α) := by
    have h_common_root :
        (Polynomial.map (Int.castRingHom (𝓞 L)) f).eval₂ (algebraMap (𝓞 L) L) (α : L) = 0 ∧
          (Polynomial.map (Int.castRingHom (𝓞 L)) f).eval₂ (algebraMap (𝓞 L) L) (β : L) = 0 := by
      simp_all [Polynomial.aeval_def, Polynomial.eval₂_eq_sum_range]
    obtain ⟨g, hg⟩ : ∃ g : Polynomial (𝓞 L),
        Polynomial.map (Int.castRingHom (𝓞 L)) f =
      ...
      exact mul_dvd_mul_left _ (Polynomial.dvd_iff_isRoot.mpr
        (h_common_root.resolve_left (sub_ne_zero_of_ne <|
          by simpa [sub_eq_zero] using Ne.symm <| by simpa [sub_eq_zero] using h_neq)))
    replace hg := congr_arg (Polynomial.map (Ideal.Quotient.mk Q)) hg
    simp_all [Polynomial.eval₂_eq_eval_map]
    replace hg := congr_arg (Polynomial.derivative) hg
    simp_all [Polynomial.derivative_mul, Polynomial.derivative_sub, Polynomial.derivative_X,
      Polynomial.derivative_C]
    replace hg := congr_arg (Polynomial.eval (Ideal.Quotient.mk Q β)) hg
  ...
  obtain ⟨a, b, h⟩ := h_separable_Q
  replace h := congr_arg (Polynomial.eval (Ideal.Quotient.mk Q α)) h
  simp_all
...
private noncomputable def residueFieldEmbedding
  ...
  have : Field (𝓞 L ⧸ Q) := Ideal.Quotient.field Q
  let _ := ZMod.algebra (𝓞 L ⧸ Q) p
  have : Algebra.IsAlgebraic (ZMod p) (𝓞 L ⧸ Q) := Algebra.IsAlgebraic.of_finite _ _
...
private lemma residueFieldEmbedding_injective
  ...
  have : CharP (𝓞 L ⧸ Q) p := residue_field_charP Q hQ
  let _ : Field (𝓞 L ⧸ Q) := Ideal.Quotient.field Q
  have : IsSimpleRing (𝓞 L ⧸ Q) := inferInstance
...
private lemma rootReduction_injective
    ...
    α1 = α2 := by
  have h_inj : (root_mem_ringOfIntegers f hf_monic α1.val α1.2).choose =
      (root_mem_ringOfIntegers f hf_monic α2.val α2.2).choose := by
...
theorem frobenius_lift (f : ℤ[X]) (hf_monic : f.Monic)
  ...
  have : IsGalois ℚ L := IsGalois.of_separable_splitting_field hf_irr.separable
  have : NumberField L := ⟨⟩
  -- Get maximal ideal Q above p and Frobenius element σ
  obtain ⟨Q, hQmax, hQp⟩ := @exists_maximal_ideal_above p hp L _ _
  have := hQmax
  ...
  have hφ_bij : Function.Bijective φ := by
    ...
    obtain ⟨a, _, ha⟩ := Finset.mem_image.mp this
    exact ⟨a, ha⟩
  -- Construct the Equiv
  ...
  have : Fact (fQ.map (algebraMap ℚ L)).Splits := ⟨Polynomial.SplittingField.splits fQ⟩
  ...
  have h_equiv : ∀ x, e (galActionAux_perm fQ σ x) =
    ...
    have h_lift_eq :
        (root_mem_ringOfIntegers f hf_monic ((galActionAux_perm fQ σ) x).val ((galActionAux_perm fQ σ) x).2).choose =
          (galRestrict ℤ ℚ L (𝓞 L) σ (root_mem_ringOfIntegers f hf_monic x.val x.2).choose) := by
      have h_lift_eq :
          (root_mem_ringOfIntegers f hf_monic (galActionAux_perm fQ σ x).val (galActionAux_perm fQ σ x).2).choose.val =
            (galRestrict ℤ ℚ L (𝓞 L) σ (root_mem_ringOfIntegers f hf_monic x.val x.2).choose).val := by
        have := root_mem_ringOfIntegers f hf_monic (galActionAux_perm fQ σ x).val (galActionAux_perm fQ σ x).2
        have := this.choose_spec
        simp_all [galActionAux_perm_val]
        have := root_mem_ringOfIntegers f hf_monic x.val x.2
        have := this.choose_spec
        simp_all [galRestrict]
        exact DFunLike.congr rfl (id (Eq.symm this))
      exact Subtype.ext h_lift_eq
    simp_all only [Equiv.ofBijective_apply, map_pow, fQ, fp, L, φ, ι, e]
    obtain ⟨val, property⟩ := x
    simp_all only [fQ]
    rfl
