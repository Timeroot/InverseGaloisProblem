...
lemma irreducible_comp_C_mul_X {L : Type*} [Field L] (q : Polynomial L) {c : L} (hc : c ≠ 0) :
  ...
  · -- Let's assume that $q$ is reducible and derive a contradiction.
    ...
    obtain ⟨a, b, ha, hb, hab⟩ : ∃ a b : L[X], a.degree > 0 ∧ b.degree > 0 ∧ q = a * b := by
      ...
      · rw [ Polynomial.isUnit_iff ] at hq_unit ; aesop;
      · rcases hq_reducible with ⟨ a, b, rfl, ha, hb ⟩ ; exact ⟨ a, not_le.mp fun ha' => ha <| Polynomial.isUnit_iff_degree_eq_zero.mpr <| le_antisymm ha' <| le_of_not_gt fun ha'' => by aesop, b, not_le.mp fun hb' => hb <| Polynomial.isUnit_iff_degree_eq_zero.mpr <| le_antisymm hb' <| le_of_not_gt fun hb'' => by aesop, rfl ⟩ ;
    simp_all [ Polynomial.isUnit_iff_degree_eq_zero ];
    cases this rfl <;> simp_all [ Polynomial.degree_eq_natDegree ( show a ≠ 0 from by aesop_cat ), Polynomial.degree_eq_natDegree ( show b ≠ 0 from by aesop_cat ) ]; all_goals rw [ Polynomial.degree_eq_natDegree ( by exact ne_of_apply_ne Polynomial.natDegree <| by erw [ Polynomial.natDegree_comp, Polynomial.natDegree_C_mul_X ] <;> aesop ) ] at * ; simp_all [ Polynomial.natDegree_comp, Polynomial.natDegree_mul' ];
  · constructor;
    · intro h; have := Polynomial.natDegree_eq_zero_of_isUnit h; simp_all [ Polynomial.natDegree_comp, Polynomial.natDegree_mul' ] ;
      exact absurd this ( Nat.ne_of_gt ( Polynomial.natDegree_pos_iff_degree_pos.mpr ( Polynomial.degree_pos_of_irreducible hq ) ) );
    · intro a b hab;
      have := @this ( a.comp ( Polynomial.C c⁻¹ * Polynomial.X ) ) ( b.comp ( Polynomial.C c⁻¹ * Polynomial.X ) ) ?_;
      · rcases this with ( h | h ) <;> have := Polynomial.natDegree_eq_zero_of_isUnit h <;> simp_all [ Polynomial.natDegree_comp, Polynomial.natDegree_mul' ]; all_goals rw [ Polynomial.eq_C_of_natDegree_eq_zero this ] at h ⊢; aesop;
      · convert congr_arg ( Polynomial.comp · ( Polynomial.C c⁻¹ * Polynomial.X ) ) hab using 1 <;> simp [ Polynomial.comp_assoc ];
...
lemma monicAssociate_absIrr (f : Polynomial (Polynomial ℚ)) (hf_deg : 2 ≤ f.natDegree)
  ...
  suffices hM_map : Irreducible (M.map (algebraMap (Polynomial K) (FractionRing (Polynomial K))) ) by
  ...
  have hM_comp : (M.map (algebraMap (Polynomial K) (FractionRing (Polynomial K)))).comp (Polynomial.C (algebraMap (Polynomial K) (FractionRing (Polynomial K)) (fK.leadingCoeff)) * Polynomial.X) = Polynomial.C (algebraMap (Polynomial K) (FractionRing (Polynomial K)) (fK.leadingCoeff ^ (fK.natDegree - 1))) * (fK.map (algebraMap (Polynomial K) (FractionRing (Polynomial K)))) := by
    have hM_comp : M.comp (Polynomial.C (fK.leadingCoeff) * Polynomial.X) = Polynomial.C (fK.leadingCoeff ^ (fK.natDegree - 1)) * fK := by
      ...
      · simp +zetaDelta at *;
        ...
        · rw [ Polynomial.leadingCoeff_map_of_leadingCoeff_ne_zero ] <;> aesop;
        · aesop_cat;
    convert congr_arg ( Polynomial.map ( algebraMap ( Polynomial K ) ( FractionRing ( Polynomial K ) ) ) ) hM_comp using 1 <;> norm_num [ Polynomial.map_comp ];
  have hM_irred : Irreducible (Polynomial.C (algebraMap (Polynomial K) (FractionRing (Polynomial K)) (fK.leadingCoeff ^ (fK.natDegree - 1))) * (fK.map (algebraMap (Polynomial K) (FractionRing (Polynomial K)))) ) := by
    have hM_irred : Irreducible (fK.map (algebraMap (Polynomial K) (FractionRing (Polynomial K)))) := by
    ...
    aesop;
  convert irreducible_comp_C_mul_X ( Polynomial.map ( algebraMap K[X] ( FractionRing K[X] ) ) M ) ( show algebraMap K[X] ( FractionRing K[X] ) fK.leadingCoeff ≠ 0 from ?_ ) |>.1 ?_ using 1; all_goals aesop
...
theorem hilbert_irreducibility_monic (f : Polynomial (Polynomial ℚ))
  ...
  swap;
  exact Set.univ \ ⋃ k ∈ Finset.Ico 1 f.natDegree, { t : ℤ | ∃ g : Polynomial ℚ, g.natDegree = k ∧ g.Monic ∧ g ∣ specialize f t };
  · have h_union_sublinear : ∃ (C : ℝ) (α : ℝ), 0 < C ∧ 0 ≤ α ∧ α < 1 ∧ ∀ N : ℕ, 0 < N →
      (Set.ncard ((⋃ k ∈ Finset.Ico 1 f.natDegree, {t : ℤ | ∃ g : Polynomial ℚ, g.natDegree = k ∧ g.Monic ∧ g ∣ specialize f t}) ∩
        Set.Icc (-(N : ℤ)) (N : ℤ)) : ℝ) ≤ C * (N : ℝ) ^ α := by
          have h_union_sublinear : ∀ k ∈ Finset.Ico 1 f.natDegree, ∃ (C : ℝ) (α : ℝ), 0 < C ∧ 0 ≤ α ∧ α < 1 ∧ ∀ N : ℕ, 0 < N →
              (Set.ncard ({t : ℤ | ∃ g : Polynomial ℚ, g.natDegree = k ∧ g.Monic ∧ g ∣ specialize f t} ∩
                Set.Icc (-(N : ℤ)) (N : ℤ)) : ℝ) ≤ C * (N : ℝ) ^ α := by
                  exact fun k hk => dorge_density_estimate f hf hf_monic hf_abs_irr k ( Finset.mem_Ico.mp hk |>.1 ) ( Finset.mem_Ico.mp hk |>.2 );
          ...
          · rcases Finset.eq_empty_or_nonempty ( Finset.image α ( Finset.Ico 1 f.natDegree ) ) with h | ⟨ x, hx ⟩ <;> simp_all;
            ...
            exact lt_of_le_of_lt ( csSup_le ( Set.Nonempty.image _ <| Set.nonempty_Ico.mpr <| by linarith ) <| Set.forall_mem_image.mpr fun j hj => hk.2 j <| Finset.mem_Ico.mpr hj ) <| hα' k ( Finset.mem_Ico.mp hk.1 |>.1 ) ( Finset.mem_Ico.mp hk.1 |>.2 );
          · intro N hN_pos
            have h_union_sublinear : (Set.ncard ((⋃ k ∈ Finset.Ico 1 f.natDegree, {t : ℤ | ∃ g : Polynomial ℚ, g.natDegree = k ∧ g.Monic ∧ g ∣ specialize f t}) ∩
              Set.Icc (-(N : ℤ)) (N : ℤ)) : ℝ) ≤ ∑ k ∈ Finset.Ico 1 f.natDegree, (Set.ncard ({t : ℤ | ∃ g : Polynomial ℚ, g.natDegree = k ∧ g.Monic ∧ g ∣ specialize f t} ∩
              Set.Icc (-(N : ℤ)) (N : ℤ)) : ℝ) := by
                rw [ Set.inter_comm, Set.inter_iUnion₂ ];
                norm_cast;
                have h_union_sublinear : ∀ (s : Finset ℕ) (f : ℕ → Set ℤ), (Set.ncard (⋃ k ∈ s, f k)) ≤ ∑ k ∈ s, (Set.ncard (f k)) := by
                  exact fun s f => Finset.set_ncard_biUnion_le s f;
                simpa only [ Set.inter_comm ] using h_union_sublinear ( Finset.Ico 1 f.natDegree ) fun k => Set.Icc ( -N : ℤ ) N ∩ { t : ℤ | ∃ g : Polynomial ℚ, g.natDegree = k ∧ g.Monic ∧ g ∣ specialize f t };
            refine' le_trans h_union_sublinear _;
            rw [ Finset.sum_mul _ _ _ ];
            exact Finset.sum_le_sum fun k hk => le_trans ( h k hk N hN_pos ) ( mul_le_mul_of_nonneg_left ( Real.rpow_le_rpow_of_exponent_le ( mod_cast hN_pos ) ( show α k ≤ sSup ( Finset.image α ( Finset.Ico 1 f.natDegree ) ) from le_csSup ( by exact Set.Finite.bddAbove <| Finset.finite_toSet _ ) <| Finset.mem_image_of_mem _ hk ) ) <| le_of_lt <| hC k hk );
  ...
  · intro t ht
    ...
    constructor;
    · intro h;
      have := Polynomial.natDegree_eq_zero_of_isUnit h; rw [ Polynomial.natDegree_map_of_leadingCoeff_ne_zero ] at this <;> aesop;
    · intro a b hab; contrapose! ht; simp_all [ Polynomial.isUnit_iff_degree_eq_zero ] ;
      ...
      have ha_deg : 1 ≤ a.natDegree := by
        ...
        · rw [ Polynomial.ext_iff ] at hab;
          specialize hab ( Polynomial.natDegree f ) ; simp_all [ Polynomial.coeff_map ] ;
        · exact Nat.pos_of_ne_zero ht.1
      have hb_deg : 1 ≤ b.natDegree := by
        by_cases hb : b = 0 <;> simp_all [ Polynomial.degree_eq_natDegree ];
        · replace hab := congr_arg ( fun p => p.coeff ( Polynomial.natDegree f ) ) hab ; simp_all [ Polynomial.coeff_map ];
      ...
      · rw [ Polynomial.natDegree_C_mul ] <;> norm_num [ ha_deg, hb_deg ];
        · have := congr_arg Polynomial.natDegree hab; rw [ Polynomial.natDegree_map_of_leadingCoeff_ne_zero ] at this <;> norm_num at *;
...
lemma hit_degree_one (f : Polynomial (Polynomial ℚ))
  ...
  obtain ⟨a, b, ha⟩ : ∃ a b : Polynomial ℚ, f = Polynomial.C a * Polynomial.X + Polynomial.C b ∧ a ≠ 0 := by
    exact ⟨ f.coeff 1, f.coeff 0, by nth_rw 1 [ Polynomial.eq_X_add_C_of_natDegree_le_one ( le_of_eq hf_deg ) ], by rw [ ← hf_deg, Polynomial.coeff_natDegree ] ; aesop ⟩;
  ...
  intro t ht; simp_all [ specialize ] ;
...
lemma irreducible_specialize_of_C_mul {f : Polynomial (Polynomial ℚ)}
  ...
  · rw [ irreducible_mul_iff ] at h;
    ...
    exact absurd ‹Irreducible ( C ( eval ( t : ℚ ) c ) ) ∧ ( map ( evalRingHom ( t : ℚ ) ) f ).degree = 0›.1 ( Polynomial.not_irreducible_C _ );
  · rw [ irreducible_mul_iff ];
    exact Or.inr ⟨ h, Polynomial.isUnit_C.mpr <| by rw [ Polynomial.isUnit_iff ] at *; aesop ⟩
...
theorem hilbert_irreducibility_theorem (f : Polynomial (Polynomial ℚ))
  ...
  · by_cases hm : f.Monic
    ...
    · -- Non-monic case: reduce to monic via Tschirnhaus substitution.
      ...
      have h_equiv : {t : ℤ | Irreducible (specialize f t)} ⊇ {t : ℤ | Irreducible (specialize (monicAssociate f) t)} \ {t : ℤ | Polynomial.eval (t : ℚ) f.leadingCoeff = 0} := by
        intro t ht; exact (monicAssociate_specialize_iff f (by linarith) t (by aesop)).mp ht.left;
...
lemma irreducible_iff_not_in_reducibleLocus (f : Polynomial (Polynomial ℚ))
    ...
    Irreducible (specialize f t) ↔
      ∀ k, 1 ≤ k → k < f.natDegree → t ∉ reducibleLocus f k := by
        ...
        · rcases h with ⟨ g, hg₁, hg₂, hg₃, hg₄ ⟩ ; rw [ irreducible_iff ] ; simp_all [ Polynomial.Monic.def ] ;
          obtain ⟨ q, hq ⟩ := hg₄; use fun _ => ⟨ g, q, by linear_combination' hq, by exact fun h => by have := Polynomial.natDegree_eq_zero_of_isUnit h; linarith, by exact fun h => by have := Polynomial.natDegree_eq_zero_of_isUnit h; have := congr_arg Polynomial.natDegree hq; rw [ Polynomial.natDegree_mul' ] at this <;> aesop ⟩ ;
        · obtain ⟨g, hg⟩ : ∃ g : Polynomial ℚ, g ∣ specialize f t ∧ 1 ≤ g.natDegree ∧ g.natDegree < f.natDegree := by
            ...
            · cases h_unit <;> simp_all [ Polynomial.natDegree_eq_zero_of_isUnit ];
              · linarith;
              · grind;
            · obtain ⟨g, hg⟩ : ∃ g : Polynomial ℚ, g ∣ specialize f t ∧ 1 ≤ g.natDegree ∧ g.natDegree < (specialize f t).natDegree := by
                have h_factor : ∃ g h : Polynomial ℚ, g ∣ specialize f t ∧ h ∣ specialize f t ∧ g.natDegree + h.natDegree = (specialize f t).natDegree ∧ 1 ≤ g.natDegree ∧ 1 ≤ h.natDegree := by
                  obtain ⟨g, h, hg, hh, hgh⟩ : ∃ g h : Polynomial ℚ, g ∣ specialize f t ∧ h ∣ specialize f t ∧ g * h = specialize f t ∧ ¬IsUnit g ∧ ¬IsUnit h := by
                    rw [ irreducible_iff ] at h;
                    push_neg at h;
                    exact Exists.elim ( h ( by tauto ) ) fun a ha => Exists.elim ha fun b hb => ⟨ a, b, hb.1.symm ▸ dvd_mul_right _ _, hb.1.symm ▸ dvd_mul_left _ _, hb.1.symm, hb.2.1, hb.2.2 ⟩;
                  refine' ⟨g, h, hg, hh, ?_, ?_, ?_⟩
                  · (rw [← hgh.1, Polynomial.natDegree_mul']; aesop)
                  · exact Nat.pos_of_ne_zero fun con => hgh.2.1 <| Polynomial.isUnit_iff_degree_eq_zero.mpr <| by rw [ Polynomial.degree_eq_natDegree ] <;> aesop;
                  · exact Nat.pos_of_ne_zero fun con => hgh.2.2 <| Polynomial.isUnit_iff_degree_eq_zero.mpr <| by rw [ Polynomial.degree_eq_natDegree ] <;> aesop;
                grind;
              aesop;
