...
* `exists_alternating_resolvent_family` — **the deep geometric input**: a regular
  `Aₙ`-resolvent family over `ℚ(T)` (Serre's §4.5 square-discriminant construction).  See its
...
/-- **The deep geometric input: an `Aₙ`-resolvent family.**
...
This is a regular `Aₙ`-extension of `ℚ(T)` — the genuine hard content.  The `Sₙ` Morse family
`Xⁿ − X − T` (`ResolventFamily`) is **not** reusable (its discriminant is not a square), so a
different family with its own monodromy analysis is used.

**Construction (Serre §4.5).** The proof splits on the parity of `n`: even `n` uses the
substituted family `AlternatingFamily.serreAnFamily n`, odd `n` the conic family
`AlternatingFamily.serreAnFamilyOdd n`.  Both descend from the base ramification family
`Xⁿ − X^{n-1} − T` (Galois group `Sₙ`) by the square-discriminant substitution of the
double-group trick: `disc F` becomes *identically* a square in `ℚ[T]`, forcing monodromy
`⊆ Aₙ`, and the finite inertia becomes a **3-cycle**, so with transitivity the geometric
monodromy is exactly `Aₙ` (`closure_three_cycles_eq_alternating`), giving absolute
irreducibility of `G`.  The `Aₙ`-orbit resolvent `G` descends to `ℚ(T)` because its coefficients
are `Aₙ`-invariant, living in `ℚ[e₁,…,eₙ][δ]` with `δ = √disc F ∈ ℚ(T)`; root-containment and
cofinite separability follow as in the `Sₙ` `ResolventFamily`. -/
...
providing the `Gal(F(t)) ↪ Aₙ` landing certificate). -/
