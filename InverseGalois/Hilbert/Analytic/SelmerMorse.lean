...
lemma selmer_rootMultiplicity_le_two {F : Type*} [CommRing F] [IsDomain F] (n : ℕ) (a : F) :
  ...
  have h_div : (Polynomial.X - Polynomial.C a)^3 ∣ selmerPoly F n := by
    exact dvd_trans ( pow_dvd_pow _ ( not_le.mp h_contradiction ) ) ( Polynomial.pow_rootMultiplicity_dvd _ _ )
  have h_root : Polynomial.eval a (selmerPoly F n) = 0 ∧ Polynomial.eval a (derivative (selmerPoly F n)) = 0 ∧ Polynomial.eval a (derivative (derivative (selmerPoly F n))) = 0 := by
  ...
  obtain ⟨ p, hp ⟩ := h_div; replace hp := congr_arg ( Polynomial.eval a ) hp; simp_all [ pow_succ', mul_assoc ] ;
...
lemma selmer_ncard_roots_field {F : Type*} [CommRing F] [IsDomain F] [DecidableEq F] (n : ℕ) :
  ...
  have h_at_most_one_repeated : Finset.card (Finset.filter (fun a => 2 ≤ (selmerPoly F n).roots.count a) (selmerPoly F n).roots.toFinset) ≤ 1 := by
    rw [ Finset.card_le_one_iff ];
    simp +zetaDelta at *;
    intro a b h₁ h₂ h₃ h₄ h₅ h₆; have := Polynomial.isRoot_iterate_derivative_of_lt_rootMultiplicity ( show 1 < rootMultiplicity a ( selmerPoly F n ) from h₃ ) ; have := Polynomial.isRoot_iterate_derivative_of_lt_rootMultiplicity ( show 1 < rootMultiplicity b ( selmerPoly F n ) from h₆ ) ; simp_all [ Polynomial.rootMultiplicity_eq_zero ] ;
    apply selmer_derivative_common_root_unique n h₂ ‹_› h₅ ‹_›;
  have h_sum_le_one : ∑ a ∈ (selmerPoly F n).roots.toFinset, ((selmerPoly F n).roots.count a - 1) ≤ Finset.card (Finset.filter (fun a => 2 ≤ (selmerPoly F n).roots.count a) (selmerPoly F n).roots.toFinset) := by
    rw [ Finset.card_filter ];
    gcongr;
    have := selmer_rootMultiplicity_le_two n ‹_›; simp_all [ Polynomial.count_roots ] ;
    grind;
  have h_sum_eq_card : ∑ a ∈ (selmerPoly F n).roots.toFinset, (selmerPoly F n).roots.count a = (selmerPoly F n).roots.card := by
    rw [ ← Multiset.toFinset_sum_count_eq ];
  have h_sum_eq_card : ∑ a ∈ (selmerPoly F n).roots.toFinset, ((selmerPoly F n).roots.count a - 1) + ∑ a ∈ (selmerPoly F n).roots.toFinset, 1 = (selmerPoly F n).roots.card := by
    rw [ ← Finset.sum_add_distrib, Finset.sum_congr rfl fun x hx => tsub_add_cancel_of_le <| Nat.succ_le_of_lt <| Multiset.count_pos.mpr <| Multiset.mem_toFinset.mp hx ] ; simp_all only [count_roots];
...
lemma selmerPoly_map {R S : Type*} [CommRing R] [CommRing S] [Algebra R S] (n : ℕ) :
  ...
  unfold selmerPoly; simp_all only [Polynomial.map_sub, Polynomial.map_pow, map_X, Polynomial.map_one];
...
lemma selmerPoly_ne_zero {F : Type*} [CommRing F] [Nontrivial F] (n : ℕ) :
  ...
  have h_contra : ∀ x : F, x^n - x - 1 = 0 := by
    exact fun x => by simpa [ selmerPoly ] using congr_arg ( Polynomial.eval x ) h_zero_poly;
  have := h_contra 0; have := h_contra 1; rcases n with ( _ | _ | n ) <;> simp_all ;
...
lemma ncard_rootSet_le_roots_card
  ...
  have h_div : (∏ a ∈ s, (Polynomial.X - Polynomial.C a)) ∣ (Polynomial.map (algebraMap R S) q) := by
    have h_div : ∀ {m : Multiset S}, (∀ a ∈ m, Polynomial.IsRoot (Polynomial.map (algebraMap R S) q) a) → Multiset.Nodup m → (m.map (fun a => Polynomial.X - Polynomial.C a)).prod ∣ (Polynomial.map (algebraMap R S) q) := by
      intro m hm hm_nodup
      induction' m using Multiset.induction with a m ih;
      · simp;
      · obtain ⟨ p, hp ⟩ := ih ( fun x hx => hm x ( Multiset.mem_cons_of_mem hx ) ) ( Multiset.nodup_cons.mp hm_nodup |>.2 ) ; simp_all [ Polynomial.eval_map ] ;
        rw [ mul_comm ];
        refine' mul_dvd_mul_left _ _;
        refine' Polynomial.dvd_iff_isRoot.mpr _;
        replace hp := congr_arg ( Polynomial.eval a ) hp ; simp_all [ Polynomial.eval_multiset_prod ] ;
        exact hm.1.resolve_left fun ⟨ b, hb, h ⟩ => hm_nodup.1 <| by simpa [ sub_eq_zero.mp h ] using hb;
    convert h_div _ _ <;> simp_all [ Polynomial.rootSet_def ];
  ...
  have h_roots : (Polynomial.map (algebraMap R D) q).roots ≥ Multiset.map (fun a => algebraMap S D a) s.val := by
    have h_roots : Polynomial.roots (∏ a ∈ s, (Polynomial.X - Polynomial.C (algebraMap S D a))) = Multiset.map (fun a => algebraMap S D a) s.val := by
      rw [ Polynomial.roots_prod ];
      · erw [ Multiset.bind_congr fun x hx => by erw [ Polynomial.roots_X_sub_C ] ] ; simp_all only [ne_eq, Multiset.bind_singleton, s];
  ...
  rw [ Set.ncard_eq_toFinset_card _ ] ; simp_all only [ne_eq, ge_iff_le, Set.toFinite_toFinset, Set.toFinset_card, Multiset.card_map, Finset.card_val, le_refl,
      s];
...
theorem selmerPoly_isMorse
  ...
  have h1 : (Polynomial.rootSet (selmerPoly R n) S).ncard ≤ (Polynomial.map (algebraMap R D) (selmerPoly R n)).roots.card := by
