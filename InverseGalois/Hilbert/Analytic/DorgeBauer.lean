...
lemma monic_int_factor_of_monic_int_dvd {f : Polynomial ℤ} {g : Polynomial ℚ}
  ...
  -- By Gauss's Lemma, since $g$ is monic and divides $f$ in $\mathbb{Q}[x]$, there exists a monic polynomial $g'$ in $\mathbb{Z}[x]$ such that $g' = g$.
  obtain ⟨g', hg', hg'_monic⟩ : ∃ g' : Polynomial ℤ, g'.Monic ∧ g = g'.map (Int.castRingHom ℚ) := by
    ...
    have h_alg_int : ∀ c ∈ g.support, ∃ z : ℤ, g.coeff c = z := by
      intro c hc; specialize h_alg_int c hc; obtain ⟨ z, hz ⟩ := IsIntegrallyClosed.isIntegral_iff.mp h_alg_int; use z; aesop;
    ...
    · rw [ Polynomial.Monic, Polynomial.leadingCoeff, Polynomial.natDegree_eq_of_degree_eq_some ];
      ...
      · rw [ Polynomial.degree_eq_of_le_of_coeff_ne_zero ] <;> norm_num;
        · exact le_trans ( Polynomial.degree_sum_le _ _ ) ( Finset.sup_le fun x hx => Polynomial.degree_C_mul_X_pow_le _ _ |> le_trans <| WithBot.coe_le_coe.mpr <| Polynomial.le_natDegree_of_mem_supp _ hx );
        · exact ⟨ hg_monic.ne_zero, by specialize hz ( Polynomial.natDegree g ) ; aesop ⟩;
    · ext; aesop;
  aesop
...
lemma cauchy_root_bound {p : Polynomial ℂ} (hp : p.Monic) {α : ℂ} (hα : p.IsRoot α) :
  ...
  · -- Since ‖α‖ > 1, we can divide both sides of the inequality by ‖α‖^{n-1} > 0: ‖α‖ ≤ ∑ᵢ ‖aᵢ‖ ≤ 1 + ∑ᵢ ‖aᵢ‖.
    have h_div : ‖α‖ ^ p.natDegree ≤ (∑ i ∈ Finset.range p.natDegree, ‖p.coeff i‖) * ‖α‖ ^ (p.natDegree - 1) := by
      ...
      exact h_div.symm ▸ le_trans ( norm_sum_le _ _ ) ( by rw [ Finset.sum_mul _ _ _ ] ; exact Finset.sum_le_sum fun i hi => by simpa [ abs_mul ] using mul_le_mul_of_nonneg_left ( pow_le_pow_right₀ ( by linarith ) ( Nat.le_sub_one_of_lt ( Finset.mem_range.mp hi ) ) ) ( by positivity ) );
...
lemma cauchy_root_bound_max {p : Polynomial ℂ} (hp : p.Monic) {α : ℂ} (hα : p.IsRoot α)
  ...
  have h_cauchy : ‖α‖ ≤ 1 + ∑ i ∈ Finset.range p.natDegree, ‖p.coeff i‖ := by
    convert cauchy_root_bound hp hα using 1;
  exact h_cauchy.trans ( by simpa using Finset.sum_le_sum fun i ( hi : i ∈ Finset.range p.natDegree ) => hB i ( Finset.mem_range.mp hi ) )
...
lemma factor_coeff_bound {g : Polynomial ℂ} (hg : g.Monic) (k : ℕ) (hk : g.natDegree = k)
  ...
  obtain ⟨l, hl⟩ : ∃ l : List ℂ, l.length = k ∧ g = List.prod (List.map (fun α => Polynomial.X - Polynomial.C α) l) := by
    have h_factor : ∃ l : Multiset ℂ, l.card = k ∧ g = Multiset.prod (Multiset.map (fun α => Polynomial.X - Polynomial.C α) l) := by
      use g.roots;
      have := Polynomial.Splits.natDegree_eq_card_roots ( show g.Splits from ?_ );
      · exact ⟨ this ▸ hk, by nth_rw 1 [ ← Polynomial.prod_multiset_X_sub_C_of_monic_of_roots_card_eq hg ] ; aesop ⟩;
      · exact IsAlgClosed.splits g;
    obtain ⟨ l, hl₁, hl₂ ⟩ := h_factor; exact ⟨ l.toList, by simpa using hl₁, by simpa using hl₂ ⟩ ;
  -- By Vieta's formulas, the coefficient of $X^j$ in $g$ is given by $(-1)^{k-j} \sigma_{k-j}(\alpha_1, \ldots, \alpha_k)$ where $\sigma_{k-j}$ is the $(k-j)$-th elementary symmetric polynomial.
  have h_vieta : g.coeff j = (-1) ^ (k - j) * Multiset.sum (Multiset.map (fun s => Multiset.prod s) (Multiset.powersetCard (k - j) (Multiset.ofList l))) := by
    have h_vieta : g.coeff j = Polynomial.coeff (Multiset.prod (Multiset.map (fun α => Polynomial.X - Polynomial.C α) (Multiset.ofList l))) j := by
      aesop;
    erw [ h_vieta, Multiset.prod_X_sub_C_coeff ];
  ...
  have h_sigma_bound : ∀ s : Multiset ℂ, (∀ α ∈ s, ‖α‖ ≤ B) → ‖Multiset.prod s‖ ≤ B ^ Multiset.card s := by
    ...
    simpa only [ pow_succ' ] using mul_le_mul hs.1 ( by apply_assumption; tauto ) ( by positivity ) ( by positivity );
  have h_sigma_bound : ∀ s ∈ Multiset.powersetCard (k - j) (Multiset.ofList l), ‖Multiset.prod s‖ ≤ B ^ (k - j) := by
    intros s hs; specialize h_sigma_bound s; simp_all [ Multiset.mem_powersetCard ] ;
    refine' h_sigma_bound fun α hα => hroots α _;
    rw [ Polynomial.eval_list_prod ] ; simp_all [ Polynomial.eval_prod, List.prod_eq_zero_iff, sub_eq_zero ] ;
    exact Multiset.mem_of_le hs.1 hα;
  have h_sigma_bound : ‖Multiset.sum (Multiset.map (fun s => Multiset.prod s) (Multiset.powersetCard (k - j) (Multiset.ofList l)))‖ ≤ (Nat.choose k (k - j)) * B ^ (k - j) := by
...
lemma int_roots_bounded {p : Polynomial ℤ} (hp : p ≠ 0) :
  ...
  convert Set.Finite.of_finite_image _ _;
  exact ℚ;
  exact fun x => x;
  · refine' Set.Finite.subset ( p.map ( Int.castRingHom ℚ ) |> Polynomial.roots |> Multiset.toFinset |> Finset.finite_toSet ) _;
...
lemma finite_specializations_for_fixed_factor
  ...
  -- By definition of polynomial division, there exists a polynomial $q(T, X)$ and a remainder $r(T, X)$ such that $f(T, X) = g(X) q(T, X) + r(T, X)$, where $\deg_X r < \deg g = k$.
  obtain ⟨q, r, hr⟩ : ∃ q r : Polynomial ℚ[X], f = g.map (algebraMap ℤ (Polynomial ℚ)) * q + r ∧ r.degree < g.natDegree := by
  ...
  -- Since $g$ does not divide $f$ over $\mathbb{Q}(T)$, the remainder $r$ must be nonzero.
  have hr_nonzero : r ≠ 0 := by
    contrapose! hg_lt; simp_all [ Polynomial.natDegree_map ] ;
    rw [ irreducible_mul_iff ] at hf_irr;
    rcases hf_irr with ( ⟨ hg₁, hg₂ ⟩ | ⟨ hg₁, hg₂ ⟩ ) <;> have := Polynomial.natDegree_eq_zero_of_isUnit hg₂ <;> simp_all [ Polynomial.natDegree_map ];
    rw [ Polynomial.natDegree_mul' ] <;> aesop;
  -- The condition $g(X) \mid f(t, X)$ is equivalent to $r(t, X) = 0$.
  have h_equiv : ∀ t : ℤ, (g.map (Int.castRingHom ℚ)) ∣ (f.map (evalRingHom (t : ℚ))) ↔ ∀ j < g.natDegree, Polynomial.eval (t : ℚ) (r.coeff j) = 0 := by
    intro t
    constructor
    intro h_div
    have h_r_zero : (r.map (evalRingHom (t : ℚ))) = 0 := by
      have h_r_zero : (g.map (Int.castRingHom ℚ)) ∣ (r.map (evalRingHom (t : ℚ))) := by
        simp_all [ dvd_add_right, dvd_add_left, dvd_mul_of_dvd_left, dvd_mul_of_dvd_right ];
        convert dvd_sub h_div ( dvd_mul_right ( map ( Int.castRingHom ℚ ) g ) ( map ( evalRingHom ( t : ℚ ) ) q ) ) using 1 ; ring!;
        simp [ mul_comm, Polynomial.map_map ];
        rw [ sub_eq_zero, mul_comm ] ; congr ; ext ; aesop;
      refine' Polynomial.eq_zero_of_dvd_of_degree_lt h_r_zero _;
      refine' lt_of_le_of_lt _ _;
      exact ↑ ( Polynomial.natDegree r );
    ...
    intro h
    have h_r_zero : (r.map (evalRingHom (t : ℚ))) = 0 := by
      ext j; by_cases hj : j < g.natDegree <;> simp_all [ Polynomial.coeff_eq_zero_of_natDegree_lt ] ;
      rw [ Polynomial.coeff_eq_zero_of_degree_lt ( lt_of_lt_of_le hr.2 ( WithBot.coe_le_coe.mpr hj ) ), Polynomial.eval_zero ];
    simp_all [ Polynomial.map_map ];
    exact dvd_mul_of_dvd_left ( by erw [ show ( evalRingHom ( t : ℚ ) |> RingHom.comp <| Int.castRingHom ℚ[X] ) = ( Int.castRingHom ℚ ) by ext; simp ] ) _;
  -- Since $r$ is nonzero, there exists some $j < g.natDegree$ such that $r.coeff j \neq 0$.
  obtain ⟨j, hj_lt, hj_nonzero⟩ : ∃ j < g.natDegree, r.coeff j ≠ 0 := by
    contrapose! hr_nonzero;
    ext j; by_cases hj : j < g.natDegree <;> simp_all [ Polynomial.coeff_eq_zero_of_degree_lt ] ;
    rw [ Polynomial.coeff_eq_zero_of_degree_lt ( lt_of_lt_of_le hr.2 ( WithBot.coe_le_coe.mpr hj ) ), Polynomial.coeff_zero ];
  refine Set.Finite.subset ( r.coeff j |> Polynomial.roots |> Multiset.toFinset |> Finset.finite_toSet |> Set.Finite.image ( fun x : ℚ => ⌊x⌋ ) ) ?_;
  intro t ht; specialize h_equiv t; aesop;
...
lemma finite_boundedMonicPolys (k : ℕ) (B : ℤ) :
  ...
  refine Set.Finite.subset ?_ ( show boundedMonicPolys k B ⊆ { g : Polynomial ℤ | ∃ c : Fin k → ℤ, g = Polynomial.monomial k 1 + ∑ i : Fin k, Polynomial.monomial i ( c i ) ∧ ∀ i : Fin k, |c i| ≤ B } from ?_ );
  · refine Set.Finite.subset ( Set.toFinite ( Finset.image ( fun c : Fin k → ℤ => ( monomial k ) 1 + ∑ i : Fin k, ( monomial i ) ( c i ) ) ( Finset.Icc ( -B • 1 ) ( B • 1 ) ) ) ) ?_;
    intro g hg; obtain ⟨ c, rfl, hc ⟩ := hg; refine' Finset.mem_image.mpr ⟨ c, _, rfl ⟩ ; simp [ Pi.le_def, abs_le ] at * ; aesop;
  · intro g hg;
    refine' ⟨ fun i => g.coeff i, _, _ ⟩ <;> have := hg.2.1 <;> simp_all [ ← Polynomial.C_mul_X_pow_eq_monomial ];
    · conv_lhs => rw [ g.as_sum_range_C_mul_X_pow ] ; simp [ this, Finset.sum_range, Fin.sum_univ_castSucc ] ;
      rw [ add_comm, ← this, hg.1.coeff_natDegree ] ; norm_num;
...
lemma infinite_complement_of_sublinear_ncard {S : Set ℤ} {C : ℝ} {α : ℝ}
  ...
  obtain ⟨m, hm⟩ : ∃ m : ℕ, ∀ N : ℕ, 0 < N → (S ∩ Set.Icc (-N : ℤ) N).ncard ≥ (2 * N + 1) - m := by
    ...
    use m
    intro N hN_pos
    have h_card : (S ∩ Set.Icc (-N : ℤ) N).ncard + (Sᶜ ∩ Set.Icc (-N : ℤ) N).ncard = 2 * N + 1 := by
      rw [ ← Set.ncard_union_eq ];
      · rw [ show S ∩ Set.Icc ( -N : ℤ ) N ∪ Sᶜ ∩ Set.Icc ( -N : ℤ ) N = Set.Icc ( -N : ℤ ) N by ext x; by_cases hx : x ∈ S <;> aesop ] ; norm_num [ Set.ncard_eq_toFinset_card' ] ; ring;
        norm_cast;
      · exact Set.disjoint_left.mpr fun x hx₁ hx₂ => hx₂.1 hx₁.1;
    simp +zetaDelta at *;
    linarith [ show Set.ncard ( Sᶜ ∩ Set.Icc ( -N : ℤ ) N ) ≤ m from hm.symm ▸ Set.ncard_le_ncard ( show Sᶜ ∩ Set.Icc ( -N : ℤ ) N ⊆ Sᶜ from fun x hx => hx.1 ) h ];
  -- Choose N large enough such that 2N + 1 - m > C * N^α.
  obtain ⟨N, hN⟩ : ∃ N : ℕ, 0 < N ∧ (2 * N + 1 - m : ℝ) > C * N ^ α := by
    -- Since $\alpha < 1$, we have $\lim_{N \to \infty} \frac{C \cdot N^\alpha}{N} = 0$.
    have h_lim : Filter.Tendsto (fun N : ℕ => C * (N : ℝ) ^ α / (N : ℝ)) Filter.atTop (nhds 0) := by
      have h_lim : Filter.Tendsto (fun N : ℕ => C * (N : ℝ) ^ (α - 1)) Filter.atTop (nhds 0) := by
        simpa using tendsto_const_nhds.mul ( tendsto_rpow_neg_atTop ( by linarith : 0 < - ( α - 1 ) ) |> Filter.Tendsto.comp <| tendsto_natCast_atTop_atTop );
      refine h_lim.congr' ( by filter_upwards [ Filter.eventually_gt_atTop 0 ] with N hN using by rw [ Real.rpow_sub_one ( by positivity ) ] ; ring );
    have := h_lim.eventually ( gt_mem_nhds <| show 0 < 1 / 2 by norm_num );
    rw [ Filter.eventually_atTop ] at this; rcases this with ⟨ N, hN ⟩ ; exact ⟨ N + m + 1, by positivity, by have := hN ( N + m + 1 ) ( by linarith ) ; rw [ div_lt_iff₀ ( by positivity ) ] at this; push_cast at *; nlinarith ⟩ ;
  specialize hm N hN.1;
  rw [ ge_iff_le, tsub_le_iff_right ] at hm;
  linarith [ hS N hN.1, show ( 2 * N + 1 : ℝ ) ≤ ( S ∩ Set.Icc ( -N : ℤ ) N |> Set.ncard : ℝ ) + m by exact_mod_cast hm ]
...
lemma specialization_at_int_nonzero
  ...
  -- By definition of $f$ being irreducible and having degree at least 2, $(X - C x₀)$ cannot divide $f$ in $\mathbb{Q}[T][X]$.
  have h_not_div : ¬(Polynomial.X - Polynomial.C (Polynomial.C (x₀ : ℚ)) ∣ f) := by
    contrapose! hf_deg; have := Polynomial.degree_le_of_dvd hf_deg; simp_all [ Polynomial.degree_sub_eq_left_of_degree_lt ] ;
    cases hf_deg ; simp_all [ Polynomial.degree_sub_eq_left_of_degree_lt ];
    rw [ irreducible_mul_iff ] at hf_irr;
    rcases hf_irr with ( ⟨ h₁, h₂ ⟩ | ⟨ h₁, h₂ ⟩ ) <;> have := Polynomial.degree_eq_zero_of_isUnit h₂ <;> simp_all [ Polynomial.degree_sub_eq_left_of_degree_lt ];
    · erw [ Polynomial.natDegree_mul' ] <;> norm_num [ Polynomial.natDegree_sub_eq_left_of_natDegree_lt ];
      · rw [ Polynomial.natDegree_eq_zero_iff_degree_le_zero.mpr ( le_of_eq this ) ] ; norm_num;
      · exact ⟨ Polynomial.X_sub_C_ne_zero _, by aesop_cat ⟩;
    · exact absurd this ( by erw [ Polynomial.degree_X_sub_C ] ; norm_num );
...
lemma int_root_locus_small_sublinear
  ...
  -- Let $m := \sum_{i=0}^{P.natDegree} (P.coeff i).natDegree$.
  set m := ∑ i ∈ Finset.range (P.natDegree + 1), (P.coeff i).natDegree;
  refine' ⟨ 3 * ( m + 1 ), _, _ ⟩ <;> norm_num;
  · positivity;
  · intro N hN_pos
    set s := Nat.sqrt N
    have h_card : Set.ncard ({t : ℤ | ∃ y : ℤ, y^2 ≤ (N : ℤ) ∧ (P.map (Polynomial.evalRingHom t)).IsRoot y} ∩ Set.Icc (-(N : ℤ)) (N : ℤ)) ≤ (2 * s + 1) * m := by
      have h_card : ∀ y : ℤ, y^2 ≤ (N : ℤ) → Set.ncard ({t : ℤ | (P.map (Polynomial.evalRingHom t)).IsRoot y} ∩ Set.Icc (-(N : ℤ)) (N : ℤ)) ≤ m := by
        intro y hy
        ...
        have hQ_natDegree : (P.eval (Polynomial.C y)).natDegree ≤ m := by
          ...
          intro i hi; by_cases hi' : P.coeff i = 0 <;> simp_all [ Polynomial.natDegree_mul' ] ;
          refine' le_trans _ ( Finset.single_le_sum ( fun i _ => Nat.zero_le ( Polynomial.natDegree ( P.coeff i ) ) ) ( Finset.mem_range.mpr ( Nat.lt_succ_of_le hi ) ) );
          exact le_trans ( Polynomial.natDegree_mul_le .. ) ( by norm_num );
        have hQ_roots : Set.ncard ({t : ℤ | (P.eval (Polynomial.C y)).IsRoot t} ∩ Set.Icc (-(N : ℤ)) (N : ℤ)) ≤ (P.eval (Polynomial.C y)).natDegree := by
          have hQ_roots : Set.ncard ({t : ℤ | (P.eval (Polynomial.C y)).IsRoot t} ∩ Set.Icc (-(N : ℤ)) (N : ℤ)) ≤ (P.eval (Polynomial.C y)).roots.toFinset.card := by
        ...
        ext; simp [eval_map_evalRingHom_eq];
      have h_card_union : Set.ncard (⋃ y ∈ Finset.Icc (-(s : ℤ)) (s : ℤ), {t : ℤ | (P.map (Polynomial.evalRingHom t)).IsRoot y} ∩ Set.Icc (-(N : ℤ)) (N : ℤ)) ≤ (2 * s + 1) * m := by
        have h_card_union : ∀ {S : Finset ℤ}, (∀ y ∈ S, y^2 ≤ (N : ℤ)) → Set.ncard (⋃ y ∈ S, {t : ℤ | (P.map (Polynomial.evalRingHom t)).IsRoot y} ∩ Set.Icc (-(N : ℤ)) (N : ℤ)) ≤ S.card * m := by
          intros S hS; induction' S using Finset.induction with y S hyS ih <;> simp_all [ Set.ncard_union_le ] ;
      ...
      · intro t ht; obtain ⟨ y, hy₁, hy₂ ⟩ := ht.1; exact Set.mem_iUnion₂.mpr ⟨ y, Finset.mem_Icc.mpr ⟨ by nlinarith [ Nat.lt_succ_sqrt N ], by nlinarith [ Nat.lt_succ_sqrt N ] ⟩, hy₂, ht.2 ⟩ ;
      · exact Set.Finite.subset ( Set.finite_Icc ( -N : ℤ ) N ) fun x hx => by aesop;
    refine' le_trans ( Nat.cast_le.mpr h_card ) _;
    norm_num [ ← Real.sqrt_eq_rpow ];
    nlinarith only [ show ( s : ℝ ) ≤ Real.sqrt N by exact Real.le_sqrt_of_sq_le <| mod_cast Nat.sqrt_le' N, show ( 1 : ℝ ) ≤ Real.sqrt N by exact Real.le_sqrt_of_sq_le <| mod_cast hN_pos, Real.mul_self_sqrt <| Nat.cast_nonneg N ]
...
lemma int_root_specialization_abs_le
  ...
  have h_cauchy_bound : ∀ (p : Polynomial ℂ), p.Monic → ∀ (y : ℂ), p.IsRoot y → ‖y‖ ≤ 1 + ∑ i ∈ Finset.range p.natDegree, ‖p.coeff i‖ := by
    -- Apply the Cauchy bound to the polynomial $P(t, Y)$ using the fact that it is monic.
    intros p hp y hy
    apply cauchy_root_bound hp hy;
  convert h_cauchy_bound ( Polynomial.map ( algebraMap ℤ ℂ ) ( P.map ( Polynomial.evalRingHom t ) ) ) _ ( y : ℂ ) _ using 1 <;> norm_num [ Polynomial.coeff_map ];
  · rw [ Polynomial.natDegree_map_of_leadingCoeff_ne_zero ] <;> norm_num [ hP_monic ];
    intro h; simp_all [ Polynomial.ext_iff ] ;
    specialize h ( Polynomial.natDegree P ) ; aesop;
  · convert Polynomial.Monic.map ( Int.castRingHom ℂ ) ( Polynomial.Monic.map ( Polynomial.evalRingHom t ) hP_monic ) using 1;
  · exact hy
...
lemma sublinear_finite_cover {n : ℕ} (S : ℕ → Set ℤ) (T : Fin n → ℕ → Set ℤ)
  ...
  obtain ⟨C, α, hC_pos, hα_nonneg, hα_lt_1, h_bound⟩ : ∃ (C : Fin n → ℝ) (α : Fin n → ℝ), (∀ j, 0 < C j) ∧ (∀ j, 0 ≤ α j) ∧ (∀ j, α j < 1) ∧ (∀ j, ∀ N : ℕ, 0 < N → (Set.ncard (T j N)) ≤ C j * (N : ℝ) ^ (α j)) := by
    exact ⟨ fun j => Classical.choose ( hsub j ), fun j => Classical.choose_spec ( hsub j ) |> Classical.choose, fun j => Classical.choose_spec ( hsub j ) |> Classical.choose_spec |> And.left, fun j => Classical.choose_spec ( hsub j ) |> Classical.choose_spec |> And.right |> And.left, fun j => Classical.choose_spec ( hsub j ) |> Classical.choose_spec |> And.right |> And.right |> And.left, fun j N hN => Classical.choose_spec ( hsub j ) |> Classical.choose_spec |> And.right |> And.right |> And.right |> fun h => h N hN ⟩;
  by_cases hn : n = 0;
  · subst hn; use 1, 0; aesop;
  · refine' ⟨ ∑ j, C j, Finset.univ.sup' ( Finset.univ_nonempty_iff.mpr ⟨ ⟨ 0, Nat.pos_of_ne_zero hn ⟩ ⟩ ) fun j => α j, _, _, _, _ ⟩ <;> norm_num [ Finset.sum_nonneg, hC_pos, hα_nonneg, hα_lt_1 ];
    ...
    · intro N hN_pos
      ...
      · push_cast [ Finset.sum_mul _ _ _ ];
        exact Finset.sum_le_sum fun i _ => le_trans ( h_bound i N hN_pos ) ( mul_le_mul_of_nonneg_left ( Real.rpow_le_rpow_of_exponent_le ( mod_cast hN_pos ) ( Finset.le_sup' ( fun j => α j ) ( Finset.mem_univ i ) ) ) ( le_of_lt ( hC_pos i ) ) )
...
lemma int_root_large_magnitude_window
  ...
  obtain ⟨K, e, hK⟩ : ∃ K e : ℤ, 0 < K ∧ ∀ N : ℕ, 0 < N → ∀ t : ℤ, -↑N ≤ t → t ≤ ↑N → ∀ y : ℤ, (map (evalRingHom t) P).IsRoot y → |(y : ℝ)| ≤ 1 + ∑ i ∈ Finset.range P.natDegree, (∑ j ∈ Finset.range ((P.coeff i).natDegree + 1), |(P.coeff i).coeff j| : ℝ) * (N : ℝ) ^ ((P.coeff i).natDegree) := by
    refine' ⟨ 1, 0, by norm_num, fun N hN t ht₁ ht₂ y hy => le_trans ( int_root_specialization_abs_le P hP_monic t y hy ) _ ⟩;
    gcongr;
    rw [ Polynomial.eval_eq_sum_range ];
    norm_cast;
    refine' le_trans ( Finset.abs_sum_le_sum_abs _ _ ) _;
    norm_num [ abs_mul, Finset.sum_mul _ _ _ ];
    exact Finset.sum_le_sum fun i hi => mul_le_mul_of_nonneg_left ( pow_le_pow_left₀ ( abs_nonneg _ ) ( abs_le.mpr ⟨ by linarith, by linarith ⟩ ) _ |> le_trans <| pow_le_pow_right₀ ( by linarith ) <| Finset.mem_range_succ_iff.mp hi ) <| abs_nonneg _;
  refine' ⟨ 1 + ∑ i ∈ Finset.range P.natDegree, ∑ j ∈ Finset.range ( ( P.coeff i |> Polynomial.natDegree ) + 1 ), |( P.coeff i |> Polynomial.coeff ) j|, ∑ i ∈ Finset.range P.natDegree, ( P.coeff i |> Polynomial.natDegree ), _, _ ⟩ <;> norm_num;
  · exact add_pos_of_pos_of_nonneg zero_lt_one <| Finset.sum_nonneg fun _ _ => Finset.sum_nonneg fun _ _ => abs_nonneg _;
  · intro N hN t ht₁ ht₂ y hy; specialize hK; have := hK.2 N hN t ht₁ ht₂ y hy; simp_all [ add_mul, Finset.sum_mul _ _ _ ] ;
    refine' le_trans this ( add_le_add _ _ );
    · exact_mod_cast Nat.one_le_pow _ _ hN;
    · exact Finset.sum_le_sum fun i hi => Finset.sum_le_sum fun j hj => mul_le_mul_of_nonneg_left ( pow_le_pow_right₀ ( mod_cast hN ) ( Finset.single_le_sum ( fun a _ => Nat.zero_le ( Polynomial.natDegree ( P.coeff a ) ) ) hi ) ) ( abs_nonneg _ )
...
lemma graph_integer_points_sublinear_false :
  ...
  · intro p hp; by_contra! h; have := h 1; have := h 2; have := h 3; norm_num [ mul_assoc, mul_comm Real.pi ] at *;
    -- Since $p$ is a polynomial of degree at most 2, we can write it as $p(x) = ax^2 + bx + c$.
    obtain ⟨a, b, c, ha⟩ : ∃ a b c : ℝ, p = Polynomial.C a * Polynomial.X ^ 2 + Polynomial.C b * Polynomial.X + Polynomial.C c := by
      exact ⟨ p.coeff 2, p.coeff 1, p.coeff 0, Polynomial.as_sum_range_C_mul_X_pow p ▸ by interval_cases p.natDegree <;> norm_num [ Finset.sum_range_succ' ] ⟩;
    norm_num [ ha, Real.sin_three_mul ] at *;
    have := h ( 3 / 2 ) ( by norm_num ) ; norm_num [ ( by ring : 3 / 2 * Real.pi = Real.pi + Real.pi / 2 ), Real.sin_add ] at this ; linarith;
  · intro x hx y hy hxy;
    -- Since $\sin(\pi t) = 0$ for all integers $t$, the set $\{t \in \mathbb{Z} \mid 1 \leq t \leq N \text{ and } \exists m \in \mathbb{Z}, \sin(\pi t) = m\}$ is just the set of integers from $1$ to $N$.
    have h_set : ∀ N : ℕ, 0 < N → {t : ℤ | 1 ≤ t ∧ t ≤ N ∧ ∃ m : ℤ, Real.sin (Real.pi * t) = m} = Set.Icc 1 (N : ℤ) := by
      intro N hN; ext t; simp [Set.mem_setOf_eq];
    ...
    obtain ⟨N, hN⟩ : ∃ N : ℕ, 0 < N ∧ x * (N : ℝ) ^ y < N := by
      ...
      obtain ⟨N, hN⟩ : ∃ N : ℕ, 0 < N ∧ x < (N : ℝ) ^ (1 - y) := by
        ...
        exact Filter.eventually_atTop.mp ( h_lim.eventually_gt_atTop x ) |> fun ⟨ N, hN ⟩ => ⟨ N + 1, Nat.succ_pos _, hN _ ( Nat.le_succ _ ) ⟩;
      exact ⟨ N, hN.1, by convert mul_lt_mul_of_pos_right hN.2 ( Real.rpow_pos_of_pos ( Nat.cast_pos.mpr hN.1 ) y ) using 1 ; rw [ ← Real.rpow_add ( Nat.cast_pos.mpr hN.1 ) ] ; norm_num ⟩;
    use N;
    convert hN using 2;
    rw [ show { t : ℤ | 1 ≤ ( t : ℝ ) ∧ ( t : ℝ ) ≤ N ∧ ∃ m : ℤ, Real.sin ( Real.pi * t ) = m } = Set.Icc 1 ( N : ℤ ) from mod_cast h_set N hN.1 ] ; norm_num [ Set.ncard_eq_toFinset_card' ]
    ...
    · intro x
      ...
      · rw [div_le_iff₀ (by simp [sub_ne_zero, hx])]
        have heq : ‖gipsCexF x - gipsCexF 2‖ = |min (x-2) 0|^3 := by
          simp only [gipsCexF]; norm_num [Real.norm_eq_abs, abs_pow]
        rw [heq]
        have hmin : |min (x-2) 0| ≤ |x-2| := by
          rcases le_or_gt (x-2) 0 with h | h
          · rw [min_eq_left h]
          · rw [min_eq_right h.le]; simp
        calc |min (x-2) 0|^3 ≤ |x-2|^3 := by gcongr
          _ = |x-2|^2 * ‖x-2‖ := by rw [Real.norm_eq_abs]; ring
    ...
    · intro x
      ...
      · rw [div_le_iff₀ (by simp [sub_ne_zero, hx])]
        have heq : ‖gipsCexF' x - gipsCexF' 2‖ = 3*|min (x-2) 0|^2 := by
          simp only [gipsCexF']; norm_num [Real.norm_eq_abs, abs_mul, abs_pow]
        rw [heq]
        have hmin : |min (x-2) 0| ≤ |x-2| := by
          rcases le_or_gt (x-2) 0 with h | h
          · rw [min_eq_left h]
        ...
        calc 3*|min (x-2) 0|^2 ≤ 3*|x-2|^2 := by gcongr
          _ = 3*|x-2| * ‖x-2‖ := by rw [Real.norm_eq_abs]; ring
...
lemma gipsCexF_contDiff : ContDiff ℝ 2 gipsCexF := by
  ...
  rw [hderiv2]; unfold gipsCexF''; fun_prop
...
lemma gipsCexF_tends : Tendsto (iteratedDerivWithin 2 gipsCexF (Ici 1)) atTop (𝓝 0) := by
  ...
  rw [gipsCexF_iterDeriv x (by simp; linarith)]
...
lemma gipsCexF_int (t : ℤ) : ∃ m : ℤ, gipsCexF (t:ℝ) = (m:ℝ) := by
  ...
  · rw [min_eq_left h, min_eq_left (show t - 2 ≤ 0 by exact_mod_cast h)]; push_cast; ring
  · rw [min_eq_right h.le, min_eq_right (show (0:ℤ) ≤ t - 2 by exact_mod_cast h.le)]; push_cast; ring
...
lemma gipsCexF_nonpoly (p : Polynomial ℝ) (_hp : p.natDegree ≤ 2) :
  ...
  have hzero : ∀ x : ℝ, 2 ≤ x → p.eval x = 0 := by
    intro x hx
    have := hcon x (by simp; linarith)
    rw [← this]; simp [gipsCexF, min_eq_right (by linarith : (0:ℝ) ≤ x - 2)]
  have hpz : p = 0 := by
    apply Polynomial.eq_zero_of_infinite_isRoot
    apply Set.Infinite.mono (s := Ici (2:ℝ))
    · intro x hx; simp only [Set.mem_setOf_eq, Polynomial.IsRoot]; exact hzero x hx
    · exact Ici_infinite 2
  have h1 := hcon 1 (by simp)
  rw [hpz] at h1; simp only [gipsCexF] at h1; norm_num at h1
...
theorem graph_integer_points_sublinear_still_false :
  ...
  have hset : ∀ N : ℕ, {t : ℤ | (1 : ℝ) ≤ (t : ℝ) ∧ (t : ℝ) ≤ (N : ℝ) ∧
    ...
    constructor
    · rintro ⟨h1, h2, _⟩; exact ⟨by exact_mod_cast h1, by exact_mod_cast h2⟩
    · rintro ⟨h1, h2⟩; exact ⟨by exact_mod_cast h1, by exact_mod_cast h2, gipsCexF_int t⟩
  have hcard : ∀ N : ℕ, (Set.ncard {t : ℤ | (1 : ℝ) ≤ (t : ℝ) ∧ (t : ℝ) ≤ (N : ℝ) ∧
    ...
    rw [hset N, show Set.Icc (1:ℤ) (N:ℤ) = ↑(Finset.Icc (1:ℤ) (N:ℤ)) by simp,
      Set.ncard_coe_finset, Int.card_Icc]; simp
...
lemma graph_integer_points_sublinear_neg
  ...
  rw [ ← Set.ncard_image_of_injective _ neg_injective ] ; congr ; ext ; aesop
...
lemma three_cover (S A B D : ℕ → Set ℤ)
  ...
  · intro N x hx; specialize hcov N hx; simp_all [ Fin.exists_fin_succ ] ;
    tauto;
  · tauto;
  · exact ⟨ by obtain ⟨ C, α, hC, hα, hα', h ⟩ := hA; exact ⟨ C, hC, α, hα, hα', h ⟩, by obtain ⟨ C, α, hC, hα, hα', h ⟩ := hB; exact ⟨ C, hC, α, hα, hα', h ⟩, by obtain ⟨ C, α, hC, hα, hα', h ⟩ := hD; exact ⟨ C, hC, α, hα, hα', h ⟩ ⟩
...
lemma pos_branches_cover_sublinear (n k : ℕ) (hk : 2 ≤ k) (g : Fin n → ℝ → ℝ)
  ...
  refine' ⟨ _, _ ⟩;
  · intro N;
    exact Set.Finite.subset ( Set.finite_Icc ( 1 : ℤ ) ( N : ℤ ) ) fun x hx => by rcases Set.mem_iUnion.mp hx with ⟨ j, hj ⟩ ; exact ⟨ by exact_mod_cast hj.1, by exact_mod_cast hj.2.1 ⟩ ;
  · convert sublinear_finite_cover ( posBranchesUnion n g ) ( fun j N => { t : ℤ | ( 1 : ℝ ) ≤ ( t : ℝ ) ∧ ( t : ℝ ) ≤ N ∧ ∃ m : ℤ, g j ( t : ℝ ) = ( m : ℝ ) } ) ?_ ?_ ?_ using 1;
    · exact fun N => Set.iUnion_subset fun j => Set.subset_iUnion ( fun j => { t : ℤ | 1 ≤ ( t : ℝ ) ∧ ( t : ℝ ) ≤ N ∧ ∃ m : ℤ, g j ( t : ℝ ) = m } ) j;
...
lemma neg_branches_cover_sublinear (n k : ℕ) (hk : 2 ≤ k) (h : Fin n → ℝ → ℝ)
  ...
  refine' ⟨ _, _ ⟩;
  · intro N
    have h_finite : ∀ j, Set.Finite {t : ℤ | (1 : ℝ) ≤ (-(t : ℝ)) ∧ (-(t : ℝ)) ≤ (N : ℝ) ∧ ∃ m : ℤ, h j (-(t : ℝ)) = m} := by
      intro j; exact Set.Finite.subset ( Set.finite_Icc ( -N : ℤ ) ( -1 : ℤ ) ) ( fun t ht => by norm_num; rcases ht with ⟨ ht₁, ht₂, m, hm ⟩ ; exact ⟨ by exact_mod_cast ( by linarith : ( -N : ℝ ) ≤ t ), by exact_mod_cast ( by linarith : ( t : ℝ ) ≤ -1 ) ⟩ ) ;
    exact Set.finite_iUnion h_finite;
  · convert sublinear_finite_cover ( negBranchesUnion n h ) ( fun j N => { t : ℤ | ( 1 : ℝ ) ≤ ( -(t : ℝ) ) ∧ ( -(t : ℝ) ) ≤ N ∧ ∃ m : ℤ, h j ( -(t : ℝ) ) = ( m : ℝ ) } ) ?_ ?_ ?_ using 1;
    · exact fun N => Set.iUnion_subset fun j => Set.subset_iUnion ( fun j => { t : ℤ | ( 1 : ℝ ) ≤ ( -(t : ℝ) ) ∧ ( -(t : ℝ) ) ≤ N ∧ ∃ m : ℤ, h j ( -(t : ℝ) ) = m } ) j;
    · intro j N; exact Set.Finite.subset ( Set.finite_Icc ( -N : ℤ ) ( -1 : ℤ ) ) fun t ht => ⟨ by exact_mod_cast ( by linarith [ ht.1, ht.2.1 ] : ( -N : ℝ ) ≤ t ), by exact_mod_cast ( by linarith [ ht.1, ht.2.1 ] : ( t : ℝ ) ≤ -1 ) ⟩ ;
...
lemma rpow_mul_analytic_package (c r : ℝ) (k : ℕ)
  ...
  · refine' ContDiffOn.mul contDiffOn_const _;
    exact ContDiffOn.rpow contDiffOn_id contDiffOn_const <| by intro x hx; linarith [ Set.mem_Ici.mp hx ] ;
  · constructor;
    · -- By definition of $f$, we know that its $k$-th derivative is $c \cdot \text{descPochhammer} \, \mathbb{R} \, k(r) \cdot x^{r-k}$.
      have h_deriv : ∀ x ∈ Set.Ici 1, iteratedDerivWithin k (fun x => c * x ^ r) (Set.Ici 1) x = c * (Polynomial.eval r (descPochhammer ℝ k)) * x ^ (r - k) := by
        intro x hx; rw [ iteratedDerivWithin_eq_iteratedDeriv ] ;
        · have := @Real.iter_deriv_rpow_const r x k; simp_all [ mul_assoc, mul_comm, mul_left_comm, iteratedDeriv_eq_iterate ] ;
        · exact uniqueDiffOn_Ici _;
        · exact ContDiffAt.mul contDiffAt_const ( ContDiffAt.rpow contDiffAt_id contDiffAt_const <| by linarith [ Set.mem_Ici.mp hx ] );
        · exact hx;
      -- Since $c \neq 0$ and $r \notin \mathbb{N}$, the term $c \cdot \text{descPochhammer} \, \mathbb{R} \, k(r)$ is nonzero.
    ...
    · -- By definition of $f$, we know that its $k$-th derivative is given by $c \cdot \prod_{i=0}^{k-1} (r - i) \cdot x^{r - k}$.
      have h_deriv : ∀ x ∈ Set.Ioi (1 : ℝ), iteratedDerivWithin k (fun x => c * x ^ r) (Set.Ici 1) x = c * (∏ i ∈ Finset.range k, (r - i)) * x ^ (r - k) := by
        intro x hx;
        convert iteratedDerivWithin_eq_iteratedDeriv _ _ _ using 1;
        · simp [ iteratedDeriv_eq_iterate, Real.iter_deriv_rpow_const ];
          rw [ descPochhammer_eval_eq_prod_range ] ; ring;
        · exact uniqueDiffOn_Ici _;
...
-/
lemma rpow_mul_hasKDerivDecay (c r : ℝ) (k : ℕ) (hk : 2 ≤ k) (hrk : r < (k : ℝ)) :
  ...
  · have h_deriv : iteratedDeriv k (fun x => c * x ^ r) x = c * (Polynomial.eval r (descPochhammer ℝ k)) * x ^ (r - k) := by
    ...
    rw [ h_deriv, abs_mul, abs_of_nonneg ( Real.rpow_nonneg ( by linarith ) _ ) ] ; ring_nf ; norm_num;
...
-/
lemma analytic_package_of_kSucc_deriv (f : ℝ → ℝ) (k : ℕ) (hk : 2 ≤ k)
  ...
  · cases' hsign with hsign hsign;
    · refine' Or.inl _;
      ...
      · convert hf.continuousOn_iteratedDerivWithin _ _ using 1;
        · norm_cast ; linarith;
        · exact uniqueDiffOn_Ici _;
      · intro x hx; specialize hsign x; simp_all [ iteratedDerivWithin_succ ] ;
        rwa [ derivWithin_of_mem_nhds ( Ici_mem_nhds hx ) ] at hsign;
    · refine' Or.inr ( strictAntiOn_of_deriv_neg ( convex_Ici 1 ) _ _ );
      · apply_rules [ ContDiffOn.continuousOn_iteratedDerivWithin ];
        exacts [ hf, by norm_cast; linarith, uniqueDiffOn_Ici _ ];
      · simp_all [ iteratedDerivWithin_succ ];
        intro x hx; specialize hsign x hx; rw [ derivWithin_of_mem_nhds ( Ici_mem_nhds hx ) ] at hsign; linarith;
