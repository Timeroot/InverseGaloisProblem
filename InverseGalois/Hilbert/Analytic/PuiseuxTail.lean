...
lemma complex_poly_eval_norm_le (p : Polynomial ℂ) (z : ℂ) :
  ...
  exact Finset.sum_le_sum fun i hi => mul_le_mul_of_nonneg_left ( pow_le_pow_left₀ ( norm_nonneg _ ) ( by linarith [ norm_nonneg z ] ) _ |> le_trans <| pow_le_pow_right₀ ( by linarith [ norm_nonneg z ] ) <| Finset.mem_range_succ_iff.mp hi ) <| norm_nonneg _
...
lemma complex_root_branch_poly_growth
  ...
  obtain ⟨C, hC⟩ : ∃ C : ℝ, ∀ z ∈ S, ∀ i < P.natDegree, ‖((P.coeff i).map (Int.castRingHom ℂ)).eval z‖ ≤ C * (1 + ‖z‖) ^ N := by
    have h_coeff_bound : ∀ i < P.natDegree, ∃ C_i : ℝ, ∀ z ∈ S, ‖((P.coeff i).map (Int.castRingHom ℂ)).eval z‖ ≤ C_i * (1 + ‖z‖) ^ N := by
      intro i hi
      use ∑ j ∈ Finset.range ((Polynomial.map (Int.castRingHom ℂ) (P.coeff i)).natDegree + 1), ‖((Polynomial.map (Int.castRingHom ℂ) (P.coeff i)).coeff j)‖;
      intro z hz
      have h_coeff_bound : ‖((P.coeff i).map (Int.castRingHom ℂ)).eval z‖ ≤ (∑ j ∈ Finset.range ((Polynomial.map (Int.castRingHom ℂ) (P.coeff i)).natDegree + 1), ‖((Polynomial.map (Int.castRingHom ℂ) (P.coeff i)).coeff j)‖) * (1 + ‖z‖) ^ ((Polynomial.map (Int.castRingHom ℂ) (P.coeff i)).natDegree) := by
        convert complex_poly_eval_norm_le _ _ using 1;
      exact h_coeff_bound.trans ( mul_le_mul_of_nonneg_left ( pow_le_pow_right₀ ( by linarith [ norm_nonneg z ] ) ( Finset.le_sup ( f := fun i => Polynomial.natDegree ( Polynomial.map ( Int.castRingHom ℂ ) ( P.coeff i ) ) ) ( Finset.mem_range.mpr hi ) ) ) ( Finset.sum_nonneg fun _ _ => norm_nonneg _ ) );
    choose! C hC using h_coeff_bound;
    exact ⟨ ∑ i ∈ Finset.range P.natDegree, |C i|, fun z hz i hi => le_trans ( hC i hi z hz ) ( mul_le_mul_of_nonneg_right ( le_trans ( le_abs_self _ ) ( Finset.single_le_sum ( fun i _ => abs_nonneg ( C i ) ) ( Finset.mem_range.mpr hi ) ) ) ( by positivity ) ) ⟩;
  ...
  have h_cauchy : ∀ z ∈ S, ‖H z‖ ≤ 1 + P.natDegree * C * (1 + ‖z‖) ^ N := by
    ...
    have h_cauchy : ∀ i < P.natDegree, ‖((P.map (evalIntPolyComplex z)).coeff i)‖ ≤ C * (1 + ‖z‖) ^ N := by
      intro i hi; specialize hC z hz i hi; simp_all [ Polynomial.coeff_map ] ;
      convert hC using 1;
    have := @cauchy_root_bound_max ( Polynomial.map ( evalIntPolyComplex z ) P );
    simp_all [ mul_assoc, Polynomial.natDegree_map_of_leadingCoeff_ne_zero ];
    exact this ( by exact Polynomial.Monic.map ( evalIntPolyComplex z ) hP_monic ) ( hroot z hz ) h_cauchy;
  refine' ⟨ 1 + P.natDegree * |C|, N, _, _ ⟩;
  · positivity;
  · intro z hz; specialize h_cauchy z hz; cases abs_cases C <;> nlinarith [ show 0 ≤ ( P.natDegree : ℝ ) * ( 1 + ‖z‖ ) ^ N by positivity, show ( 1 + ‖z‖ ) ^ N ≥ 1 by exact one_le_pow₀ ( by linarith [ norm_nonneg z ] ) ] ;
...
lemma H_root_on_ball
  ...
  have hF_zero : ∀ y : ℝ, x - x / 2 < y ∧ y < x + x / 2 → (P.map (evalIntPolyComplex (y : ℂ))).eval (H (y : ℂ)) = 0 := by
    intros y hy; rw [ hagree y ( by linarith ) ] ; exact (by
    convert congr_arg ( ( ↑ ) : ℝ → ℂ ) ( hroot y ( by linarith [ show ( T₀ : ℝ ) ≤ y by linarith ] ) ) using 1 ; norm_num [ evalIntPolyComplex_ofReal ]);
  have hF_zero : AnalyticOnNhd ℂ (fun z : ℂ => (P.map (evalIntPolyComplex z)).eval (H z)) (Metric.ball (x : ℂ) (x / 2)) := by
    apply_rules [ DifferentiableOn.analyticOnNhd, Metric.isOpen_ball ];
    have hF_zero : DifferentiableOn ℂ (fun z : ℂ => (P.map (evalIntPolyComplex z)).eval (H z)) (Metric.ball (x : ℂ) (x / 2)) := by
      have hH_diff : DifferentiableOn ℂ H (Metric.ball (x : ℂ) (x / 2)) := by
        exact hcont x hx |> fun h => h.differentiableOn
      convert ( evalIntPolyComplex_eval_contDiff P |> ContDiff.differentiable <| by norm_num ) |> Differentiable.comp_differentiableOn <| differentiableOn_id.prodMk hH_diff using 1;
    exact hF_zero;
  ...
  · exact ⟨ by rw [ abs_of_nonneg ( by linarith [ show 0 ≤ min ε ( x / 2 ) by exact le_min ε_pos.le ( by linarith ) ] ) ] ; linarith [ min_le_left ε ( x / 2 ), min_le_right ε ( x / 2 ) ], by linarith [ show 0 < min ε ( x / 2 ) by exact lt_min ε_pos ( by linarith ) ] ⟩;
  · convert ‹∀ y : ℝ, x - x / 2 < y ∧ y < x + x / 2 → Polynomial.eval ( H ↑y ) ( Polynomial.map ( evalIntPolyComplex ↑y ) P ) = 0› ( x + Min.min ε ( x / 2 ) / 2 ) ⟨ by linarith [ show 0 < Min.min ε ( x / 2 ) by exact lt_min ε_pos ( by linarith ) ], by linarith [ show 0 < Min.min ε ( x / 2 ) by exact lt_min ε_pos ( by linarith ), min_le_left ε ( x / 2 ), min_le_right ε ( x / 2 ) ] ⟩ using 1 ; norm_num
...
lemma H_poly_growth_on_spheres
  ...
  have hH_root : ∀ x : ℝ, T₁ ≤ x → ∀ z ∈ Metric.closedBall (x : ℂ) (x / 2), (P.map (evalIntPolyComplex z)).eval (H z) = 0 := by
    intros x hx z hz
    ...
    have h_cont : ContinuousOn (fun z => (P.map (evalIntPolyComplex z)).eval (H z)) (Metric.closedBall (x : ℂ) (x / 2)) := by
      ...
      have h_cont : ContinuousOn (fun p : ℂ × ℂ => (P.map (evalIntPolyComplex p.1)).eval p.2) (Metric.closedBall (x : ℂ) (x / 2) ×ˢ Set.univ) := by
    ...
    have h_cont : ∀ z ∈ Metric.closedBall (x : ℂ) (x / 2), (P.map (evalIntPolyComplex z)).eval (H z) = 0 := by
      intro z hz
      have h_seq : ∃ seq : ℕ → ℂ, (∀ n, seq n ∈ Metric.ball (x : ℂ) (x / 2)) ∧ Filter.Tendsto seq Filter.atTop (nhds z) := by
        have h_seq : z ∈ closure (Metric.ball (x : ℂ) (x / 2)) := by
          rw [ closure_ball ] <;> norm_num [ show x ≠ 0 by linarith ] at * ; simp_all only;
        rwa [ mem_closure_iff_seq_limit ] at h_seq
      obtain ⟨ seq, hseq₁, hseq₂ ⟩ := h_seq;
      exact tendsto_nhds_unique ( h_cont.continuousWithinAt hz |> fun h => h.tendsto.comp <| Filter.tendsto_inf.mpr ⟨ hseq₂, Filter.tendsto_principal.mpr <| Filter.Eventually.of_forall fun n => by simpa using hseq₁ n |> fun h => Metric.mem_closedBall.mpr <| le_of_lt h ⟩ ) <| tendsto_const_nhds.congr' <| by filter_upwards [ Filter.eventually_gt_atTop 0 ] with n hn; simp_all only [Metric.mem_closedBall, Metric.mem_ball, Function.comp_apply];
    exact h_cont z hz;
  obtain ⟨ C, N, hC, hCbound ⟩ := DorgeBauer.complex_root_branch_poly_growth P hP_monic hP_deg { z : ℂ | ∃ x : ℝ, T₁ ≤ x ∧ z ∈ Metric.closedBall ( x : ℂ ) ( x / 2 ) } H ( fun z hz => by obtain ⟨ x, hx₁, hx₂ ⟩ := hz; exact hH_root x hx₁ z hx₂ );
...
lemma removable_pole_factor
  ...
  · refine' DifferentiableOn.analyticOnNhd _ ( Metric.isOpen_ball );
    ...
    · refine' ⟨ Cf, Set.forall_mem_image.2 fun w hw => _ ⟩;
      simp_all [ norm_mul ];
      exact le_trans ( mul_le_mul_of_nonneg_left ( hFgrow w hw.2 hw.1 ) ( by positivity ) ) ( by rw [ mul_div_cancel₀ _ ( by exact pow_ne_zero _ ( norm_ne_zero_iff.mpr hw.2 ) ) ] );
  · intro w hw₁ hw₂; rw [ Function.update_of_ne ( by simp_all only [norm_pos_iff, ne_eq, not_false_eq_true] ) ] ; rw [ mul_div_cancel_left₀ _ ( pow_ne_zero _ ( by simp_all only [norm_pos_iff, ne_eq, not_false_eq_true] ) ) ] ;
...
-/
lemma laurent_cpow_term (z : ℂ) (hz : 0 < z.re) (e : ℕ) (he : 1 ≤ e) (j Kf : ℕ) :
    (z⁻¹ ^ ((e : ℂ)⁻¹)) ^ j / (z⁻¹ ^ ((e : ℂ)⁻¹)) ^ Kf
      = z ^ ((((Kf : ℝ) - j) / e : ℝ) : ℂ) := by
  rw [ ← Complex.cpow_nat_mul, ← Complex.cpow_nat_mul ];
  rw [ Complex.cpow_def_of_ne_zero, Complex.cpow_def_of_ne_zero ] <;> norm_num [ show z ≠ 0 by rintro rfl; norm_num at hz ];
  rw [ ← Complex.exp_sub, Complex.log_inv ] ; ring;
  · rw [ Complex.cpow_def_of_ne_zero ( by rintro rfl; norm_num at hz ) ] ; ring;
...
-/
lemma laurent_G_poly_of_vanishing (ρ : ℝ) (hρ : 0 < ρ) (G : ℂ → ℂ) (Kf : ℕ)
  ...
  specialize this ( show DifferentiableOn ℂ G ( Metric.ball 0 ρ ) from hG.differentiableOn ) ( show w ∈ Metric.ball 0 ρ from by simpa using hw );
  rw [ ← this, tsum_eq_sum ];
  exacts [ Finset.sum_congr rfl fun n hn => by simp [ div_eq_inv_mul, mul_assoc, mul_comm, mul_left_comm ], fun n hn => by rw [ hvan n ( by simpa using hn ) ] ; simp ]
...
lemma separable_ramified_root_section
  ...
  rw [ Complex.log_re ] ; norm_num ; exact Real.log_lt_log ( by linarith ) ( by linarith ) ;) (H (T₁ : ℂ)) (by
  ...
  · -- Apply `root_comp_holomorphic` with the given conditions.
    have h_analytic : AnalyticOnNhd ℂ (fun w => g (-(e : ℂ) * Complex.log w)) {w : ℂ | 0 < ‖w‖ ∧ ‖w‖ < Real.exp (-(Real.log B) / e)} := by
      have hφ : DifferentiableOn ℂ (fun w => (w⁻¹) ^ e) {w : ℂ | 0 < ‖w‖ ∧ ‖w‖ < Real.exp (-(Real.log B) / e)} := by
        exact DifferentiableOn.pow ( differentiableOn_id.inv fun w hw => by simp_all only [Metric.mem_ball, norm_pos_iff, ne_eq, Set.mem_setOf_eq, id_eq, not_false_eq_true] ) _
      have hF : ContinuousOn (fun w => g (-(e : ℂ) * Complex.log w)) {w : ℂ | 0 < ‖w‖ ∧ ‖w‖ < Real.exp (-(Real.log B) / e)} := by
        convert DorgeBauer.periodic_log_comp_continuous g B hB e he hg.1 hg.2.2.2 using 1
      have hroot : ∀ w : ℂ, 0 < ‖w‖ → ‖w‖ < Real.exp (-(Real.log B) / e) → (Q.map (evalIntPolyComplex ((w⁻¹) ^ e))).eval (g (-(e : ℂ) * Complex.log w)) = 0 := by
        intros w hw_pos hw_lt
        have h_exp : Complex.exp (-(e : ℂ) * Complex.log w) = (w⁻¹) ^ e := by
          have := Complex.exp_int_mul ( -Complex.log w ) e; simp_all [ Complex.exp_neg, Complex.exp_log ] ;
        rw [ ← h_exp, hg.2.2.1 ];
        have := Real.log_lt_log hw_pos hw_lt;
        norm_num [ Complex.log_re ] at *;
        rw [ lt_div_iff₀ ] at this <;> first | positivity | linarith;
      have := DorgeBauer.root_comp_holomorphic Q { w : ℂ | 0 < ‖w‖ ∧ ‖w‖ < Real.exp ( -Real.log B / e ) } ( isOpen_Ioi.preimage continuous_norm |> IsOpen.inter <| isOpen_Iio.preimage continuous_norm ) ( fun w => w⁻¹ ^ e ) ( fun w => g ( -↑e * Complex.log w ) ) hφ hF ( fun w hw => hroot w hw.1 hw.2 ) ( fun w hw => ?_ );
      · exact this.analyticOnNhd ( isOpen_Ioi.preimage continuous_norm |> IsOpen.inter <| isOpen_Iio.preimage continuous_norm );
      · convert hQsep ( w⁻¹ ^ e ) _ using 1;
        simp_all [ Real.exp_neg, Real.exp_log ( zero_lt_one.trans_le hB ) ];
        rw [ lt_inv_comm₀ ] <;> try positivity;
        · refine' lt_of_lt_of_le ( pow_lt_pow_left₀ hw.2 ( norm_nonneg _ ) ( by positivity ) ) _;
          rw [ ← Real.exp_nat_mul, mul_comm, div_mul_cancel₀ _ ( by positivity ), Real.exp_neg, Real.exp_log ( by positivity ) ];
        · exact pow_pos ( norm_pos_iff.mpr hw.1 ) _;
    exact h_analytic;
  · refine' ⟨ _, _ ⟩;
    · intro w hw₁ hw₂; convert hg.2.2.1 ( - ( e : ℂ ) * Complex.log w ) _ using 1 <;> norm_num [ Complex.exp_neg, Complex.exp_log, hw₁.ne' ] ; ring;
      · rw [ Complex.exp_nat_mul, Complex.exp_log ( by simp_all only [Metric.mem_ball, norm_pos_iff, ne_eq, not_false_eq_true] ) ] ; ring;
      · rw [ Complex.log_re ];
        have := Real.log_lt_log hw₁ hw₂ ; norm_num at * ; nlinarith [ show ( e : ℝ ) ≥ 1 by norm_cast, mul_div_cancel₀ ( -Real.log B ) ( by positivity : ( e : ℝ ) ≠ 0 ) ] ;
    · intro x hx z hz
...
lemma root_pole_bound
    (P : Polynomial (Polynomial ℤ)) (hP_monic : P.Monic) (hP_deg : 1 ≤ P.natDegree)
    (e : ℕ) (ρ : ℝ) (hρ0 : 0 < ρ) (hρ1 : ρ ≤ 1) (F : ℂ → ℂ)
    (hFroot : ∀ w : ℂ, 0 < ‖w‖ → ‖w‖ < ρ →
        (P.map (evalIntPolyComplex ((w⁻¹) ^ e))).eval (F w) = 0) :
    ∃ (Cf : ℝ) (Kf : ℕ), 0 ≤ Cf ∧
      ∀ w : ℂ, 0 < ‖w‖ → ‖w‖ < ρ → ‖F w‖ ≤ Cf / ‖w‖ ^ Kf := by
  -- Step 1: Coefficient growth bound
  obtain ⟨C, N, hC_nonneg, hC_bound⟩ : ∃ C N, 0 ≤ C ∧ ∀ z : ℂ, (P.map (evalIntPolyComplex z)).natDegree = P.natDegree ∧ ∀ i < (P.map (evalIntPolyComplex z)).natDegree, ‖(P.map (evalIntPolyComplex z)).coeff i‖ ≤ C * (1 + ‖z‖) ^ N := by
    obtain ⟨C, N, hC_nonneg, hC_bound⟩ : ∃ C N, 0 ≤ C ∧ ∀ i < P.natDegree, ∀ z : ℂ, ‖((P.coeff i).map (Int.castRingHom ℂ)).eval z‖ ≤ C * (1 + ‖z‖) ^ N := by
      use ∑ i ∈ Finset.range P.natDegree, ∑ j ∈ Finset.range ( ( P.coeff i |> Polynomial.map ( Int.castRingHom ℂ ) |> Polynomial.natDegree ) + 1 ), ‖ ( P.coeff i |> Polynomial.map ( Int.castRingHom ℂ ) |> Polynomial.coeff ) j‖, ∑ i ∈ Finset.range P.natDegree, ( ( P.coeff i |> Polynomial.map ( Int.castRingHom ℂ ) |> Polynomial.natDegree ) );
      refine' ⟨ Finset.sum_nonneg fun _ _ => Finset.sum_nonneg fun _ _ => norm_nonneg _, fun i hi z => _ ⟩;
      refine' le_trans _ ( mul_le_mul_of_nonneg_right ( Finset.single_le_sum ( fun i _ => Finset.sum_nonneg fun j _ => norm_nonneg ( Polynomial.coeff ( Polynomial.map ( Int.castRingHom ℂ ) ( P.coeff i ) ) j ) ) ( Finset.mem_range.mpr hi ) ) ( by positivity ) );
      refine' le_trans ( complex_poly_eval_norm_le _ _ ) _;
      exact mul_le_mul_of_nonneg_left ( pow_le_pow_right₀ ( by linarith [ norm_nonneg z ] ) ( Finset.single_le_sum ( fun i _ => Nat.zero_le ( Polynomial.natDegree ( Polynomial.map ( Int.castRingHom ℂ ) ( P.coeff i ) ) ) ) ( Finset.mem_range.mpr hi ) ) ) ( Finset.sum_nonneg fun _ _ => norm_nonneg _ );
    refine' ⟨ C, N, hC_nonneg, fun z => ⟨ _, _ ⟩ ⟩;
    · rw [ Polynomial.natDegree_map_of_leadingCoeff_ne_zero ] ; simp_all only [norm_pos_iff, ne_eq, inv_pow, Polynomial.Monic.leadingCoeff, map_one, one_ne_zero, not_false_eq_true];
    · intro i hi; specialize hC_bound i; simp_all [ Polynomial.coeff_map ] ;
  ...
  have h_cauchy_bound : ∀ w : ℂ, 0 < ‖w‖ → ‖w‖ < ρ → ‖F w‖ ≤ 1 + P.natDegree * (C * (1 + ‖w⁻¹ ^ e‖) ^ N) := by
    ...
    have hQz_bound : ∀ i < (P.map (evalIntPolyComplex (w⁻¹ ^ e))).natDegree, ‖(P.map (evalIntPolyComplex (w⁻¹ ^ e))).coeff i‖ ≤ C * (1 + ‖w⁻¹ ^ e‖) ^ N := by
      exact hC_bound _ |>.2
    exact (by
    grind only [ComplexSeparableReduction.exists_complex_separable_reduction, cauchy_root_bound_max, #693d, #a720, #0d9f]);
  -- Step 3: Turn ‖z‖ into a pole in ‖w‖
  have h_pole_bound : ∀ w : ℂ, 0 < ‖w‖ → ‖w‖ < ρ → 1 + P.natDegree * (C * (1 + ‖w⁻¹ ^ e‖) ^ N) ≤ (1 + P.natDegree * C * 2 ^ N) / ‖w‖ ^ (e * N) := by
    intros w hw_pos hw_lt_ρ
    have h_norm_bound : (1 + ‖w⁻¹ ^ e‖) ^ N ≤ 2 ^ N / ‖w‖ ^ (e * N) := by
      have h_norm_bound : (1 + ‖w⁻¹ ^ e‖) ≤ 2 / ‖w‖ ^ e := by
        norm_num +zetaDelta at *;
        rw [ le_div_iff₀ ( pow_pos ( norm_pos_iff.mpr hw_pos ) _ ) ] ; nlinarith [ pow_le_pow_of_le_one ( norm_nonneg w ) ( by linarith ) ( show e ≥ 0 by positivity ), inv_mul_cancel₀ ( ne_of_gt ( pow_pos ( norm_pos_iff.mpr hw_pos ) e ) ) ];
      convert pow_le_pow_left₀ ( by positivity ) h_norm_bound N using 1 ; ring;
    rw [ le_div_iff₀ ( by positivity ) ] at *;
    nlinarith [ show 0 ≤ ( P.natDegree : ℝ ) * C by positivity, show ‖w‖ ^ ( e * N ) ≤ 1 by exact pow_le_one₀ ( by positivity ) ( by linarith ) ];
  exact ⟨ 1 + P.natDegree * C * 2 ^ N, e * N, by positivity, fun w hw hw' => le_trans ( h_cauchy_bound w hw hw' ) ( h_pole_bound w hw hw' ) ⟩
...
lemma monodromy_ramification_index
  ...
  have hP_deg : 1 ≤ P.natDegree := by
    ...
    have hmoniceval : (P.map (evalIntPolyComplex (T₁ : ℂ))).eval (H (T₁ : ℂ)) = 1 := by
      have hpeq : P = 1 := by
        have := hP_monic.natDegree_eq_zero_iff_eq_one.mp (Nat.lt_one_iff.mp hdeg)
        exact this
      simp [hpeq]
...
lemma laurent_tail_term_bound (z : ℂ) (x σ s' : ℝ) (hx : 2 ≤ x)
  ...
  have h_norm_bound : ‖z‖ ^ σ ≤ ‖z‖ ^ s' := by
    ...
    have := norm_sub_le ( z : ℂ ) ( z - x ) ; norm_num at *;
  ...
  obtain ⟨θ, hθ⟩ : ∃ θ : ℝ, ‖z‖ = θ * x ∧ 1 / 2 ≤ θ ∧ θ ≤ 2 := by
    ...
    · rw [ div_le_iff₀ ( by positivity ) ];
      have := norm_add_le ( z - x ) x ; simp_all [ Complex.normSq, Complex.norm_def ];
      grind;
  -- Now bound `θ ^ s'`. Since `1/2 ≤ θ ≤ 2`, we have `θ ^ s' ≤ 2 ^ |s'|`.
  have h_theta_bound : θ ^ s' ≤ 2 ^ |s'| := by
    by_cases hs' : 0 ≤ s';
    · rw [ abs_of_nonneg hs' ] ; exact Real.rpow_le_rpow ( by linarith ) ( by linarith ) ( by linarith );
    · rw [ abs_of_neg ( not_le.mp hs' ), Real.rpow_neg ( by linarith ) ];
      rw [ ← Real.inv_rpow ( by linarith ) ] ; exact Real.rpow_le_rpow_of_nonpos ( by linarith ) ( by linarith ) ( by linarith );
  convert h_norm_bound.trans _ using 1;
  · rw [ ← Complex.norm_cpow_real ];
  · rw [ hθ.1, Real.mul_rpow ( by linarith ) ( by linarith ) ] ; exact mul_le_mul_of_nonneg_right h_theta_bound ( by positivity ) ;
...
lemma laurent_coeff_real
  ...
  apply DorgeBauer.iteratedDeriv_im_zero_of_real_on_pos G (min ρ ((3 * Tr / 2) ^ (-(1 : ℝ) / e))) (lt_min hρ (by
  positivity)) (hG.mono (Metric.ball_subset_ball (min_le_left _ _))) (by
  intro t ht ht'; have := hrel ( ( 2 / 3 ) * t ^ ( - ( e : ℝ ) ) ) ?_ ( ( 3 * ( ( 2 / 3 ) * t ^ ( - ( e : ℝ ) ) ) / 2 : ℝ ) : ℂ ) ?_ <;> norm_num at *;
  · rw [ hFG ] at this;
    · convert congr_arg Complex.im ( show G t = ( g ( 3 * ( 2 / 3 * ( t ^ e ) ⁻¹ ) / 2 ) : ℂ ) * t ^ Kf from ?_ ) using 1;
      · norm_cast;
      · convert congr_arg ( fun x : ℂ => x * t ^ Kf ) this.symm using 1;
        · rw [ show ( 2 / ( 3 * ( 2 / 3 * ( t ^ e : ℂ ) ⁻¹ ) ) ) = ( t ^ e : ℂ ) by ring_nf; norm_num [ ht.ne' ] ] ; norm_cast ; norm_num [ ht.ne', ht.le ];
          rw [ ← Complex.cpow_natCast, ← Complex.cpow_mul, mul_inv_cancel₀ ( by norm_cast; linarith ), Complex.cpow_one ] ; norm_num [ ht.ne' ];
          · norm_num [ Complex.log_im ];
            rw [ Complex.arg_ofReal_of_nonneg ht.le ] ; norm_num ; linarith [ Real.pi_pos ];
          · norm_num [ Complex.log_im ];
            rw [ Complex.arg_ofReal_of_nonneg ht.le ] ; norm_num ; linarith [ Real.pi_pos ];
        · convert congr_arg ( fun x : ℂ => x * t ^ Kf ) ( hagree ( 3 * ( 2 / 3 * ( t ^ e ) ⁻¹ ) / 2 ) ?_ |> Eq.symm ) using 1;
          · norm_num [ Complex.ofReal_cpow, ht.le ];
          · have h_exp : t ^ e < (3 * Tr / 2) ^ (-1 : ℝ) := by
              exact lt_of_lt_of_le ( pow_lt_pow_left₀ ht'.2 ( by positivity ) ( by positivity ) ) ( by rw [ ← Real.rpow_natCast, ← Real.rpow_mul ( by positivity ), div_mul_cancel₀ _ ( by positivity ) ] );
            rw [ Real.rpow_neg_one ] at h_exp ; nlinarith [ inv_mul_cancel₀ ( show ( 3 * Tr / 2 ) ≠ 0 by positivity ), inv_mul_cancel₀ ( show ( t ^ e ) ≠ 0 by positivity ), pow_pos ht e ];
    · norm_num [ ht.ne' ];
    · convert ht'.1 using 1 ; ring_nf ; norm_num [ ht.ne' ];
      rw [ abs_of_pos ht, ← Real.rpow_natCast, ← Real.rpow_mul ( by positivity ), mul_inv_cancel₀ ( by positivity ), Real.rpow_one ];
  · have h_exp : t ^ e < (3 * Tr / 2) ^ (-1 : ℝ) := by
      exact lt_of_lt_of_le ( pow_lt_pow_left₀ ht'.2 ( by positivity ) ( by positivity ) ) ( by rw [ ← Real.rpow_natCast, ← Real.rpow_mul ( by positivity ), div_mul_cancel₀ _ ( by positivity ) ] );
...
lemma laurent_H_expansion_of_G
  ...
  obtain ⟨T, hT⟩ : ∃ T : ℝ, Tr ≤ T ∧ 2 ≤ T ∧ ∀ x : ℝ, T ≤ x → ∀ z : ℂ, z ∈ Metric.sphere (x : ℂ) (x / 2) → ‖z⁻¹ ^ ((e : ℂ)⁻¹)‖ < min δ ρ := by

    obtain ⟨T, hT⟩ : ∃ T : ℝ, Tr ≤ T ∧ 2 ≤ T ∧ ∀ x : ℝ, T ≤ x → ∀ z : ℂ, z ∈ Metric.sphere (x : ℂ) (x / 2) → ‖z⁻¹‖ < (min δ ρ) ^ e := by
      ...
      · intro x hx z hz
        ...
        norm_num at *;
        rw [ inv_eq_one_div, div_lt_iff₀ ] <;> nlinarith [ show 0 < min δ ρ ^ e by positivity, inv_mul_cancel₀ ( show ( min δ ρ ^ e ) ≠ 0 by positivity ) ];
    refine' ⟨ T, hT.1, hT.2.1, fun x hx z hz => _ ⟩;
    convert Real.rpow_lt_rpow ( by positivity ) ( hT.2.2 x hx z hz ) ( inv_pos.mpr ( by positivity : 0 < ( e : ℝ ) ) ) using 1;
  ...
  have hG_val : G (z⁻¹ ^ ((e : ℂ)⁻¹)) = ∑ m ∈ Finset.range n, (iteratedDeriv m G 0 / (m.factorial : ℂ)) * (z⁻¹ ^ ((e : ℂ)⁻¹)) ^ m + (z⁻¹ ^ ((e : ℂ)⁻¹)) ^ n * R (z⁻¹ ^ ((e : ℂ)⁻¹)) := by
    exact htay _ hw_δ
  have hR_bound : ‖R (z⁻¹ ^ ((e : ℂ)⁻¹))‖ ≤ M' := by
    exact hRb _ hw_δ
  exact ⟨by
  convert laurent_sphere_identity e Kf n he ( fun m => iteratedDeriv m G 0 / ( m.factorial : ℂ ) ) ( R ( z⁻¹ ^ ( e : ℂ ) ⁻¹ ) ) ( G ( z⁻¹ ^ ( e : ℂ ) ⁻¹ ) ) z hz_pos ( by rw [ hG_val ] ) using 1, hR_bound⟩
...
lemma eq_poly_of_tail_of_analytic (T₀ : ℤ) (g : ℝ → ℝ) (q : Polynomial ℝ) (T : ℝ)
  ...
  have h_eq_on_Ioi : ∀ x : ℝ, x > T₀ → g x = q.eval x := by
    have h_eq_on_Ioi : AnalyticOnNhd ℝ (fun x => q.eval x) (Set.Ioi (T₀ : ℝ)) := by
      ...
      simpa only [ Polynomial.eval_eq_sum_range ] using ContDiff.sum fun i hi => ContDiff.mul ( contDiff_const ) ( contDiff_id.pow i );
  ...
  intro x hx; cases lt_or_eq_of_le hx <;> simp_all ;
  ...
  exact tendsto_nhds_unique h_cont_right ( Filter.Tendsto.congr' ( Filter.eventuallyEq_of_mem self_mem_nhdsWithin fun y hy => by rw [ h_eq_on_Ioi y hy ] ) ( q.continuous.continuousWithinAt ) )
...
lemma laurent_nonnat_exp
    ...
    (hTrb : 2 ≤ Tr) (hTrT1 : T₁ ≤ Tr) (hT1T0 : (T₀ : ℝ) < T₁)
    (hagree : ∀ y : ℝ, T₁ / 2 ≤ y → H (y : ℂ) = (g y : ℂ))
    (hnp : ¬ ∃ q : Polynomial ℝ, ∀ x : ℝ, (T₀ : ℝ) ≤ x → g x = q.eval x)
    (hrel : ∀ x : ℝ, Tr ≤ x → ∀ z ∈ Metric.sphere (x : ℂ) (x / 2),
        H z = F (z⁻¹ ^ ((e : ℂ)⁻¹))) :
    ∃ m : ℕ, iteratedDeriv m G 0 ≠ 0 ∧ ∀ i : ℕ, (((Kf : ℝ) - m) / e) ≠ (i : ℝ) := by
  contrapose! hnp; simp_all [ Polynomial.eval_eq_sum_range ] ;
  obtain ⟨T, hT⟩ : ∃ T : ℝ, T₀ < T ∧ ∀ y : ℝ, T ≤ y → g y = ∑ m ∈ Finset.range (Kf + 1), (iteratedDeriv m G 0 / (m.factorial : ℂ)).re * y ^ ((Kf - m) / e : ℕ) := by
    -- Choose a threshold $T$ such that for all $y \geq T$, $y^{-1/e} < \rho$.
    obtain ⟨T, hT⟩ : ∃ T : ℝ, T₀ < T ∧ ∀ y : ℝ, T ≤ y → (y : ℂ)⁻¹ ^ ((e : ℂ)⁻¹) ∈ Metric.ball 0 ρ ∧ 0 < ‖(y : ℂ)⁻¹ ^ ((e : ℂ)⁻¹)‖ := by
      ...
      obtain ⟨T, hT⟩ : ∃ T : ℝ, T₀ < T ∧ ∀ y : ℝ, T ≤ y → (y : ℝ)⁻¹ ^ (1 / e : ℝ) < ρ := by
        have h_lim : Filter.Tendsto (fun y : ℝ => y⁻¹ ^ (1 / e : ℝ)) Filter.atTop (nhds 0) := by
          exact le_trans ( Filter.Tendsto.rpow ( tendsto_inv_atTop_zero ) tendsto_const_nhds <| Or.inr <| by positivity ) <| by norm_num [ show e ≠ 0 by positivity ] ;
        exact Filter.eventually_atTop.mp ( h_lim.eventually ( gt_mem_nhds hρ ) ) |> fun ⟨ T, hT ⟩ ↦ ⟨ Max.max ( T₀ + 1 ) T, by norm_num, fun y hy ↦ hT y <| le_trans ( le_max_right _ _ ) hy ⟩;
    ...
    · exact Or.inl hT.1;
    · intro y hy₁ hy₂ hy₃
      have hH : H (y : ℂ) = ∑ m ∈ Finset.range (Kf + 1), (iteratedDeriv m G 0 / (m.factorial : ℂ)) * y ^ (((Kf : ℝ) - m) / e : ℂ) := by
        convert hrel ( 2 * y / 3 ) ( by linarith ) ( y : ℂ ) _ using 1;
        · rw [ hFG ];
          · rw [ laurent_G_poly_of_vanishing ρ hρ G Kf hG ];
            · rw [ Finset.sum_div _ _ _ ] ; refine' Finset.sum_congr rfl fun m hm => _ ; rw [ ← Complex.cpow_nat_mul ] ; ring;
              rw [ Complex.cpow_sub ] <;> norm_num ; ring;
              · norm_num [ Complex.cpow_def_of_ne_zero, show y ≠ 0 by linarith ] ; ring;
                rw [ Complex.log_inv ] ; norm_num [ ← Complex.exp_nat_mul, ← Complex.exp_neg, ← Complex.exp_add ] ; ring;
                rw [ Complex.arg_ofReal_of_nonneg ( by linarith ) ] ; linarith [ Real.pi_pos ];
              · linarith;
            · intro m hm; specialize hnp m; contrapose! hnp;
              exact ⟨ hnp, fun i hi => by rw [ div_eq_iff ( by positivity ) ] at hi; nlinarith [ show ( Kf : ℝ ) + 1 ≤ m by norm_cast, show ( e : ℝ ) ≥ 1 by norm_cast ] ⟩;
            · convert hT.2 y hy₁ |>.1 using 1 ; norm_num [ Complex.norm_cpow_of_ne_zero, show y ≠ 0 by linarith ];
          · norm_num [ Complex.cpow_def ];
            split_ifs <;> norm_num [ Complex.exp_ne_zero ] ; linarith;
          · convert hT.2 y hy₁ |>.1 using 1 ; norm_num [ Complex.norm_cpow_of_ne_zero, show y ≠ 0 by linarith ];
        · norm_num [ Complex.normSq, Complex.norm_def ] ; ring ; norm_num [ show y ≥ 0 by linarith ];
      convert congr_arg Complex.re hH using 1;
      · rw [ hagree y ( by linarith ) ] ; norm_num;
      · rw [ Complex.re_sum ];
        refine' Finset.sum_congr rfl fun m hm => _ ; by_cases hm' : iteratedDeriv m G 0 = 0 <;> simp_all [ Complex.div_re, Complex.div_im, Complex.cpow_def ] ; ring;
        obtain ⟨ i, hi ⟩ := hnp m hm' ; rw [ div_eq_iff ( by positivity ) ] at hi ; norm_cast at * ; simp_all [ Nat.succ_div ] ; ring;
        rw [ show Kf = m + i * e by linarith [ Nat.sub_add_cancel hm ] ] ; norm_num [ Complex.exp_re, Complex.exp_im, Complex.log_re, Complex.log_im, show e ≠ 0 by linarith ] ; ring;
        split_ifs <;> simp_all [ Complex.exp_re, Complex.exp_im, Complex.log_re, Complex.log_im, mul_assoc, mul_comm, mul_left_comm, ne_of_gt ( zero_lt_one.trans_le he ) ] ; ring;
        norm_num [ Complex.arg_ofReal_of_nonneg ( by linarith : 0 ≤ y ), Real.exp_nat_mul, Real.exp_log ( by linarith : 0 < y ) ] ; ring;
  obtain ⟨q, hq⟩ : ∃ q : Polynomial ℝ, ∀ y : ℝ, T ≤ y → g y = q.eval y := by
    use ∑ m ∈ Finset.range (Kf + 1), Polynomial.C ((iteratedDeriv m G 0 / (m.factorial : ℂ)).re) * Polynomial.X ^ ((Kf - m) / e);
    simp_all [ Polynomial.eval_finset_sum ];
  exact ⟨ q, fun x hx => by rw [ eq_poly_of_tail_of_analytic T₀ g q T hg hgana hT.1 hq x hx, Polynomial.eval_eq_sum_range ] ⟩
...
-/
lemma laurent_principal_eq (G : ℂ → ℂ) (e Kf n : ℕ) (he : 1 ≤ e) (z : ℂ) (hz : z ≠ 0)
  ...
  · simp [ Polynomial.eval_finset_sum ];
    ...
    rw [ ← Finset.sum_add_distrib ] ; refine' Finset.sum_congr rfl fun x hx => _ ; split_ifs <;> simp_all [ Complex.ext_iff ] ;
    · rcases ‹_› with ⟨ i, hi ⟩ ; rw [ div_eq_iff ( by positivity ) ] at hi ; norm_cast at * ; simp_all [ Nat.div_eq_of_lt ] ;
      rw [ Int.subNatNat_eq_coe ] at hi ; norm_num [ show Kf - x = i * e by exact Nat.sub_eq_of_eq_add <| by linarith ] ; norm_cast ; simp [ Nat.mul_div_cancel _ he ] ;
      norm_num [ Rat.mkRat_eq_div, mul_div_cancel_right₀, show e ≠ 0 by linarith ];
    · rw [ Finset.sum_eq_single x ] <;> simp_all [ Complex.ext_iff ];
      intro b hb hb' hb'' hb''' hb''''; simp_all [ div_eq_mul_inv ] ;
  · intro m hm m' hm' h; simp_all [ div_eq_iff, ne_of_gt ( zero_lt_one.trans_le he ) ] ;
...
lemma convergent_laurent_tail_bound
  ...
  obtain ⟨m', hm'ne, hm'nn⟩ : ∃ m' : ℕ, iteratedDeriv m' G 0 ≠ 0 ∧ ∀ i : ℕ, ((Kf : ℝ) - (m' : ℝ)) / (e : ℝ) ≠ (i : ℝ) ∧ ∀ m < m', ¬(iteratedDeriv m G 0 ≠ 0 ∧ ∀ i : ℕ, ((Kf : ℝ) - (m : ℝ)) / (e : ℝ) ≠ (i : ℝ)) := by
    have h_exists_m' : ∃ m' : ℕ, iteratedDeriv m' G 0 ≠ 0 ∧ ∀ i : ℕ, ((Kf : ℝ) - (m' : ℝ)) / (e : ℝ) ≠ (i : ℝ) := by
      apply DorgeBauer.laurent_nonnat_exp T₀ g ρ Kf F G hρ hg hgana hGan hFG hFan e he H T₁ Tr hTrb hTrT1 (by
      rcases T₀ with ⟨ _ | _ | T₀ ⟩ <;> norm_num at * <;> linarith) hagree hnp hrel;
    obtain ⟨m', hm'⟩ : ∃ m' : ℕ, iteratedDeriv m' G 0 ≠ 0 ∧ ∀ i : ℕ, ((Kf : ℝ) - (m' : ℝ)) / (e : ℝ) ≠ (i : ℝ) ∧ ∀ m < m', ¬(iteratedDeriv m G 0 ≠ 0 ∧ ∀ i : ℕ, ((Kf : ℝ) - (m : ℝ)) / (e : ℝ) ≠ (i : ℝ)) := by
      have h_well_founded : WellFounded (fun m n : ℕ => m < n) := by
        exact wellFounded_lt
      have := h_well_founded.has_min { m | iteratedDeriv m G 0 ≠ 0 ∧ ∀ i : ℕ, ( Kf - m : ℝ ) / e ≠ i } ⟨ h_exists_m'.choose, h_exists_m'.choose_spec ⟩;
      exact ⟨ this.choose, this.choose_spec.1.1, fun i => ⟨ this.choose_spec.1.2 i, fun m hm => fun h => this.choose_spec.2 m h hm ⟩ ⟩;
    use m';
  obtain ⟨R, T, M, hTrT, hT2, hM, hexp⟩ : ∃ R : ℂ → ℂ, ∃ T : ℝ, ∃ M : ℝ, Tr ≤ T ∧ 2 ≤ T ∧ 0 ≤ M ∧ ∀ x : ℝ, T ≤ x → ∀ z ∈ Metric.sphere (x : ℂ) (x / 2), H z = (∑ m ∈ Finset.range (Kf + m' + 1), (iteratedDeriv m G 0 / (Nat.factorial m : ℂ)) * z ^ ((((Kf : ℝ) - m) / e : ℝ) : ℂ)) + z ^ ((((Kf : ℝ) - (Kf + m' + 1)) / e : ℝ) : ℂ) * R (z⁻¹ ^ ((e : ℂ)⁻¹)) ∧ ‖R (z⁻¹ ^ ((e : ℂ)⁻¹))‖ ≤ M := by
    convert laurent_H_expansion_of_G ρ Kf F G hρ hGan hFG e he H Tr hTrb hrel ( Kf + m' + 1 ) using 1;
    norm_cast;
  refine' ⟨ laurentPoly G e Kf ( Kf + m' + 1 ), laurentI G e Kf ( Kf + m' + 1 ), laurentA G e Kf ( Kf + m' + 1 ), ( Kf - m' : ℝ ) / e, ( Kf - ( Kf + m' + 1 ) : ℝ ) / e, 2 ^ |( Kf - ( Kf + m' + 1 ) : ℝ ) / e| * M, T, _, _, _, _, _ ⟩;
  · refine' Finset.mem_image.mpr ⟨ m', _, _ ⟩ <;> norm_num [ hm'ne, hm'nn ];
  · intro σ hσ;
    unfold laurentI at hσ; obtain ⟨ m, hm₁, rfl ⟩ := Finset.mem_image.mp hσ; simp_all [ Finset.mem_filter, Finset.mem_range ] ;
    obtain ⟨ a, ⟨ ha₁, ha₂, ha₃ ⟩, rfl ⟩ := hm₁; gcongr;
    exact le_of_not_gt fun h => by obtain ⟨ x, hx ⟩ := hm'nn 0 |>.2 a h ha₂; exact ha₃ x hx;
  · exact fun i => hm'nn i |>.1;
  · unfold laurentA;
    rw [ Finset.sum_eq_single m' ] <;> simp +contextual [ hm'ne, hm'nn ];
    · have hreal : ∀ m : ℕ, (iteratedDeriv m G 0).im = 0 := by
        apply laurent_coeff_real ρ Kf F G hρ hGan hFG e he H g T₁ Tr hTrb hTrT1 hagree (fun x hx z hz => hrel x hx z hz);
      exact ⟨ fun h => hm'ne <| Complex.ext h <| hreal m', Nat.factorial_ne_zero _ ⟩;
    · intro b hb₁ hb₂ hb₃ hb₄ hb₅; specialize hm'nn b; simp_all [ div_eq_iff, ne_of_gt ( zero_lt_one.trans_le he ) ] ;
  · refine' ⟨ _, _, _, _, _, _ ⟩;
    any_goals linarith [ show ( e : ℝ ) ≥ 1 by norm_cast, show ( m' : ℝ ) ≥ 0 by positivity, div_lt_div_iff_of_pos_right ( by positivity : 0 < ( e : ℝ ) ) |>.2 ( by linarith : ( Kf : ℝ ) - ( Kf + m' + 1 ) < Kf - m' ) ];
    · positivity;
    · intro x hx z hz;
      ...
      convert mul_le_mul_of_nonneg_right ( laurent_tail_term_bound z x ( ( Kf - ( Kf + m' + 1 ) : ℝ ) / e ) ( ( Kf - ( Kf + m' + 1 ) : ℝ ) / e ) ( by linarith ) hz le_rfl ) hM using 1 ; ring;
      · norm_num [ mul_comm ];
      · ring
...
lemma real_branch_analytic
  ...
  contrapose! hroot; contrapose! hg; (
  intro h_cont_diff
  have h_analytic : AnalyticOnNhd ℝ g (Set.Ioi (T₀ : ℝ)) := by
    intro x hx; have := h_cont_diff x ( Set.mem_Ici.mpr hx.out.le ) ; exact (by
    have := this.contDiffAt ( Ici_mem_nhds hx ) ; exact this.analyticAt;);
  contradiction)
