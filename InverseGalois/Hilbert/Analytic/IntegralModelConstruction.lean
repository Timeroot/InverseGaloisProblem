...
lemma scaleRoots_integral_coeffs
    (f : Polynomial (Polynomial ℚ)) (hf_monic : f.Monic) (D : ℕ) (hD : 0 < D)
  ...
  · rw [ Polynomial.coeff_scaleRoots ];
    rw [ Polynomial.coeff_eq_zero_of_natDegree_lt hi ] ; exact ⟨ 0, by norm_num ⟩;
  · by_cases hi' : i < f.natDegree;
    · obtain ⟨ b, hb ⟩ := hD_clears i;
      use Polynomial.C (D ^ (f.natDegree - i - 1) : ℤ) * b;
      simp_all [ Polynomial.coeff_scaleRoots, Polynomial.map_mul, Polynomial.map_pow, Polynomial.map_C ];
      rw [ ← hb ] ; ring;
      rw [ ← Nat.sub_add_cancel ( Nat.sub_pos_of_lt hi' ), pow_succ, mul_comm ] ; norm_num [ mul_assoc, mul_comm, mul_left_comm ];
...
lemma lift_integral_poly (g : Polynomial (Polynomial ℚ))
  ...
  simp [ Polynomial.coeff_sum, Polynomial.coeff_monomial ];
  split_ifs with hi <;> [ simp [ hi ] ; rw [ ← Classical.choose_spec ( hg i ) ] ]
...
lemma scaleRoots_unit_irreducible
  ...
  have h_scale_surjective : ∀ p : R[X], p ∣ f.scaleRoots c → ∃ q : R[X], q ∣ f ∧ p = q.scaleRoots c := by
    ...
    obtain ⟨q, hq⟩ : ∃ q : R[X], p = q.scaleRoots c := by
      ...
      simp [ mul_assoc, ← mul_pow, hc.unit.inv_mul ];
    ...
    have h_div : r * s = f := by
      ...
      have h_div : ∀ p q : R[X], p.scaleRoots c = q.scaleRoots c → p = q := by
        ...
        have h_coeff : ∀ i, p.coeff i = q.coeff i := by
          ...
          rw [ Polynomial.natDegree_scaleRoots, Polynomial.natDegree_scaleRoots ] at hpq ; aesop;
  ...
  · intro h_unit
    ...
    have h_deg : f.natDegree = 0 := by
      have := congr_arg Polynomial.natDegree hq; rw [ Polynomial.natDegree_mul' ] at this <;> simp_all [ Polynomial.natDegree_scaleRoots ] ;
      exact ⟨ hf.ne_zero, by rintro rfl; simp at hq ⟩
    ...
    have h_unit_r : IsUnit r := by
      replace hq := congr_arg ( Polynomial.eval 0 ) hq ; simp_all [ Polynomial.eval ]
...
lemma gauss_lemma_bivariate (F : Polynomial (Polynomial ℤ)) (hF_monic : F.Monic)
  ...
  · obtain ⟨a, b, ha, hb, h⟩ : ∃ a b : Polynomial ℤ[X], F = a * b ∧ ¬IsUnit a ∧ ¬IsUnit b := by
      ...
      obtain ⟨ r, hr, rfl ⟩ := h; simp_all [ Polynomial.Monic.def ] ;
    simp_all [ irreducible_mul_iff ];
    constructor <;> intro H <;> simp_all [ Polynomial.Monic.def, Polynomial.leadingCoeff_mul ];
    · intro H';
      have := Polynomial.degree_eq_zero_of_isUnit H'; rw [ Polynomial.degree_map_eq_of_leadingCoeff_ne_zero ] at this <;> simp_all [ Polynomial.degree_eq_natDegree ] ;
      · rw [ Polynomial.eq_C_of_natDegree_eq_zero this ] at h hF_monic; simp_all [ Polynomial.leadingCoeff_C ] ;
        exact h ( isUnit_of_dvd_one <| hF_monic ▸ dvd_mul_left _ _ );
      · intro H; simp_all [ Polynomial.ext_iff ] ;
        specialize H ( Polynomial.natDegree ( Polynomial.leadingCoeff b ) ) ; simp_all [ Polynomial.coeff_natDegree ] ;
    · intro H';
      have := Polynomial.natDegree_eq_zero_of_isUnit H';
      rw [ Polynomial.natDegree_map_of_leadingCoeff_ne_zero ] at this <;> simp_all [ Polynomial.natDegree_eq_zero_iff_degree_le_zero ];
      · rw [ Polynomial.eq_C_of_degree_le_zero this ] at hb hF_monic ha; simp_all [ Polynomial.Monic.def ] ;
        exact hb ( isUnit_of_dvd_one <| hF_monic ▸ dvd_mul_right _ _ );
      · intro H''; replace hF_monic := congr_arg ( Polynomial.map ( Int.castRingHom ℚ ) ) hF_monic; simp_all [ Polynomial.map_mul ] ;
