...
/-!
# The intermediate ring `ℚ̄[T][x]` and the ramified branch point of the Morse family

The reduction of `morseGeomPoly_hasSwap` set up in `InverseGalois.Hilbert.Analytic.MorseSwap`
needs one local computation: at a critical value of `Xⁿ − X` exactly two sheets of the cover
collide, so the inertia there is generated by a transposition.  Carrying it out means descending
from the full integral closure `B` to the concrete monogenic ring `A[x] = ℚ̄[T][x]`, where
Kummer–Dedekind reads the factorisation at a branch point off the reduction of the defining
polynomial.

That computation, the branch point it produces and the assembly of the transposition are
collected here, leaving the file above to install the Galois infrastructure on `B`.

## Main results

* `MorseSwap.branch_ramified` — the chosen branch point is ramified with index at least two.
* `MorseSwap.exists_nontrivial_inertia` — a place above it with nontrivial inertia.
* `morseGeomPoly_hasSwap` — the transposition in the geometric monodromy group.
-/
...
lemma Smid_eval_surjective :
  ...
  · exact fun x hx ↦ ⟨Polynomial.X, by simp_all⟩
  · intro r
