...
theorem analytic_taylor_remainder_bound (G : ℂ → ℂ) (hG : AnalyticAt ℂ G 0) (n : ℕ) :
  ...
  obtain ⟨ δ₁, hδ₁_pos, hδ₁ ⟩ : ∃ δ₁ > 0, ∀ w, ‖w‖ < δ₁ → ‖F w‖ ≤ ‖F 0‖ + 1 := by
    ...
    exact ⟨ this.choose, this.choose_spec.1, fun w hw => by have := this.choose_spec.2 ( show dist w 0 < this.choose from by simpa using hw ) ; exact le_trans ( norm_le_of_mem_closedBall <| by simpa [ dist_eq_norm ] using this.le ) ( by linarith ) ⟩;
  obtain ⟨ δ₂, hδ₂_pos, hδ₂ ⟩ := Metric.eventually_nhds_iff.mp hF₂;
  refine' ⟨ F, Min.min δ₁ δ₂, ‖F 0‖ + 1, lt_min hδ₁_pos hδ₂_pos, by positivity, _, _ ⟩ <;> simp_all [ div_eq_inv_mul, mul_assoc, mul_comm, mul_left_comm ]
  ...
  have h_eq : ∀ m, iteratedDeriv m G 0 = starRingEnd ℂ (iteratedDeriv m G 0) := by
    have h_eq : ∀ w ∈ Metric.ball 0 r, starRingEnd ℂ (G (starRingEnd ℂ w)) = G w := by
      ...
      have h_eq : AnalyticOnNhd ℂ (fun w => (starRingEnd ℂ) (G (starRingEnd ℂ w))) (Metric.ball 0 r) := by
        ...
        · intro w hw; have := hG ( starRingEnd ℂ w ) ; simp_all [ DifferentiableOn ] ;
          have h_diff : HasDerivAt (fun w => starRingEnd ℂ (G (starRingEnd ℂ w))) (starRingEnd ℂ (deriv G (starRingEnd ℂ w))) w := by
      ...
      · rw [ Metric.nhdsWithin_basis_ball.frequently_iff ];
        intro ε ε_pos; use ( r / 2 : ℂ ) + Min.min ε ( r / 2 ) / 2; norm_num [ Complex.ext_iff, hr.ne' ];
        erw [ Complex.conj_ofReal ] ; norm_num;
        exact ⟨ ⟨ by rw [ abs_of_nonneg ( by positivity ) ] ; linarith [ min_le_left ε ( r / 2 ), min_le_right ε ( r / 2 ) ], by positivity ⟩, by have := hreal ( r / 2 + Min.min ε ( r / 2 ) / 2 ) ( by positivity ) ( by linarith [ min_le_left ε ( r / 2 ), min_le_right ε ( r / 2 ) ] ) ; norm_num [ Complex.ext_iff ] at *; linarith ⟩;
    -- Apply the fact that the derivative of a composition is the composition of the derivatives.
    have h_deriv_comp : ∀ m, iteratedDeriv m (fun w => starRingEnd ℂ (G (starRingEnd ℂ w))) 0 = starRingEnd ℂ (iteratedDeriv m G 0) := by
      intro m
      have h_deriv_comp : ∀ w ∈ Metric.ball 0 r, iteratedDeriv m (fun w => starRingEnd ℂ (G (starRingEnd ℂ w))) w = starRingEnd ℂ (iteratedDeriv m G (starRingEnd ℂ w)) := by
        induction' m with m ih;
        · intro w hw
          simp_all only [Metric.mem_ball, dist_zero_right, iteratedDeriv_zero]
        · intro w hw;
          rw [ iteratedDeriv_succ, iteratedDeriv_succ ];
          convert HasDerivAt.deriv ( HasDerivAt.congr_of_eventuallyEq _ <| Filter.eventuallyEq_of_mem ( IsOpen.mem_nhds ( Metric.isOpen_ball ) hw ) fun x hx => ih x hx ) using 1;
          have h_deriv : HasDerivAt (iteratedDeriv m G) (deriv (iteratedDeriv m G) (starRingEnd ℂ w)) (starRingEnd ℂ w) := by
            ...
            exact h_deriv.differentiableOn.differentiableAt ( Metric.isOpen_ball.mem_nhds <| by simpa [ Complex.norm_conj ] using hw ) |> DifferentiableAt.hasDerivAt;
          rw [ hasDerivAt_iff_tendsto_slope_zero ] at *;
          convert Complex.continuous_conj.continuousAt.tendsto.comp ( h_deriv.comp ( show Filter.Tendsto ( fun t : ℂ => starRingEnd ℂ t ) ( 𝓝[≠] 0 ) ( 𝓝[≠] 0 ) from ?_ ) ) using 2 <;> norm_num;
          rw [Metric.tendsto_nhdsWithin_nhdsWithin]
          intro ε a
    ...
    intro m; rw [ ← h_deriv_comp m ] ; rw [ Filter.EventuallyEq.iteratedDeriv_eq ] ; filter_upwards [ Metric.ball_mem_nhds 0 hr ] with w hw; simp_all only [Metric.mem_ball, dist_zero_right];
  specialize h_eq m; rw [ Complex.ext_iff ] at h_eq; norm_num at h_eq; linarith;
