...
lemma squarefree_map_frac_int {s : Polynomial (Polynomial ℤ)}
  ...
  obtain ⟨x', hx'⟩ : ∃ x' : Polynomial (Polynomial ℤ), x'.map (algebraMap (Polynomial ℤ) (FractionRing (Polynomial ℤ))) = xm ∧ x'.Monic := by
    obtain ⟨x', hx'⟩ : ∃ x' : Polynomial (Polynomial ℤ), x'.map (algebraMap (Polynomial ℤ) (FractionRing (Polynomial ℤ))) = xm := by
...
lemma exists_bezout_of_squarefree_int {s : Polynomial (Polynomial ℤ)}
  ...
  obtain ⟨U, V, h_bezout⟩ : ∃ U V : Polynomial (FractionRing (Polynomial ℤ)), U * (s.map (algebraMap (Polynomial ℤ) (FractionRing (Polynomial ℤ)))) + V * (Polynomial.derivative (s.map (algebraMap (Polynomial ℤ) (FractionRing (Polynomial ℤ))))) = 1 := by
    ...
    exact ⟨U, V, hUV⟩;
  obtain ⟨w0, hw0⟩ : ∃ w0 : Polynomial ℤ, w0 ≠ 0 ∧ ∃ A B : Polynomial (Polynomial ℤ), A * s + B * (derivative s) = Polynomial.C w0 := by
    obtain ⟨w0, hw0⟩ : ∃ w0 : Polynomial ℤ, w0 ≠ 0 ∧ ∃ A B : Polynomial (Polynomial ℤ), Polynomial.map (algebraMap (Polynomial ℤ) (FractionRing (Polynomial ℤ))) A = U * Polynomial.C (algebraMap (Polynomial ℤ) (FractionRing (Polynomial ℤ)) w0) ∧ Polynomial.map (algebraMap (Polynomial ℤ) (FractionRing (Polynomial ℤ))) B = V * Polynomial.C (algebraMap (Polynomial ℤ) (FractionRing (Polynomial ℤ)) w0) := by
      have h_clear_denom : ∀ p : Polynomial (FractionRing (Polynomial ℤ)), ∃ w0 : Polynomial ℤ, w0 ≠ 0 ∧ ∃ A : Polynomial (Polynomial ℤ), Polynomial.map (algebraMap (Polynomial ℤ) (FractionRing (Polynomial ℤ))) A = p * Polynomial.C (algebraMap (Polynomial ℤ) (FractionRing (Polynomial ℤ)) w0) := by
        intro p
        induction' p using Polynomial.induction_on' with p hp ih;
        · obtain ⟨ w0, hw0, A, hA ⟩ := ih; obtain ⟨ w1, hw1, B, hB ⟩ := ‹∃ w0 : Polynomial ℤ, w0 ≠ 0 ∧ ∃ A : Polynomial ( Polynomial ℤ ), map ( algebraMap ( Polynomial ℤ ) ( FractionRing ( Polynomial ℤ ) ) ) A = hp * C ( algebraMap ( Polynomial ℤ ) ( FractionRing ( Polynomial ℤ ) ) w0 ) ›; use w0 * w1; simp_all [ mul_assoc, mul_comm, mul_left_comm ] ;
    ...
    have h_eq : Polynomial.map (algebraMap (Polynomial ℤ) (FractionRing (Polynomial ℤ))) (A * s + B * (derivative s)) = Polynomial.C (algebraMap (Polynomial ℤ) (FractionRing (Polynomial ℤ)) w0) := by
    ...
    exact ⟨ w0, hw0.1, A, B, Polynomial.map_injective ( algebraMap ( Polynomial ℤ ) ( FractionRing ( Polynomial ℤ ) ) ) ( IsFractionRing.injective _ _ ) <| by aesop ⟩;
...
theorem exists_complex_separable_reduction
  ...
  refine' ⟨ radical P, 1 + ∑ r ∈ ( w0.map ( Int.castRingHom ℂ ) |> Polynomial.roots |> Multiset.toFinset ), ‖r‖, _, _, _, _, _ ⟩;
  ...
  · intro z hz
    ...
    have h_bezout : (Polynomial.map (evalIntPolyComplex z) A) * (Polynomial.map (evalIntPolyComplex z) (radical P)) + (Polynomial.map (evalIntPolyComplex z) B) * (Polynomial.derivative (Polynomial.map (evalIntPolyComplex z) (radical P))) = Polynomial.C (Polynomial.eval z (Polynomial.map (Int.castRingHom ℂ) w0)) := by
      convert congr_arg ( Polynomial.map ( evalIntPolyComplex z ) ) hbez using 1 <;> norm_num [ Polynomial.derivative_map ];
      unfold evalIntPolyComplex; aesop;
    refine' ⟨ Polynomial.C ( Polynomial.eval z ( Polynomial.map ( Int.castRingHom ℂ ) w0 ) ) ⁻¹ * Polynomial.map ( evalIntPolyComplex z ) A, Polynomial.C ( Polynomial.eval z ( Polynomial.map ( Int.castRingHom ℂ ) w0 ) ) ⁻¹ * Polynomial.map ( evalIntPolyComplex z ) B, _ ⟩;
    convert congr_arg ( fun p => Polynomial.C ( Polynomial.eval z ( Polynomial.map ( Int.castRingHom ℂ ) w0 ) ) ⁻¹ * p ) h_bezout using 1 <;> ring;
