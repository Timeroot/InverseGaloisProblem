...
lemma exists_int_multiple_of_rat_poly (a : Polynomial ℚ) :
  ...
  obtain ⟨D, hD⟩ : ∃ D : ℕ, 0 < D ∧ ∀ i ∈ a.support, D * a.coeff i ∈ Set.range (algebraMap ℤ ℚ) := by
    obtain ⟨D, hD⟩ : ∃ D : ℕ, 0 < D ∧ ∀ i ∈ a.support, (a.coeff i).den ∣ D := by
      exact ⟨ ∏ i ∈ a.support, ( a.coeff i |> Rat.den ), Finset.prod_pos fun i hi => Nat.cast_pos.mpr ( Rat.pos _ ), fun i hi => Finset.dvd_prod_of_mem _ hi ⟩;
    refine' ⟨ D, hD.1, fun i hi => _ ⟩;
    obtain ⟨ k, hk ⟩ := hD.2 i hi; use k * ( a.coeff i |> Rat.num ) ; simp [ *, Rat.cast_def, mul_comm, mul_left_comm ] ;
...
lemma exists_common_denominator (f : Polynomial (Polynomial ℚ)) :
  ...
  have hD_exists : ∀ i, ∃ D_i : ℕ, 0 < D_i ∧ ∃ b : Polynomial ℤ, (D_i : ℚ) • (f.coeff i) = b.map (Int.castRingHom ℚ) := by
    exact fun i => by obtain ⟨ D, b, hD, hb ⟩ := exists_int_multiple_of_rat_poly ( f.coeff i ) ; exact ⟨ D, hD, b, hb ⟩ ;
  choose D hD_pos b hb using hD_exists;
  refine' ⟨ ∏ i ∈ f.support, D i, Finset.prod_pos _, fun i => _ ⟩;
  · intro i a
    simp_all only [mem_support_iff, ne_eq];
  · by_cases hi : i ∈ f.support <;> simp_all [ Finset.prod_eq_zero_iff, ne_of_gt ];
    · use b i * ∏ j ∈ f.support \ {i}, (D j : ℤ[X]);
      simp_all [ Finset.prod_eq_prod_diff_singleton_mul ( Polynomial.mem_support_iff.mpr hi ), mul_assoc, mul_comm, mul_left_comm, Algebra.smul_def ];
...
theorem monic_int_factor_of_monic_int_dvd' {f : Polynomial ℤ} {g : Polynomial ℚ}
  ...
  have h_int_coeffs : ∀ i, ∃ (ci : ℤ), Polynomial.coeff g i = (ci : ℚ) := by
    intro i; have := isIntegral_coeff_of_dvd f g hf_monic hg_monic hg_dvd i; simp_all [ IsIntegrallyClosed.isIntegral_iff ] ;
    tauto;
  ...
  · rw [ Polynomial.Monic, Polynomial.leadingCoeff, Polynomial.natDegree_eq_of_degree_eq_some ];
    ...
    · have := hg_monic.coeff_natDegree; simp_all only [Rat.intCast_eq_one_iff, finset_sum_coeff, coeff_intCast_mul, Int.cast_eq, coeff_X_pow, mul_ite, mul_one,
      mul_zero, Finset.sum_ite_eq, mem_support_iff, Int.cast_one, ne_eq, one_ne_zero, not_false_eq_true, ↓reduceIte];
    · rw [ Polynomial.degree_eq_of_le_of_coeff_ne_zero ] <;> norm_num [ Polynomial.coeff_sum, Polynomial.coeff_X_pow, hci ];
      · exact le_trans ( Polynomial.degree_sum_le _ _ ) ( Finset.sup_le fun i hi => Polynomial.degree_C_mul_X_pow_le _ _ |> le_trans <| WithBot.coe_le_coe.mpr <| Polynomial.le_natDegree_of_mem_supp _ hi );
      · have := hg_monic.coeff_natDegree; simp_all only [Rat.intCast_eq_one_iff, one_ne_zero, not_false_eq_true];
  · rw [ Polynomial.natDegree_eq_of_degree_eq_some ] ; rw [ Polynomial.degree_eq_of_le_of_coeff_ne_zero ] <;> norm_num [ Polynomial.coeff_sum, hci ];
    · exact le_trans ( Polynomial.degree_sum_le _ _ ) ( Finset.sup_le fun i hi => Polynomial.degree_C_mul_X_pow_le _ _ |> le_trans <| WithBot.coe_le_coe.mpr <| Polynomial.le_natDegree_of_mem_supp _ hi );
    · have := hg_monic.coeff_natDegree; simp_all only [Rat.intCast_eq_one_iff, one_ne_zero, not_false_eq_true];
...
lemma tschirnhaus_monic (g : Polynomial ℚ) (D : ℚ) (hD : D ≠ 0) (hg_monic : g.Monic) :
  ...
  · simp_all only [ne_eq, Monic.leadingCoeff, leadingCoeff_mul, leadingCoeff_C, monic_X, mul_one, inv_pow, one_mul, map_pow,
      leadingCoeff_pow, pow_eq_zero_iff', Monic.natDegree_eq_zero, false_and, not_false_eq_true, inv_mul_cancel₀];
...
-/
lemma tschirnhaus_factor_dvd (f g : Polynomial ℚ) (D : ℚ) (hD : D ≠ 0)
  ...
  simp_all [ Polynomial.natDegree_mul' ] ;
