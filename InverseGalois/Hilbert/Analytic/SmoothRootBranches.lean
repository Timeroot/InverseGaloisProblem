...
lemma local_branch_of_simple
  ...
  have hF : HasFDerivAt F (fderiv ℝ F (x₀, y₀)) (x₀, y₀) := by
    exact DifferentiableAt.hasFDerivAt ( hsmooth.contDiffAt.differentiableAt ( by norm_num ) )
  have hF_contDiff : ContDiffAt ℝ ⊤ F (x₀, y₀) := by
    exact hsmooth.contDiffAt
  have hL : ∀ x, (fderiv ℝ F (x₀, y₀)).comp (ContinuousLinearMap.inr ℝ ℝ ℝ) x = (Polynomial.derivative (Q x₀)).eval y₀ * x := by
    intro x
    have hL : deriv (fun y => (Q x₀).eval y) y₀ = (fderiv ℝ F (x₀, y₀)).comp (ContinuousLinearMap.inr ℝ ℝ ℝ) 1 := by
      ...
      convert HasFDerivAt.hasDerivAt ( hF.comp y₀ ( HasFDerivAt.prodMk ( hasFDerivAt_const _ _ ) ( hasFDerivAt_id _ ) ) ) using 1
    generalize_proofs at *; (
    convert congr_arg ( fun z => z * x ) hL.symm using 1 ; norm_num [ Polynomial.derivative_eval ] ; ring!;
    · rw [ mul_comm ] ; erw [ ← smul_eq_mul ] ; erw [ ← ContinuousLinearMap.map_smul ] ; norm_num;
    · norm_num [ Polynomial.derivative_eval ])
  have hL_inv : ∃ L_inv : ℝ →L[ℝ] ℝ, ∀ x, L_inv ((Polynomial.derivative (Q x₀)).eval y₀ * x) = x := by
    exact ⟨ ContinuousLinearMap.smulRight ( 1 : ℝ →L[ℝ] ℝ ) ( ( Polynomial.eval y₀ ( Polynomial.derivative ( Q x₀ ) ) ) ⁻¹ ), fun x => by simp [ hsimple, mul_assoc, mul_comm, mul_left_comm ] ⟩
  ...
  have h_implicit : IsContDiffImplicitAt ⊤ F (fderiv ℝ F (x₀, y₀)) (x₀, y₀) := by
    ...
    · exact hF_contDiff;
    · constructor;
      · intro x y hxy; have := hL_inv x; have := hL_inv y; aesop;
      · intro x; use x / ( Polynomial.eval y₀ ( Polynomial.derivative ( Q x₀ ) ) ) ; simp [ hL, mul_div_cancel₀ _ hsimple ] ;
    · decide +revert
  generalize_proofs at *; (
  refine' ⟨ h_implicit.implicitFunction, _, _, _ ⟩ <;> norm_num [ h_implicit ];
  · convert h_implicit.eventually_implicitFunction_apply_eq.self_of_nhds using 1 ; aesop ( simp_config := { singlePass := true } ) ;
  · exact h_implicit.contDiffAt_implicitFunction;
  · have := h_implicit.apply_implicitFunction; aesop;)
...
lemma local_unique_of_simple
  ...
  obtain ⟨δ, hδ_pos, hδ⟩ : ∃ δ > 0, ∀ x y, abs (x - x₀) < δ → abs (y - y₀) < δ → (Q x).derivative.eval y ≠ 0 := by
    have h_cont : ContinuousAt (fun p : ℝ × ℝ => (Q p.1).derivative.eval p.2) (x₀, y₀) := by
      have h_cont : ContDiff ℝ ⊤ (fun p : ℝ × ℝ => (Q p.1).derivative.eval p.2) := by
        ...
        ext; simp [ Polynomial.derivative_eval ] ;
      exact h_cont.continuous.continuousAt;
    have := Metric.continuousAt_iff.mp h_cont;
    exact Exists.elim ( this ( |eval y₀ ( derivative ( Q x₀ ) )| ) ( abs_pos.mpr hsimple ) ) fun δ hδ => ⟨ δ, hδ.1, fun x y hx hy => by cases abs_cases ( eval y₀ ( derivative ( Q x₀ ) ) ) <;> linarith [ abs_lt.mp ( hδ.2 ( show dist ( x, y ) ( x₀, y₀ ) < δ from max_lt hx hy ) ) ] ⟩;
  refine' ⟨ Metric.ball x₀ δ, Metric.ball y₀ δ, Metric.ball_mem_nhds _ hδ_pos, Metric.ball_mem_nhds _ hδ_pos, _ ⟩;
  intro x hx y₁ hy₁ y₂ hy₂ h₁ h₂; contrapose! hδ;
  ...
  exact ⟨ x, hx, c, abs_lt.mpr ⟨ by cases hc.1.1 <;> cases hc.1.2 <;> linarith [ abs_lt.mp hy₁, abs_lt.mp hy₂ ], by cases hc.1.1 <;> cases hc.1.2 <;> linarith [ abs_lt.mp hy₁, abs_lt.mp hy₂ ] ⟩, hc.2 ⟩
...
lemma coeff_continuous
  ...
  have h_coeff_cont : ∀ p : ℕ, ContDiff ℝ ⊤ (fun x => Polynomial.eval 0 (Polynomial.derivative^[p] (Q x))) := by
    ...
    have h_coeff_cont : ContDiff ℝ ⊤ (fun p' : ℝ × ℝ => Polynomial.eval p'.2 (Polynomial.derivative^[p] (Q p'.1))) := by
      ...
      have h_eval_cont : ContDiff ℝ ⊤ (fun p' : ℝ × ℝ => deriv (fun y => Polynomial.eval y (Polynomial.derivative^[p] (Q p'.1))) p'.2) := by
  ...
  ext x; rw [ eq_div_iff ( by positivity ) ] ; simp [ Polynomial.eval, Polynomial.coeff_iterate_derivative ] ;
...
lemma exists_uniform_root_bound (d : ℕ) (hd : 1 ≤ d)
  ...
  obtain ⟨M, u, hu⟩ : ∃ M u, u ∈ nhds x₀ ∧ ∀ x ∈ u, B x ≤ M := by
    ...
    exact ⟨ B x₀ + 1, { x | B x < B x₀ + 1 }, hB_cont.continuousAt.eventually ( gt_mem_nhds <| lt_add_one _ ), fun x hx => le_of_lt hx ⟩;
  ...
  · -- By definition of $B$, we know that $|y|^d \leq \sum_{i=0}^{d-1} |(Q x).coeff i| |y|^i$.
    have h_bound : |y|^d ≤ ∑ i ∈ Finset.range d, |(Q x).coeff i| * |y|^i := by
      have h_bound : |y|^d ≤ |∑ i ∈ Finset.range d, (Q x).coeff i * y^i| := by
        ...
        simp_all [ add_eq_zero_iff_eq_neg, Polynomial.Monic.def, Polynomial.leadingCoeff, Polynomial.natDegree_eq_of_degree_eq_some ];
      exact h_bound.trans ( le_trans ( Finset.abs_sum_le_sum_abs _ _ ) ( Finset.sum_le_sum fun i hi => by rw [ abs_mul, abs_pow ] ) );
    ...
    have h_factor : |y|^d ≤ (∑ i ∈ Finset.range d, |(Q x).coeff i|) * |y|^(d-1) := by
      exact h_bound.trans ( by rw [ Finset.sum_mul _ _ _ ] ; exact Finset.sum_le_sum fun i hi => mul_le_mul_of_nonneg_left ( pow_le_pow_right₀ hy_abs.le ( Nat.le_sub_one_of_lt ( Finset.mem_range.mp hi ) ) ) ( abs_nonneg _ ) );
    rcases d <;> simp_all [ pow_succ' ];
    nlinarith [ hu.2 x hx, pow_pos ( zero_lt_one.trans hy_abs ) ‹_›, show ∑ i ∈ Finset.range ( ‹_› + 1 ), |( Q x |> Polynomial.coeff ) i| ≤ M - 1 from by linarith [ hu.2 x hx ] ];
  · linarith [ hu.2 x hx, show 0 ≤ ∑ i ∈ Finset.range d, |( Q x |> Polynomial.coeff ) i| from Finset.sum_nonneg fun _ _ => abs_nonneg _ ]
...
lemma roots_near_roots (d : ℕ) (hd : 1 ≤ d)
  ...
  have hK_compact : IsCompact K := by
    ...
    exacts [ -M, M, by exact IsClosed.inter ( isClosed_Icc ) ( isClosed_of_closure_subset fun x hx => by
      ...
      exact fun y₀ hy₀ => le_of_tendsto_of_tendsto' tendsto_const_nhds ( Filter.Tendsto.abs ( hx.choose_spec.2.sub_const y₀ ) ) fun n => hx.choose_spec.1 n y₀ hy₀ ), fun x hx => hx.1 ];
  -- Apply the generalized tube lemma to get open `u₀, v` with `x₀ ∈ u₀`, `K ⊆ v`, and `u₀ ×ˢ v ⊆ O`.
  obtain ⟨u₀, v, hu₀, hv, huv⟩ : ∃ u₀ v : Set ℝ, IsOpen u₀ ∧ IsOpen v ∧ x₀ ∈ u₀ ∧ K ⊆ v ∧ u₀ ×ˢ v ⊆ {p : ℝ × ℝ | eval p.2 (Q p.1) ≠ 0} := by
    have h_generalized_tube : {x₀} ×ˢ K ⊆ {p : ℝ × ℝ | eval p.2 (Q p.1) ≠ 0} := by
      simp +zetaDelta at *;
      simp +contextual [ Set.subset_def, mem_realRootFinset ( show Q x₀ ≠ 0 from Polynomial.Monic.ne_zero ( hmonic x₀ ) ) ];
      exact fun a b ha hb₁ hb₂ hb₃ hb₄ => not_lt_of_ge ( hb₃ b hb₄ ) ( by simpa [ hb₄ ] using hε );
    have := @generalized_tube_lemma;
    specialize this ( isCompact_singleton : IsCompact { x₀ } ) hK_compact ( show IsOpen { p : ℝ × ℝ | eval p.2 ( Q p.1 ) ≠ 0 } from isOpen_compl_iff.mpr <| isClosed_eq ( hsmooth.continuous ) continuous_const ) h_generalized_tube ; simp_all [ Set.subset_def ];
  refine' ⟨ u₀ ∩ u₁, Filter.inter_mem ( hu₀.mem_nhds huv.1 ) hu₁.1, fun x hx y hy => _ ⟩;
  contrapose! huv;
  simp_all [ Set.not_subset ];
  exact fun _ _ => ⟨ x, y, ⟨ hx.1, by have := hu₁.2 x hx.2 y hy; exact ‹K ⊆ v› ⟨ ⟨ by linarith [ abs_le.mp this ], by linarith [ abs_le.mp this ] ⟩, fun y₀ hy₀ => huv y₀ <| by simpa using mem_realRootFinset ( hmonic x₀ |> fun h => h.ne_zero ) |>.1 hy₀ ⟩ ⟩, hy ⟩
...
lemma local_root_cover
  ...
  obtain ⟨y, hy⟩ : ∃ y : Fin (S.card) → ℝ, StrictMono y ∧ S = Finset.image y Finset.univ := by
    use fun i => S.orderEmbOfFin rfl i; simp [ StrictMono ] ;
  generalize_proofs at *; (
  ...
  obtain ⟨φ, hφ⟩ : ∃ φ : Fin (S.card) → ℝ → ℝ, (∀ i, φ i x₀ = y i) ∧ (∀ i, ContDiffAt ℝ ⊤ (φ i) x₀) ∧ (∀ i, ∀ᶠ x in nhds x₀, (Q x).eval (φ i x) = 0) := by
    have h_local_branch : ∀ i, ∃ φ : ℝ → ℝ, φ x₀ = y i ∧ ContDiffAt ℝ ⊤ φ x₀ ∧ ∀ᶠ x in nhds x₀, (Q x).eval (φ x) = 0 := by
      intro i
      have h_root : (Q x₀).eval (y i) = 0 := by
        exact mem_realRootFinset ( hmonic x₀ |> fun h => h.ne_zero ) |>.1 ( hy.2.symm ▸ Finset.mem_image_of_mem _ ( Finset.mem_univ _ ) ) |> fun h => by simpa using h;
      generalize_proofs at *; (
      apply local_branch_of_simple hsmooth x₀ (y i) h_root (eval_derivative_ne_zero_of_separable (hsep x₀ hx₀) h_root))
    generalize_proofs at *; (
    exact ⟨ fun i => Classical.choose ( h_local_branch i ), fun i => Classical.choose_spec ( h_local_branch i ) |>.1, fun i => Classical.choose_spec ( h_local_branch i ) |>.2.1, fun i => Classical.choose_spec ( h_local_branch i ) |>.2.2 ⟩)
  generalize_proofs at *; (
  -- Apply `local_unique_of_simple` to get `u i ∈ 𝓝 x₀`, `v i ∈ 𝓝 (y i)`.
  obtain ⟨u, v, hu, hv, huv⟩ : ∃ u : Fin (S.card) → Set ℝ, ∃ v : Fin (S.card) → Set ℝ, (∀ i, u i ∈ nhds x₀) ∧ (∀ i, v i ∈ nhds (y i)) ∧ (∀ i, ∀ x ∈ u i, ∀ y₁ ∈ v i, ∀ y₂ ∈ v i, (Q x).eval y₁ = 0 → (Q x).eval y₂ = 0 → y₁ = y₂) := by
    have h_unique : ∀ i, ∃ u v : Set ℝ, u ∈ nhds x₀ ∧ v ∈ nhds (y i) ∧ ∀ x ∈ u, ∀ y₁ ∈ v, ∀ y₂ ∈ v, (Q x).eval y₁ = 0 → (Q x).eval y₂ = 0 → y₁ = y₂ := by
      intro i
      apply local_unique_of_simple hsmooth x₀ (y i) (eval_derivative_ne_zero_of_separable (hsep x₀ hx₀) (by
      exact mem_realRootFinset ( hmonic x₀ |> fun h => h.ne_zero ) |>.1 ( hy.2.symm ▸ Finset.mem_image_of_mem _ ( Finset.mem_univ _ ) ) |> fun h => by simpa using h;))
    generalize_proofs at *; (
    exact ⟨ fun i => Classical.choose ( h_unique i ), fun i => Classical.choose_spec ( h_unique i ) |> Classical.choose, fun i => Classical.choose_spec ( h_unique i ) |> Classical.choose_spec |> And.left, fun i => Classical.choose_spec ( h_unique i ) |> Classical.choose_spec |> And.right |> And.left, fun i => Classical.choose_spec ( h_unique i ) |> Classical.choose_spec |> And.right |> And.right ⟩)
  generalize_proofs at *; (
  ...
  obtain ⟨ε, hε_pos, hε⟩ : ∃ ε > 0, (∀ i j, i ≠ j → 2 * ε ≤ |y i - y j|) ∧ (∀ i, Metric.ball (y i) ε ⊆ v i) := by
    ...
    obtain ⟨ε₁, hε₁_pos, hε₁⟩ : ∃ ε₁ > 0, ∀ i j, i ≠ j → 2 * ε₁ ≤ |y i - y j| := by
      by_cases h_empty : Finset.card (Finset.image (fun p : Fin S.card × Fin S.card => |y p.1 - y p.2|) (Finset.filter (fun p => p.1 ≠ p.2) (Finset.univ : Finset (Fin S.card × Fin S.card)))) = 0;
      · simp +zetaDelta at *;
        exact ⟨ 1, zero_lt_one, fun i j hij => False.elim <| hij <| h_empty i j ⟩;
      · obtain ⟨ε₁, hε₁⟩ : ∃ ε₁ ∈ Finset.image (fun p : Fin S.card × Fin S.card => |y p.1 - y p.2|) (Finset.filter (fun p => p.1 ≠ p.2) (Finset.univ : Finset (Fin S.card × Fin S.card))), ∀ ε ∈ Finset.image (fun p : Fin S.card × Fin S.card => |y p.1 - y p.2|) (Finset.filter (fun p => p.1 ≠ p.2) (Finset.univ : Finset (Fin S.card × Fin S.card))), ε₁ ≤ ε := by
          exact ⟨ Finset.min' _ <| Finset.card_pos.mp <| Nat.pos_of_ne_zero h_empty, Finset.min'_mem _ _, fun ε hε => Finset.min'_le _ _ hε ⟩
        generalize_proofs at *; (
        simp +zetaDelta at *;
        exact ⟨ ε₁ / 2, half_pos ( by obtain ⟨ a, b, hab, rfl ⟩ := hε₁.1; exact abs_pos.mpr ( sub_ne_zero.mpr <| hy.1.injective.ne hab ) ), fun i j hij => by linarith [ hε₁.2 _ i j hij rfl ] ⟩)
    generalize_proofs at *; (
    ...
    obtain ⟨ε₂, hε₂_pos, hε₂⟩ : ∃ ε₂ > 0, ∀ i, Metric.ball (y i) ε₂ ⊆ v i := by
      ...
      · exact ⟨ Finset.min' ( Finset.univ.image ε₂ ) ⟨ _, Finset.mem_image_of_mem _ ( Finset.mem_univ h.some ) ⟩, by have := Finset.min'_mem ( Finset.univ.image ε₂ ) ⟨ _, Finset.mem_image_of_mem _ ( Finset.mem_univ h.some ) ⟩ ; aesop, fun i => Set.Subset.trans ( Metric.ball_subset_ball ( Finset.min'_le _ _ <| Finset.mem_image_of_mem _ <| Finset.mem_univ i ) ) <| hε₂ i |>.2 ⟩;
      · exact ⟨ 1, zero_lt_one ⟩
    generalize_proofs at *; (
    exact ⟨ Min.min ε₁ ε₂, lt_min hε₁_pos hε₂_pos, fun i j hij => le_trans ( mul_le_mul_of_nonneg_left ( min_le_left _ _ ) zero_le_two ) ( hε₁ i j hij ), fun i => Set.Subset.trans ( Metric.ball_subset_ball ( min_le_right _ _ ) ) ( hε₂ i ) ⟩))
  generalize_proofs at *; (
  -- Apply `roots_near_roots` to get `uε ∈ 𝓝 x₀`.
  obtain ⟨uε, huε⟩ : ∃ uε ∈ nhds x₀, ∀ x ∈ uε, ∀ y : ℝ, (Q x).eval y = 0 → ∃ y₀ : ℝ, (Q x₀).eval y₀ = 0 ∧ |y - y₀| < ε := by
    by_cases hd : 1 ≤ d <;> simp_all [ realRootFinset ];
    · exact roots_near_roots ( Q := Q ) d hd hmonic hdeg hsmooth x₀ hε_pos |> fun ⟨ u, hu₁, hu₂ ⟩ => ⟨ u, hu₁, fun x hx y hy => by simpa using hu₂ x hx y hy ⟩ ;
  ...
  obtain ⟨δ, hδ_pos, hδ⟩ : ∃ δ > 0, Metric.ball x₀ δ ⊆ uε ∧ (∀ i, Metric.ball x₀ δ ⊆ u i) ∧ (∀ i, ∀ x ∈ Metric.ball x₀ δ, (Q x).eval (φ i x) = 0) ∧ (∀ i, ∀ x ∈ Metric.ball x₀ δ, φ i x ∈ Metric.ball (y i) ε) ∧ (∀ i, ContDiffOn ℝ ⊤ (φ i) (Metric.ball x₀ δ)) := by
    have hδ : ∀ i, ∃ δi > 0, Metric.ball x₀ δi ⊆ u i ∧ (∀ x ∈ Metric.ball x₀ δi, (Q x).eval (φ i x) = 0) ∧ (∀ x ∈ Metric.ball x₀ δi, φ i x ∈ Metric.ball (y i) ε) ∧ ContDiffOn ℝ ⊤ (φ i) (Metric.ball x₀ δi) := by
      intro i
      obtain ⟨δi, hδi_pos, hδi⟩ : ∃ δi > 0, Metric.ball x₀ δi ⊆ u i ∧ (∀ x ∈ Metric.ball x₀ δi, (Q x).eval (φ i x) = 0) ∧ (∀ x ∈ Metric.ball x₀ δi, φ i x ∈ Metric.ball (y i) ε) := by
        have hδi : ∀ᶠ x in nhds x₀, (Q x).eval (φ i x) = 0 ∧ φ i x ∈ Metric.ball (y i) ε := by
          exact Filter.Eventually.and ( hφ.2.2 i ) ( by simpa [ hφ.1 i ] using hφ.2.1 i |> ContDiffAt.continuousAt |> fun h => h.eventually ( Metric.ball_mem_nhds _ hε_pos ) ) ;
        generalize_proofs at *; (
        rcases Metric.mem_nhds_iff.mp ( hu i ) with ⟨ δ₁, hδ₁_pos, hδ₁ ⟩ ; rcases Metric.mem_nhds_iff.mp hδi with ⟨ δ₂, hδ₂_pos, hδ₂ ⟩ ; exact ⟨ Min.min δ₁ δ₂, lt_min hδ₁_pos hδ₂_pos, fun x hx => hδ₁ <| Metric.mem_ball.mpr <| lt_of_lt_of_le hx <| min_le_left _ _, fun x hx => hδ₂ ( Metric.mem_ball.mpr <| lt_of_lt_of_le hx <| min_le_right _ _ ) |>.1, fun x hx => hδ₂ ( Metric.mem_ball.mpr <| lt_of_lt_of_le hx <| min_le_right _ _ ) |>.2 ⟩ ;)
      generalize_proofs at *; (
      obtain ⟨δi', hδi'_pos, hδi'⟩ : ∃ δi' > 0, ContDiffOn ℝ ⊤ (φ i) (Metric.ball x₀ δi') := by
        have := hφ.2.1 i
        generalize_proofs at *; (
        have := this.eventually ( by norm_num ) ; simp_all [ Metric.eventually_nhds_iff ] ;
        exact ⟨ this.choose, this.choose_spec.1, fun x hx => this.choose_spec.2 hx |> ContDiffAt.contDiffWithinAt ⟩)
      generalize_proofs at *; (
      exact ⟨ Min.min δi δi', lt_min hδi_pos hδi'_pos, Set.Subset.trans ( Metric.ball_subset_ball ( min_le_left _ _ ) ) hδi.1, fun x hx => hδi.2.1 x ( Metric.ball_subset_ball ( min_le_left _ _ ) hx ), fun x hx => hδi.2.2 x ( Metric.ball_subset_ball ( min_le_left _ _ ) hx ), hδi'.mono ( Metric.ball_subset_ball ( min_le_right _ _ ) ) ⟩))
    generalize_proofs at *; (
    ...
    obtain ⟨δ', hδ'_pos, hδ'⟩ : ∃ δ' > 0, Metric.ball x₀ δ' ⊆ uε ∧ ∀ i, δ' ≤ δ i := by
      ...
      · exact ⟨ Min.min δ' ( Finset.min' ( Finset.univ.image δ ) ⟨ _, Finset.mem_image_of_mem δ ( Finset.mem_univ ⟨ 0, Nat.pos_of_ne_zero hS_card ⟩ ) ⟩ ), lt_min hδ'_pos ( Finset.min'_mem ( Finset.univ.image δ ) ⟨ _, Finset.mem_image_of_mem δ ( Finset.mem_univ ⟨ 0, Nat.pos_of_ne_zero hS_card ⟩ ) ⟩ |> fun h => by obtain ⟨ i, _, hi ⟩ := Finset.mem_image.mp h; linarith [ hδ_pos i ] ), Set.Subset.trans ( Metric.ball_subset_ball ( min_le_left _ _ ) ) hδ', fun i => min_le_right _ _ |> le_trans <| Finset.min'_le _ _ <| Finset.mem_image_of_mem δ <| Finset.mem_univ i ⟩)
    generalize_proofs at *; (
    exact ⟨ δ', hδ'_pos, hδ'.1, fun i => Set.Subset.trans ( Metric.ball_subset_ball ( hδ'.2 i ) ) ( hδ i |>.1 ), fun i x hx => hδ i |>.2.1 x ( Metric.ball_subset_ball ( hδ'.2 i ) hx ), fun i x hx => hδ i |>.2.2.1 x ( Metric.ball_subset_ball ( hδ'.2 i ) hx ), fun i => ContDiffOn.mono ( hδ i |>.2.2.2 ) ( Metric.ball_subset_ball ( hδ'.2 i ) ) ⟩)))
  generalize_proofs at *; (
  ...
  · refine' ⟨ _, _, _ ⟩
    ...
    · intro x hx₁ hx₂ i j hij; have := hε.1 i j; contrapose! this; simp_all [ dist_eq_norm ] ;
      exact abs_lt.mpr ⟨ by linarith [ abs_lt.mp ( hδ.2.2.2.1 i x hx₂ ), abs_lt.mp ( hδ.2.2.2.1 j x hx₂ ) ], by linarith [ abs_lt.mp ( hδ.2.2.2.1 i x hx₂ ), abs_lt.mp ( hδ.2.2.2.1 j x hx₂ ) ] ⟩;
    · intro x hx₁ hx₂ y hy; specialize huε; specialize hδ; have := huε.2 x ( hδ.1 hx₂ ) y hy; obtain ⟨ y₀, hy₀₁, hy₀₂ ⟩ := this; simp_all [ Finset.ext_iff ] ;
      -- Since $y₀$ is a root of $Q x₀$, there exists some $i$ such that $y₀ = y i$.
      obtain ⟨i, hi⟩ : ∃ i : Fin S.card, y₀ = ‹Fin S.card → ℝ› i := by
        exact hS_def y₀ |>.2 ( by exact Multiset.mem_toFinset.mpr <| Polynomial.mem_roots ( hmonic x₀ |> fun h => h.ne_zero ) |>.2 hy₀₁ ) |> fun ⟨ i, hi ⟩ => ⟨ i, hi.symm ⟩ ;
      generalize_proofs at *; (
      specialize huv i x ( hδ.2.1 i hx₂ ) y ( by
        exact hε.2 i <| by simpa [ hi ] using hy₀₂; ) ( φ i x ) ( by
        exact hε.2 i ( hδ.2.2.2.1 i x hx₂ ) ) hy ( hδ.2.2.1 i x hx₂ ) ; aesop ( simp_config := { singlePass := true } ) ;))))))))
...
lemma nRealRoots_eventuallyConst
  ...
  have h_realRootFinset_eq : ∀ x ∈ s, realRootFinset Q x = Finset.image (fun i => φ i x) Finset.univ := by
    intro x hx; ext y; simp [ realRootFinset, hs₅, hs₇ x hx ] ;
    exact ⟨ fun h => hs₇ x hx y h.2, fun ⟨ i, hi ⟩ => ⟨ by specialize hmonic x; aesop_cat, by simpa [ hi ] using hs₅ i x hx ⟩ ⟩;
  filter_upwards [ hs₁ ] with x hx ; simp_all [ nRealRoots ];
...
lemma nRealRoots_const
  ...
  intro x hx;
  have h_const : IsPreconnected (Set.Ici T₀) := by
    exact isPreconnected_Ici;
  exact h_const.image _ h_cont |> fun h => h.subsingleton ( Set.mem_image_of_mem _ hx ) ( Set.mem_image_of_mem _ ( Set.self_mem_Ici ) )
...
theorem smooth_separable_family_root_branches
    (Q : ℝ → Polynomial ℝ) (T₀ : ℝ) (d : ℕ) (hd : 1 ≤ d)
    (hmonic : ∀ x, (Q x).Monic) (hdeg : ∀ x, (Q x).natDegree = d)
    (hsmooth : ContDiff ℝ ⊤ (fun p : ℝ × ℝ => (Q p.1).eval p.2))
    (hsep : ∀ x, T₀ ≤ x → (Q x).Separable) :
    ∃ (n : ℕ) (g : Fin n → ℝ → ℝ),
  ...
  let g := fun j : Fin n => fun x => if h : (realRootFinset Q x).card = n then (realRootFinset Q x).orderEmbOfFin h j else 0;
  refine' ⟨ n, g, _, _, _, _ ⟩;
  · intro j;
    intro x₀ hx₀;
    obtain ⟨ m, φ, s, hs₁, hs₂, hs₃, hs₄, hs₅, hs₆, hs₇ ⟩ := local_root_cover T₀ d hmonic hdeg hsmooth hsep hx₀;
    ...
    have hm_eq_n : m = n := by
      have hm_eq_n : ∀ x ∈ s, (realRootFinset Q x).card = m := by
        ...
        have h_card_eq : (realRootFinset Q x).card = Finset.card (Finset.image (fun i => φ i x) Finset.univ) := by
          congr with y ; simp [ mem_realRootFinset ( hmonic x |> fun h => h.ne_zero ) ];
    ...
    obtain ⟨σ, hσ⟩ : ∃ σ : Equiv.Perm (Fin n), StrictMono (fun j => φ (Fin.cast hm_eq_n.symm (σ j)) x₀) := by
      have h_order : ∃ σ : Fin n → Fin n, StrictMono (fun j => φ (Fin.cast hm_eq_n.symm (σ j)) x₀) := by
        have h_distinct : ∀ i j : Fin n, i ≠ j → φ (Fin.cast hm_eq_n.symm i) x₀ ≠ φ (Fin.cast hm_eq_n.symm j) x₀ := by
          exact fun i j hij => fun h => hij <| Fin.ext <| by have := hs₆ x₀ hs₂; have := @this ( Fin.cast hm_eq_n.symm i ) ( Fin.cast hm_eq_n.symm j ) ; aesop;
        have h_order : ∃ σ : Fin n → ℝ, StrictMono σ ∧ ∀ i, σ i ∈ Finset.image (fun i : Fin n => φ (Fin.cast hm_eq_n.symm i) x₀) Finset.univ := by
          have h_order : Finset.card (Finset.image (fun i : Fin n => φ (Fin.cast hm_eq_n.symm i) x₀) Finset.univ) = n := by
            rw [ Finset.card_image_of_injective _ fun i j hij => by contrapose hij; exact h_distinct i j hij, Finset.card_fin ];
    ...
    have h_strict_mono : ∀ x ∈ s, StrictMono (fun j => φ (Fin.cast hm_eq_n.symm (σ j)) x) := by
      ...
      have h_ivt : ∃ c ∈ Set.uIcc x₀ x, φ (Fin.cast hm_eq_n.symm (σ j)) c - φ (Fin.cast hm_eq_n.symm (σ k)) c = 0 := by
        have h_ivt : ContinuousOn (fun x => φ (Fin.cast hm_eq_n.symm (σ j)) x - φ (Fin.cast hm_eq_n.symm (σ k)) x) (Set.uIcc x₀ x) := by
          exact ContinuousOn.sub ( hs₄ _ |> ContDiffOn.continuousOn |> ContinuousOn.mono <| by intro y hy; cases Set.mem_uIcc.mp hy <;> [ exact hs₃.ordConnected.out hs₂ hx ‹_› ; exact hs₃.ordConnected.out hx hs₂ ‹_› ] ) ( hs₄ _ |> ContDiffOn.continuousOn |> ContinuousOn.mono <| by intro y hy; cases Set.mem_uIcc.mp hy <;> [ exact hs₃.ordConnected.out hs₂ hx ‹_› ; exact hs₃.ordConnected.out hx hs₂ ‹_› ] );
        have := h_ivt.image_uIcc;
        exact this.symm.subset ( Set.mem_Icc.mpr ⟨ by nlinarith [ hσ hjk, Set.mem_Icc.mp ( this ▸ Set.mem_image_of_mem _ ( Set.left_mem_uIcc ) ), Set.mem_Icc.mp ( this ▸ Set.mem_image_of_mem _ ( Set.right_mem_uIcc ) ) ], by nlinarith [ hσ hjk, Set.mem_Icc.mp ( this ▸ Set.mem_image_of_mem _ ( Set.left_mem_uIcc ) ), Set.mem_Icc.mp ( this ▸ Set.mem_image_of_mem _ ( Set.right_mem_uIcc ) ) ] ⟩ );
      obtain ⟨ c, hc₁, hc₂ ⟩ := h_ivt;
      have := hs₆ c ( by cases Set.mem_uIcc.mp hc₁ <;> [ exact hs₃.ordConnected.out hs₂ hx ‹_› ; exact hs₃.ordConnected.out hx hs₂ ‹_› ] ) ; simp_all [ sub_eq_iff_eq_add, Function.Injective.eq_iff this ] ;
    ...
    have h_eq : ∀ x ∈ s, T₀ ≤ x → g j x = φ (Fin.cast hm_eq_n.symm (σ j)) x := by
      ...
      have h_eq : ∀ j : Fin n, φ (Fin.cast hm_eq_n.symm (σ j)) x = (realRootFinset Q x).orderEmbOfFin (h_card x hxT₀) j := by
    ...
    · exact h_eq x₀ hs₂ hx₀;
  · intro j x hx; specialize h_card x hx; simp [ g, h_card ] ;
    exact mem_realRootFinset ( hmonic x |> fun h => h.ne_zero ) |>.1 ( Finset.orderEmbOfFin_mem _ _ _ );
  · intro x hx y hy; specialize h_card x hx;
    have := Finset.mem_image.mp ( show y ∈ Finset.image ( fun j : Fin n => ( realRootFinset Q x |> Finset.orderEmbOfFin <| h_card ) j ) Finset.univ from ?_ ) ; aesop;
    simp_all [ Finset.ext_iff ];
    exact Multiset.mem_toFinset.mpr ( Polynomial.mem_roots ( show Q x ≠ 0 from by specialize hmonic x; aesop ) |>.2 hy )
  · intro x hx i i' hii
    have hc := h_card x hx
