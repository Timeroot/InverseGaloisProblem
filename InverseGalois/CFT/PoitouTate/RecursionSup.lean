...
theorem exists_place_sUnit_prescribed_of_sup (hp : p.Prime)
  ...
  have hrad : ∀ u ∈ sUnits K (Tn : Set (HeightOneSpectrum (𝓞 K))), ∀ y : (↥Ω)ˣ,
    ...
    exact prescriptionChar_eq_one_of_pow_sup hp hres hζ hT hpTn hg hginf hc hcT
      (fun v hv => hcunr v (hT hv)) hcn hsup hcomm hexp hsplit hram₁ hram₂ hb
  obtain ⟨V, hVT, hVstab, hVP, hQTn, z, hzmem, hzinf, hzTn, hzval, hzQ⟩ :=
  ...
  exact ⟨V, hVT, hVstab, hVP, hQTn, z, hzmem, hzinf, hzTn,
...
theorem exists_place_sUnit_prescribed_of_sup_zpowers (hp : p.Prime)
    {ζ : K} (hζ : IsPrimitiveRoot ζ p)
    (hres : ∀ v : HeightOneSpectrum (𝓞 K), HasResidueChar (v.adicCompletion K) (Pc v) (Ec v))
    (Tk : Finset (HeightOneSpectrum (𝓞 k))) {T Tn : Finset (HeightOneSpectrum (𝓞 K))}
    (hT : T ⊆ Tn) (hTk : ∀ v ∈ Tn, primeUnder (𝓞 k) v ∈ Tk)
  ...
  have hcunr : ∀ v ∈ Tn, v ∉ T → c v ∈ localUnramified v p := by
    ...
    rw [hcT v hv hvT]
    exact one_mem _
  exact exists_place_sUnit_prescribed_of_rad (T := T) hp hζ hres Tk hTk hpTn hrepr hcunr
    fun u hu y hy => by
      have hb : algebraMap K ↥Ω ((u : Kˣ) : K) = ((y : (↥Ω)ˣ) : ↥Ω) ^ p := by
        have hy' := congrArg Units.val hy
        rwa [Units.coe_map, MonoidHom.coe_coe, Units.val_pow_eq_pow_val] at hy'
      exact prescriptionChar_eq_one_of_pow_sup_zpowers hp hres hζ hT hpTn hg hginf hc hcT
        hnegT hsup hcomm hexp hsplit hram₁ hcyc₂ hb
