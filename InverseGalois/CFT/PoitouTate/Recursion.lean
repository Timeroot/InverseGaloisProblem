...
unramified everywhere, agrees with the class of a fixed unit on a fixed set of places, and records
the inverse of the class of each unit at the nontrivial conjugates of its own place; the chosen
places satisfy the given splitting condition, have trivial decomposition group, and are pairwise
...
structure RecInv (k : Type) {K : Type} [Field k] [Field K] [NumberField K] [Algebra k K]
  ...
  unram : ∀ v ∈ d.places, d.pres v ∈ localUnramified v p
...
theorem exists_recInv_succ
  ...
  have hunram : ∀ v ∈ S', c' v ∈ localUnramified v p := by
    intro v _
    by_cases hvS : v ∈ d.places
    · rw [hc'old v hvS]
      exact hd.unram v hvS
    · by_cases hvQ : v = Q
      · subst hvQ
        rw [hc'Q]
        exact one_mem _
  ...
  have hunitUnram : ∀ i < n + 1, ∀ v : HeightOneSpectrum (𝓞 K), v ≠ Pl' i →
