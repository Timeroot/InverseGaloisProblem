...
* `InverseGalois.CFT.exists_units_named_unram_prescribed`: the same for an unramified naming made
  away from a stable set containing the places above the exponent and the places ramified over the
  base, where the units carry no ramification at the named places either.
...
variable (Ω) in
/-- **An unramified naming has a family of totally positive units carrying it**, trivial on a
prescribed set of places disjoint from the named ones, dying at every conjugate of a named place
which is not itself named, and ramified only over the named places or at places completely split in
the auxiliary field with a single coordinate surviving.

The named places are asked to avoid a Galois stable set containing the places above the exponent
and the places ramified over the base, and the naming to be unramified; the prescription the
two-place construction is run over is then unramified everywhere, and the units realising it may be
taken to be local powers at every infinite place. -/
theorem exists_units_named_unram_prescribed (hp : p.Prime) {ζ : K}
    (hζ : IsPrimitiveRoot ζ p)
    (hres : ∀ v : HeightOneSpectrum (𝓞 K), HasResidueChar (v.adicCompletion K) (Pc v) (Ec v))
    {Tp Tz Tram B : Finset (HeightOneSpectrum (𝓞 K))} (hdisj : ∀ v ∈ Tp, v ∉ Tz)
    (hram : ∀ v ∉ Tram, ∃ W : HeightOneSpectrum (𝓞 ↥Ω),
      primeUnder (𝓞 K) W = v ∧ ramIdx (𝓞 K) W = 1)
  ...
  have hTrst : ∀ (σ : Gal(K/k)) (v : HeightOneSpectrum (𝓞 K)), v ∈ Tr → σ • v ∈ Tr := by
    ...
    obtain ⟨τ, w, hw⟩ := (hmemTr v).1 hv
    exact (hmemTr _).2 ⟨σ * τ, w, by rw [mul_smul, hw]⟩
  obtain ⟨Ts, hTs0, hTsst, -, hrepr⟩ := exists_stable_ord_places k K (ℓ := p) hp.ne_zero
    (((Tr ∪ Tz ∪ Tram ∪ B : Finset (HeightOneSpectrum (𝓞 K))) :
      Set (HeightOneSpectrum (𝓞 K)))) (Finset.finite_toSet _)
  have hTrTs : Tr ⊆ Ts := fun v hv => hTs0 (Finset.mem_coe.2
    (Finset.mem_union_left _ (Finset.mem_union_left _ (Finset.mem_union_left _ hv))))
  have hTzTs : Tz ⊆ Ts := fun v hv => hTs0 (Finset.mem_coe.2
    (Finset.mem_union_left _ (Finset.mem_union_left _ (Finset.mem_union_right _ hv))))
  have hTramTs : Tram ⊆ Ts := fun v hv => hTs0 (Finset.mem_coe.2
    (Finset.mem_union_left _ (Finset.mem_union_right _ hv)))
  have hBTs : B ⊆ Ts := fun v hv => hTs0 (Finset.mem_coe.2 (Finset.mem_union_right _ hv))
  obtain ⟨Tn, hTsTn, -, S, Q, R, E, z, hfam⟩ :=
    exists_isTwoPlaceFamily_named_unram_of_orthogonal Ω hp hζ hres
      (fun v hv => hTrTs (hTpTr hv)) hBTs hBstable hBwild hBram hTpB hTsst hrepr hclunr
      (fun Tn hTn => horth Tn fun v hv => hTn (hTrTs (hTpTr hv)))
      (fun v hv => hram v fun hc => hv (hTramTs hc)) d
  refine ⟨z, ?_, ?_, ?_, ?_⟩
  · intro q hq v hv
  ...
  · rintro v ⟨q, hqd, hq⟩
    have hQR : v = Q q ∨ v = R q ∨ v = E q := by
      by_contra hcon
      push_neg at hcon
      exact hq (hfam.unram q hqd v (Finset.notMem_empty v) hcon.1 hcon.2.1 hcon.2.2)
    rcases hQR with rfl | rfl | rfl
    · refine ⟨?_, ⟨q, hqd, ?_⟩, ?_⟩
      · have hS : Q q ∈ S := by
          have h1 := hfam.memQ q hqd 1
