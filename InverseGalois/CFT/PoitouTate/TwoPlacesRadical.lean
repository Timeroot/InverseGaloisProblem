...
The prescription may be ramified on a distinguished stable part of the set carrying it, provided it
is supported at no more than one place of each orbit there and its class lies, together with the
classes of the radicands, on one line at each of those places.

## Main results

* `InverseGalois.CFT.exists_places_sUnit_radical`: **three places completely split in a compositum
  whose second factor is generated by radicals, killing every radicand of that factor, and a unit
  ramified only at those places realising a prescribed local behaviour.**
* `InverseGalois.CFT.exists_two_places_sUnit_radical_zpowers`: the same for a prescription which
  may be ramified where it is carried, its class lying there on a line together with the classes of
  the radicands.
...
/-- **Three places completely split in a compositum with a radical second factor, and a unit
ramified only at them realising a prescribed local behaviour.**  A single step of the recursion over
the compositum moves the prescription onto a unit which fails to be a unit at one new place only,
and that place is completely split in the whole compositum; the construction of the places may then
be run over the compositum starting from it.  The places it produces are completely split there, so
every radicand of the second factor is a power in their completions and has trivial class.  At an
odd exponent the last two places coincide. -/
theorem exists_places_sUnit_radical (hp : p.Prime)
  ...
  classical
  have hpTn : ∀ v : HeightOneSpectrum (𝓞 K), FinitePlace.mk v ((p : ℕ) : K) ≠ 1 → v ∈ Tn :=
    fun v hv => hT (hBT (hBwild v hv))
  have hcn : ∀ v ∈ T, FinitePlace.mk v ((p : ℕ) : K) ≠ 1 → c v = 1 :=
    fun v _ hv => hcB v (hBwild v hv)
  obtain ⟨V, -, hVstab, -, -, -, y, hyS, hyinf, hyc, -, -⟩ :=
    exists_place_sUnit_prescribed_of_radical (Ω := Ω) (M₁ := M₁) (M₂ := M₂) hp hζ hres
      (Tn.image (primeUnder (𝓞 k))) hT (fun v hv => Finset.mem_image_of_mem _ hv) hpTn hrepr
      hcunr hg hginf hc hcT hcn hsup hpow hgen hord hsplit hram₁
  ...
  have hyunr : ∀ v ∈ Tn, localClassHom v p y ∈ localUnramified v p := by
    ...
    rw [hyc v hv]
    exact hcunr v hv
  have hyB : ∀ v ∈ B, localClassHom v p y = 1 := by
    intro v hv
    rw [hyc v (hT (hBT hv))]
    exact hcB v hv
  obtain ⟨Q, R, E, hQT, hRT, hET, hQspl, hRspl, hEspl, hQstab, hRstab, hEstab, z, hzT, hzinf,
    hzunr, hzQ, hzQc, hzRc, hzEc⟩ :=
    exists_places_sUnit_class_eq_of_split (Ω := Ω) hp hζ hres hTstable (hBT.trans hT) hBstable
      hBwild hBram hysplit hyinf hyunr hyB
  refine ⟨Q, R, E, hQT, hRT, hET, hQspl, hRspl, hEspl, ?_, ?_, ?_, hQstab, hRstab, hEstab, z,
    fun v hv => (hzT v hv).trans (hyc v hv), hzinf, hzunr, hzQ, hzQc, hzRc, hzEc⟩
...
theorem exists_two_places_sUnit_radical_zpowers (hp : p.Prime) (hodd : 2 < p)
  ...
  obtain ⟨V, -, hVstab, -, -, -, y, hyS, -, hyc, -, -⟩ :=
    exists_place_sUnit_prescribed_of_radical_zpowers (Ω := Ω) (M₁ := M₁) (M₂ := M₂) hp hζ hres
      (Tn.image (primeUnder (𝓞 k))) hT (fun v hv => Finset.mem_image_of_mem _ hv) hpTn hrepr
      hg (fun w => infClassHom_eq_one_of_isNegOnePow hp hneg hζ w _) hc hcT
      (fun v _ => hneg.map (algebraMap K (v.adicCompletion K))) hsup hpow hgen hDc hDb hsplit
      hram₁
  have hysplit : ∀ v : HeightOneSpectrum (𝓞 K), v ∉ Tn → Rigidity.RET.ord K v (y : K) ≠ 0 →
      ∃ w : HeightOneSpectrum (𝓞 ↥Ω), primeUnder (𝓞 K) w = v ∧
        stabilizer Gal(↥Ω/k) w = ⊥ := by
    ...
    · exact ⟨V, rfl, hVstab⟩
    · exact absurd hmem hv
  have hyunr : ∀ v ∈ Tn, v ∉ Tr → localClassHom v p y ∈ localUnramified v p := by
    intro v hv hvTr
    rw [hyc v hv]
