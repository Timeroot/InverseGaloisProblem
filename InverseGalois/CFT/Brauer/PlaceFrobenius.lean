...
* `InverseGalois.CFT.restrictToBase_divisionFrobenius_eq_of_adjoin`: **when the extension is
  generated by that root of unity the description determines the Frobenius.**
* `InverseGalois.CFT.restrictToBase_divisionFrobenius_rootOfUnity_prime`: at a place of the base of
  residue degree one the power is the rational prime below the place.
* `InverseGalois.CFT.restrictToBase_divisionFrobenius_rootOfUnity_rat`: **over the rationals the
  Frobenius raises a root of unity to the power of the rational prime below the place.**
* `InverseGalois.CFT.restrictToBase_divisionFrobenius_eq_of_adjoin_rat`: **over the rationals the
  Frobenius of an unramified place of a field generated by a root of unity is the automorphism
  raising it to the power of the rational prime below the place.**
...
automorphism raising that root of unity to the power given by the number of residues of the
base.**  Two automorphisms which agree on a generator of the extension are equal, so the cyclotomic
description determines the Frobenius as an element of the Galois group. -/
theorem restrictToBase_divisionFrobenius_eq_of_adjoin (hram : ramIdx (𝓞 k) w = 1) {m : ℕ}
    (hm : m ≠ 0) (hmv : w.valuation K (m : K) = 1) {ζ : K} (hζ : ζ ^ m = 1)
    (hgen : Algebra.adjoin k ({ζ} : Set K) = ⊤) {τ : Gal(K/k)}
    (hτ : τ ζ = ζ ^ Nat.card (DivisionResidue ((primeUnder (𝓞 k) w).adicCompletion k)
      ((primeUnder (𝓞 k) w).adicCompletion k))) :
    restrictToBase k w (divisionFrobenius ((primeUnder (𝓞 k) w).adicCompletion k)
        (w.adicCompletion K)
        (fun z hz => exists_divisionNorm_eq_norm_adicCompletion k w hram z hz)) = τ := by
  have h : ((restrictToBase k w (divisionFrobenius ((primeUnder (𝓞 k) w).adicCompletion k)
        (w.adicCompletion K)
        (fun z hz => exists_divisionNorm_eq_norm_adicCompletion k w hram z hz)) :
        K →ₐ[k] K)) = (τ : K →ₐ[k] K) := by
    refine AlgHom.ext_of_adjoin_eq_top hgen fun x hx => ?_
    rw [Set.mem_singleton_iff] at hx
    subst hx
    simpa [hτ] using restrictToBase_divisionFrobenius_rootOfUnity k w hram hm hmv hζ
  exact AlgEquiv.ext fun x => DFunLike.congr_fun h x

variable (k) in
/-- **At a place of the base of residue degree one the Frobenius raises a root of unity of the
number field to the power of the rational prime below the place.**  A place of residue degree one
has as many residues as that prime. -/
theorem restrictToBase_divisionFrobenius_rootOfUnity_prime {p : ℕ} (hp : p.Prime)
    [(primeUnder (𝓞 k) w).asIdeal.LiesOver (Ideal.span {(p : ℤ)})]
    (hdeg : (Ideal.span {(p : ℤ)}).inertiaDeg (primeUnder (𝓞 k) w).asIdeal = 1)
    (hram : ramIdx (𝓞 k) w = 1) {m : ℕ} (hm : m ≠ 0) (hmv : w.valuation K (m : K) = 1)
    {ζ : K} (hζ : ζ ^ m = 1) :
    restrictToBase k w (divisionFrobenius ((primeUnder (𝓞 k) w).adicCompletion k)
        (w.adicCompletion K)
        (fun z hz => exists_divisionNorm_eq_norm_adicCompletion k w hram z hz)) ζ = ζ ^ p := by
  rw [restrictToBase_divisionFrobenius_rootOfUnity k w hram hm hmv hζ,
    natCard_divisionResidue_adicCompletion_eq_prime hp (primeUnder (𝓞 k) w) hdeg]

end PlaceFrobenius

/-! ### The Frobenius of a place over the rationals -/

section RatFrobenius

attribute [local instance] isGalois_adicCompletion

variable {K : Type} [Field K] [NumberField K] [IsGalois ℚ K] (w : HeightOneSpectrum (𝓞 K))

/-- **Over the rationals the Frobenius of an unramified place raises a root of unity of the number
field to the power of the rational prime below the place.**  Every finite place of the rationals has
residue degree one, so the number of residues of the base is the prime itself. -/
theorem restrictToBase_divisionFrobenius_rootOfUnity_rat {q : ℕ} (hq : q.Prime)
    (hqmem : ((q : ℕ) : 𝓞 ℚ) ∈ (primeUnder (𝓞 ℚ) w).asIdeal) (hram : ramIdx (𝓞 ℚ) w = 1) {m : ℕ}
    (hm : m ≠ 0) (hmv : w.valuation K (m : K) = 1) {ζ : K} (hζ : ζ ^ m = 1) :
    restrictToBase ℚ w (divisionFrobenius ((primeUnder (𝓞 ℚ) w).adicCompletion ℚ)
        (w.adicCompletion K)
        (fun z hz => exists_divisionNorm_eq_norm_adicCompletion ℚ w hram z hz)) ζ = ζ ^ q := by
  haveI := liesOver_span_of_natCast_mem hq (primeUnder (𝓞 ℚ) w) hqmem
  exact restrictToBase_divisionFrobenius_rootOfUnity_prime ℚ w hq
    (inertiaDeg_rat_eq_one hq (primeUnder (𝓞 ℚ) w)) hram hm hmv hζ

/-- **Over the rationals the Frobenius of an unramified place of a field generated by a root of
unity is the automorphism raising that root of unity to the power of the rational prime below the
place.**  This is the cyclotomic description of the Frobenius as an element of the Galois group. -/
theorem restrictToBase_divisionFrobenius_eq_of_adjoin_rat {q : ℕ} (hq : q.Prime)
    (hqmem : ((q : ℕ) : 𝓞 ℚ) ∈ (primeUnder (𝓞 ℚ) w).asIdeal) (hram : ramIdx (𝓞 ℚ) w = 1) {m : ℕ}
    (hm : m ≠ 0) (hmv : w.valuation K (m : K) = 1) {ζ : K} (hζ : ζ ^ m = 1)
    (hgen : Algebra.adjoin ℚ ({ζ} : Set K) = ⊤) {τ : Gal(K/ℚ)} (hτ : τ ζ = ζ ^ q) :
    restrictToBase ℚ w (divisionFrobenius ((primeUnder (𝓞 ℚ) w).adicCompletion ℚ)
        (w.adicCompletion K)
        (fun z hz => exists_divisionNorm_eq_norm_adicCompletion ℚ w hram z hz)) = τ := by
  haveI := liesOver_span_of_natCast_mem hq (primeUnder (𝓞 ℚ) w) hqmem
  refine restrictToBase_divisionFrobenius_eq_of_adjoin ℚ w hram hm hmv hζ hgen ?_
  rw [natCard_divisionResidue_adicCompletion_eq_prime hq (primeUnder (𝓞 ℚ) w)
    (inertiaDeg_rat_eq_one hq (primeUnder (𝓞 ℚ) w))]
  exact hτ
