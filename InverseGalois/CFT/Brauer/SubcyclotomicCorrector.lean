...
save at one auxiliary place.  The cyclic algebra with the prime as coefficient, split by the
subfield of that degree of the cyclotomic field of the auxiliary prime, is such a class.
...
non-residue for the prime exponent modulo the auxiliary prime.  The invariant then generates the
whole group of values of that order, so every prescribed value is one of its powers, and raising
the class to the matching power leaves the vanishing of the other invariants untouched.
...
/-- The class of an integer prime to a prime is invertible among the residues modulo any power of
that prime, an integer prime to a prime being coprime to each of its powers. -/
theorem isUnit_intCast_zmod_of_primePow {ℓ e : ℕ} (hℓ : ℓ.Prime) {c : ℤ} (hc : ¬ (ℓ : ℤ) ∣ c) :
    IsUnit ((c : ZMod (ℓ ^ e))) := by
  have hcop : IsCoprime (c : ℤ) ((ℓ ^ e : ℕ) : ℤ) := by
    push_cast
