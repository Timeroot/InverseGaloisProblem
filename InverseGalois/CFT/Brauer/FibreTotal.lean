...
together with the product at the infinite ones.  The archimedean contribution is a single factor
on each side, so all that is needed for the total invariants of a class of a number field and a
class of the rationals to agree is that those two factors agree and that the products over the
finite places agree, and the latter comparison is made fibre by fibre over the rational primes.

Consequently a class of a number field whose invariants above each rational prime multiply to the
invariant there of a class of the rationals, and whose archimedean invariants multiply to the
archimedean invariant of that class, has the same total invariant as it, and in particular has
vanishing total invariant as soon as the rational class does.  This is the shape in which
reciprocity over the rationals carries reciprocity over an arbitrary number field.  The case most
often met is the one where the archimedean invariants vanish separately on both sides.
...
* `InverseGalois.CFT.totalInvariant_eq_of_fibre_arch`: the same comparison asking only that the
  products of the invariants at the infinite places agree, rather than that both vanish.
* `InverseGalois.CFT.totalInvariant_eq_one_of_fibre_arch`: the vanishing statement under that
  weaker archimedean hypothesis.
...
/-- **The total invariant of a Brauer class of a number field is the total invariant of a Brauer
class of the rationals** when the products of the invariants at the infinite places agree on the
two sides and the invariants above each rational prime multiply to the invariant there.  Each
total invariant is the product over the finite places times the archimedean factor, and the two
pairs of factors agree separately. -/
theorem totalInvariant_eq_of_fibre_arch (X : BrauerGroup.{0, 0} k) (Z : BrauerGroup.{0, 0} ℚ)
    (harch : (∏ u : InfinitePlace k, infinitePlaceInvariant k u X)
      = ∏ u : InfinitePlace ℚ, infinitePlaceInvariant ℚ u Z)
    (hfib : ∀ P : HeightOneSpectrum (𝓞 ℚ),
      ∏ᶠ v : HeightOneSpectrum (𝓞 k),
          Set.mulIndicator {u | primeUnder (𝓞 ℚ) u = P} (fun u => placeInvariant k u X) v
        = placeInvariant ℚ P Z) :
    totalInvariant k X = totalInvariant ℚ Z := by
  rw [totalInvariant_apply, totalInvariant_apply, harch,
    finprod_placeInvariant_eq_of_fibre X Z hfib]

/-- **The total invariant of a Brauer class of a number field vanishes** when the products of the
invariants at the infinite places agree with those of a class of the rationals, the invariants
above each rational prime multiply to the invariant there of that class, and the total invariant
of that class vanishes. -/
theorem totalInvariant_eq_one_of_fibre_arch (X : BrauerGroup.{0, 0} k) (Z : BrauerGroup.{0, 0} ℚ)
    (harch : (∏ u : InfinitePlace k, infinitePlaceInvariant k u X)
      = ∏ u : InfinitePlace ℚ, infinitePlaceInvariant ℚ u Z)
    (hfib : ∀ P : HeightOneSpectrum (𝓞 ℚ),
      ∏ᶠ v : HeightOneSpectrum (𝓞 k),
    ...
    (hZ : totalInvariant ℚ Z = 1) :
    totalInvariant k X = 1 := by
  rw [totalInvariant_eq_of_fibre_arch X Z harch hfib, hZ]
