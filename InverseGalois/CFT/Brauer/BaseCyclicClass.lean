...
is generated by a totally real field.  Their total invariant is trivial on the integral units at the
conductor, which is the reciprocity law already known for these particular classes.  And at a finite
...
* `InverseGalois.CFT.exists_cyclicBrauerHom_base`: **the correcting Brauer classes of a number
  field**, with their exponent, their archimedean triviality, their reciprocity on the units at the
  conductor, and their invariants away from it.
...
total invariant on the integral units at the conductor, and have prescribed invariants at the finite
places away from the conductor.  The homomorphism is the cyclic algebra construction for the
compositum of the base with a totally real subfield of the cyclotomic field of the conductor. -/
theorem exists_cyclicBrauerHom_base (k : Type) [Field k] [NumberField k] {q : ℕ} (hq : q.Prime)
  ...
  refine ⟨cyclicBrauerHom (forall_mem_zpowers_restrictNormal (L := ↥(baseCompositum ιF)) hσ),
    fun a => ?_, fun a u => ?_, fun a hab hav => ?_, fun v p hp hpq hmem => ?_⟩
