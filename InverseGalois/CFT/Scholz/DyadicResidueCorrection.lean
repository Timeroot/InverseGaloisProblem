...
* `InverseGalois.CFT.isScholzRealizable_of_solution_two`: **a solution of a central Frattini
  embedding problem with kernel of order two, whose Frobenius defects sum to zero over each block
  of a family accounting for the square roots of products of ramified primes, gives a Scholz
...
ramified primes, gives a Scholz realization at the given level.**  A square root of such a product
lying in the constraint field lies in the solution, because the cyclotomic layer of the constraint
field is ramified only at two while the solution is not; and it lies in the field below, because
the automorphisms fixing that field lie in the Frattini subgroup.  So the blocks account for every
exponent vector obstructing the correction, and summing to zero over each block is orthogonality to
all of them. -/
