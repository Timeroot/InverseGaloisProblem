...
file restates the two forms in which the ramified finite places are discharged — the values of the
cocycle are local powers with exponent the order of a cyclic decomposition group, and the residue
characteristic is congruent to one modulo the product of the two orders — for that base.
...
* `InverseGalois.CFT.exists_isMulCoboundary_of_sq_eq_neg_one_of_forall_exists_pow`: **over a Galois
  extension of the rational numbers containing a square root of minus one, a two-cocycle with
  values in the units of the rational numbers is a coboundary as soon as at every ramified finite
  place its values are local powers with exponent the order of a cyclic decomposition group.**
