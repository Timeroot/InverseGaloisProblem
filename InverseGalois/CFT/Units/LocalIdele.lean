...
form a single orbit of the Galois group, and the group carries the product of the unit groups of the
completions at them to itself.  Transporting all of the factors to the completion at a chosen place
turns that action into a shift of the index carrying with it, once around the orbit, the action of
the decomposition group of the chosen place; and Shapiro's lemma then reads the Herbrand quotient of
the product off the Herbrand quotient of the units of one completion.

The units of a completion have Herbrand quotient the order of the decomposition group, so the whole
local factor has that same Herbrand quotient: the order of the decomposition group at any place
above the chosen place of the base.
...
number field, idele, decomposition group, Herbrand quotient, Shapiro's lemma
-/
...
variable {X : Type*} [Fintype X] [MulAction Gal(K/k) X] {ι : X → HeightOneSpectrum (𝓞 K)}
...
place presents it as the module induced from the decomposition group, and the units of a completion
have Herbrand quotient the order of that group. -/
theorem herbrand_twistShiftAut_adicUnits [Finite Gal(K/k)] [IsPretransitive Gal(K/k) X]
    ...
    herbrand (twistShiftAut
        (smulUnitsAut (G := ↥(stabilizer Gal(K/k) (ι x₀))) (R := (ι x₀).adicCompletion K))
  ...
  exact herbrand_adicUnits_eq_card (ι x₀)
    (mem_zpowers_orbitTurn x₀ hH hgen (smul_eq_of_mem_stabilizer hι hinj x₀)) hturn rfl
