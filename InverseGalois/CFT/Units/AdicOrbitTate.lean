...
/-!
# The complete cohomology of the local factor of the ideles at a place

The places of a Galois extension of number fields lying above a fixed place of the base field form
one orbit of the Galois group, and the units of the completions at them form a family of modules
over that orbit whose sections are the local factor of the group of ideles there.  The sections of a
family over a transitive orbit are coinduced from the module at a base point, and coinduction is
transparent to complete cohomology.
...
family it needs no hypothesis on the Galois group beyond finiteness.  Both the finite places and the
infinite ones are treated, the two arguments differing only in the family they are applied to.
...
* `InverseGalois.CFT.infiniteOrbitTateEquiv`: **the same statement at an infinite place of the base
  field.**
...
/-- **The infinite places above an infinite place of the base field form a single orbit**: some
automorphism carries any one of them to the chosen one. -/
theorem exists_smul_eq_orbit_infinite (y : ω.orbit) : ∃ g : Gal(K/k), g • y = w₀ :=
  MulAction.exists_smul_eq Gal(K/k) y w₀

/-- **The representation of the decomposition group carried by the family of completions above an
infinite place is the action on the units of the completion there.** -/
theorem orbitStabRep_infiniteUnits :
    orbitStabRep w₀ (smul_orbit_of_mem_stabilizer_infinite w₀)
        (orbitFamily (infiniteRingFamily (k := k) (K := K)).unitsFamily ω)
      = repOfAddAut (smulUnitsAut (G := ↥(stabilizer Gal(K/k) (w₀ : InfinitePlace K)))
...
/-- **The complete cohomology of the local factor of the ideles at an infinite place of the base
field is the complete cohomology of the decomposition group with coefficients in the units of the
completion at one place above it.**  The places above the given one form a single orbit, so the
sections of the family of units of the completions at them are coinduced from the module at any one
of them, and coinduction is transparent to complete cohomology. -/
def infiniteOrbitTateEquiv (n : ℤ) :
    tateModule
        (orbitSectionsRep (orbitFamily (infiniteRingFamily (k := k) (K := K)).unitsFamily ω)) n
      ≃ₗ[ℤ] tateModule
        (repOfAddAut (smulUnitsAut (G := ↥(stabilizer Gal(K/k) (w₀ : InfinitePlace K)))
          (R := (w₀ : InfinitePlace K).Completion))) n := by
  rw [← orbitStabRep_infiniteUnits w₀]
  exact orbitTateEquiv w₀ (exists_smul_eq_orbit_infinite w₀)
    (mem_stabilizer_of_smul_orbit_infinite w₀) (smul_orbit_of_mem_stabilizer_infinite w₀) _ n
