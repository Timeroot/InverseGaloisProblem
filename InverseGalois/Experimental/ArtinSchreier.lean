...
theorem artinSchreier_no_roots (a : ZMod p) (ha : a ≠ 0) :
  ...
  simp +contextual [ha, sub_eq_iff_eq_add]
...
lemma artinSchreier_root_shift {K : Type*} [Field K] [Algebra (ZMod p) K]
  ...
  simp_all [← eq_sub_iff_add_eq', map_sub, map_add, map_one, map_pow, sub_eq_zero]
  ...
  simp_all only [one_pow, add_sub_add_right_eq_sub]
...
lemma frobenius_preserves_roots_of_zmod_poly
    ...
    Polynomial.aeval (β ^ p) (g.map (algebraMap (ZMod p) K)) = 0 := by
      -- By definition of polynomial evaluation, we can rewrite the goal using the properties of the Frobenius homomorphism.
      have h_eval : (Polynomial.eval (β ^ p) (Polynomial.map (algebraMap (ZMod p) K) g)) = (Polynomial.eval β (Polynomial.map (algebraMap (ZMod p) K) g)) ^ p := by
        have h_frobenius : ∀ x y : K, (x + y) ^ p = x ^ p + y ^ p := by
          have h_char : ringChar K = p := by
...
lemma natDegree_ge_of_distinct_roots {R : Type*} [CommRing R] [IsDomain R]
    ...
    S.card ≤ g.natDegree := by
      ...
      -- Since $f$ is a polynomial of degree at most $g.natDegree$, and $S$ has more than $g.natDegree$ elements, $f$ must be the zero polynomial.
...
lemma artinSchreier_factor_degree (a : ZMod p) (ha : a ≠ 0)
    ...
    g.natDegree = p := by
      -- By the properties of the minimal polynomial, we know that $g$ divides $X^{p^d} - X$ where $d = \deg(g)$.
      have h_div : g ∣ (X ^ (p ^ (g.natDegree)) - X : Polynomial (ZMod p)) := by
        -- Let $K$ be the finite field extension of $\mathbb{F}_p$ generated by a root of $g$.
        set K := AdjoinRoot g;
...
theorem artinSchreier_irreducible (a : ZMod p) (ha : a ≠ 0) :
    Irreducible (X ^ p - X - C a : Polynomial (ZMod p)) := by
      ...
      -- By the properties of the minimal polynomial, we know that $minpoly (ZMod p) α$ divides $f$.
      have h_min_div : ∀ α : AlgebraicClosure (ZMod p), Polynomial.aeval α f = 0 → minpoly (ZMod p) α = f := by
        intro α hα
        have h_deg : (minpoly (ZMod p) α).natDegree = p := by
          convert artinSchreier_minpoly_degree a ha α hα using 1
        have h_monic : (minpoly (ZMod p) α).Monic := by
          apply minpoly.monic;
        ...
        have h_eq : minpoly (ZMod p) α = f := by
          ...
          subst h_g_one
    simp_all only [ne_eq, aeval_sub, map_pow, aeval_X, aeval_C, natDegree_sub_C, mul_one, natDegree_one, monic_one, f]
        exact h_eq;
