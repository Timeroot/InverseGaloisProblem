  ...
  have h_ker : (Nat.card (MonoidHom.ker (Equiv.Perm.sign.comp (Polynomial.Gal.galActionHom q₄ ℂ))) = 6) := by
    have h_ker : (Nat.card (MonoidHom.range (Equiv.Perm.sign.comp (Polynomial.Gal.galActionHom q₄ ℂ))) = 2) := by
      ...
      · have := conj_support_card_eq_two
        ...
        simp +decide [h, hxy]
      · intro a
        cases' Int.units_eq_one_or (Equiv.Perm.sign (Polynomial.Gal.galActionHom q₄ ℂ a)) with h h <;>
          cases' Int.units_eq_one_or (Equiv.Perm.sign (Polynomial.Gal.galActionHom q₄ ℂ
            (Polynomial.Gal.restrict q₄ ℂ (Complex.conjAe.restrictScalars ℚ)))) with h' h' <;>
          simp +decide only [h, h']
        have := conj_support_card_eq_two
        simp_all +decide [Equiv.Perm.card_support_eq_two]
        obtain ⟨x, y, hxy, h⟩ := this
        simp_all +decide
    have := Subgroup.card_mul_index (MonoidHom.ker (Equiv.Perm.sign.comp (Polynomial.Gal.galActionHom q₄ ℂ)))
