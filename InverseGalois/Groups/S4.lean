...
private lemma q₄_irreducible : Irreducible q₄ := by
  -- We'll use that $X^4 - X - 1$ is irreducible over $\mathbb{Z}$.
  have h_irred_Z : Irreducible (X ^ 4 - X - 1 : Polynomial ℤ) := by
    ...
    have h_irred_int : ∀ p q : Polynomial ℤ, p.degree > 0 → q.degree > 0 → p * q = Polynomial.X ^ 4 - Polynomial.X - 1 → False := by
      ...
      rcases h_deg_cases with (⟨hp_deg, hq_deg⟩ | ⟨hp_deg, hq_deg⟩ | ⟨hp_deg, hq_deg⟩) <;> simp_all +decide only [Polynomial.degree_eq_natDegree];
      · -- If $p$ is a linear polynomial, then $p$ must have a root in $\mathbb{Z}$.
        obtain ⟨a, ha⟩ : ∃ a : ℤ, p.eval a = 0 := by
          rw [ Polynomial.eq_X_add_C_of_degree_eq_one hp_deg ];
          -- Since $p$ is a linear polynomial with integer coefficients, its leading coefficient must be $\pm 1$.
          have h_leading_coeff : p.leadingCoeff = 1 ∨ p.leadingCoeff = -1 := by
            have h_leading_coeff : p.leadingCoeff * q.leadingCoeff = 1 := by
              rw [ ← Polynomial.leadingCoeff_mul, h_eq, Polynomial.leadingCoeff, Polynomial.natDegree_sub_eq_left_of_natDegree_lt ] <;> rw [ Polynomial.natDegree_sub_eq_left_of_natDegree_lt ] <;> norm_num [ Polynomial.coeff_one, Polynomial.coeff_X ];
        ...
        replace h_eq := congr_arg ( Polynomial.eval a ) h_eq ; simp_all +decide [ Polynomial.eval_mul ] ; have := ( show a ≤ 1 by nlinarith [ sq_nonneg ( a^2 - 1 ) ] ) ; have := ( show a ≥ -1 by nlinarith [ sq_nonneg ( a^2 - 1 ) ] ) ; interval_cases a <;> trivial;
        ...
        obtain ⟨r, hr⟩ : ∃ r : ℤ, q.eval r = 0 := by
          ...
          have h_leading_coeff : q.leadingCoeff ∣ 1 := by
            have h_leading_coeff : q.leadingCoeff * p.leadingCoeff = 1 := by
              rw [ mul_comm, ← Polynomial.leadingCoeff_mul, h_eq, Polynomial.leadingCoeff, Polynomial.natDegree_sub_eq_left_of_natDegree_lt ] <;> rw [ Polynomial.natDegree_sub_eq_left_of_natDegree_lt ] <;> norm_num [ Polynomial.coeff_one, Polynomial.coeff_X ];
            exact dvd_of_mul_right_eq _ h_leading_coeff;
          obtain h | h := Int.isUnit_iff.mp ( isUnit_of_dvd_one h_leading_coeff ) <;> use -q.coeff 0 / q.leadingCoeff <;> simp_all +decide [ mul_comm ];
        replace h_eq := congr_arg ( Polynomial.eval r ) h_eq ; simp_all +decide;
        have := ( show r ≤ 1 by nlinarith [ sq_nonneg ( r^2 - 1 ) ] ) ; ( have := ( show r ≥ -1 by nlinarith [ sq_nonneg ( r^2 - 1 ) ] ) ; interval_cases r <;> trivial; );
      · -- Let $p(x) = ax^2 + bx + c$ and $q(x) = dx^2 + ex + f$.
        obtain ⟨a, b, c, d, e, f, hp, hq⟩ : ∃ a b c d e f : ℤ, p = Polynomial.C a * Polynomial.X ^ 2 + Polynomial.C b * Polynomial.X + Polynomial.C c ∧ q = Polynomial.C d * Polynomial.X ^ 2 + Polynomial.C e * Polynomial.X + Polynomial.C f := by
          rw [ @Polynomial.as_sum_range_C_mul_X_pow ℤ _ p, @Polynomial.as_sum_range_C_mul_X_pow ℤ _ q ] ; exact ⟨ p.coeff 2, p.coeff 1, p.coeff 0, q.coeff 2, q.coeff 1, q.coeff 0, by simp +arith +decide [ Polynomial.natDegree_eq_of_degree_eq_some hp_deg, Finset.sum_range_succ' ], by simp +arith +decide [ Polynomial.natDegree_eq_of_degree_eq_some hq_deg, Finset.sum_range_succ' ] ⟩ ;
        ...
        have h_coeff : a * d = 1 ∧ a * e + b * d = 0 ∧ a * f + b * e + c * d = 0 ∧ b * f + c * e = -1 ∧ c * f = -1 := by
          subst_vars; have h₁ := congr_arg ( Polynomial.eval ( -2 ) ) h_eq; have h₂ := congr_arg ( Polynomial.eval ( -1 ) ) h_eq; have h₃ := congr_arg ( Polynomial.eval 0 ) h_eq; have h₄ := congr_arg ( Polynomial.eval 1 ) h_eq; have h₅ := congr_arg ( Polynomial.eval 2 ) h_eq; norm_num at h₁ h₂ h₃ h₄ h₅; exact ⟨ by linarith, by linarith, by linarith, by linarith, by linarith ⟩ ;
        rcases Int.eq_one_or_neg_one_of_mul_eq_one h_coeff.1 with ( rfl | rfl ) <;> rcases Int.eq_one_or_neg_one_of_mul_eq_neg_one h_coeff.2.2.2.2 with ( rfl | rfl ) <;> norm_num at h_coeff ⊢ <;> nlinarith [ show b = 0 by nlinarith ] ;
    constructor <;> contrapose! h_irred_int;
    · exact absurd ( Polynomial.degree_eq_zero_of_isUnit h_irred_int ) ( by erw [ Polynomial.degree_sub_eq_left_of_degree_lt ] <;> erw [ Polynomial.degree_sub_eq_left_of_degree_lt ] <;> norm_num );
    · obtain ⟨ a, b, h₁, h₂, h₃ ⟩ := h_irred_int; use a, b; simp_all +decide [ Polynomial.isUnit_iff ] ;
      constructor <;> refine' lt_of_not_ge fun h => _;
      · rw [ Polynomial.eq_C_of_degree_le_zero h ] at h₁ h₂; replace h₁ := congr_arg ( fun p => Polynomial.coeff p 4 ) h₁; norm_num [ Polynomial.coeff_one, Polynomial.coeff_X, pow_succ' ] at h₁;
        exact h₂ ( a.coeff 0 ) ( isUnit_of_dvd_one <| h₁.symm ▸ dvd_mul_right _ _ ) ( by aesop );
      · rw [ Polynomial.eq_C_of_degree_le_zero h ] at h₃ h₁;
        replace h₁ := congr_arg ( fun p => p.coeff 4 ) h₁ ; norm_num [ Polynomial.coeff_one, Polynomial.coeff_X, Polynomial.coeff_C ] at h₁;
  ...
  have h_gauss : Irreducible (Polynomial.map (Int.castRingHom ℚ) (X ^ 4 - X - 1 : Polynomial ℤ)) := by
    have h_primitive : Polynomial.IsPrimitive (X ^ 4 - X - 1 : Polynomial ℤ) := by
      exact Polynomial.Monic.isPrimitive ( by erw [ Polynomial.Monic, Polynomial.leadingCoeff ] ; erw [ Polynomial.natDegree_sub_C ] ; erw [ Polynomial.natDegree_sub_eq_left_of_natDegree_lt ] <;> norm_num [ Polynomial.coeff_one, Polynomial.coeff_X ] )
...
private lemma q₄_real_roots : Fintype.card (q₄.rootSet ℝ) = 2 := by
  ...
  have h_card : (q₄.map (algebraMap ℚ ℝ)).roots.toFinset = {x₁, x₂} := by
    ...
    · by_cases h_cases : ‹ℝ› < 0;
      · by_contra h_contra;
        exact h_contra <| Or.inl <| by nlinarith [ mul_pos ( sub_pos.mpr h_cases ) ( sub_pos.mpr hx₁.1.1 ), mul_pos ( sub_pos.mpr h_cases ) ( sub_pos.mpr hx₁.1.2 ), mul_pos ( sub_pos.mpr hx₁.1.1 ) ( sub_pos.mpr hx₁.1.2 ), pow_two_nonneg ( ( ‹ℝ› : ℝ ) ^ 2 - x₁ ^ 2 ), pow_two_nonneg ( ( ‹ℝ› : ℝ ) - x₁ ) ] ;
      · exact Or.inr <| by nlinarith [ sq_nonneg ( ( ‹_› : ℝ ) ^ 2 - x₂ ^ 2 ), mul_le_mul_of_nonneg_left ( le_of_not_gt h_cases ) <| sq_nonneg <| ( ‹_› : ℝ ) - x₂ ] ;
    · exact ⟨ by exact ne_of_apply_ne ( Polynomial.eval 0 ) ( by norm_num ), by rcases h with ( rfl | rfl ) <;> linarith ⟩
  generalize_proofs at *; (
  convert congr_arg Finset.card h_card using 1 ; norm_num [ Polynomial.rootSet_def ];
...
private lemma resolvent_irreducible :
    Irreducible (X ^ 3 + C 4 * X - C 1 : ℚ[X]) := by
      -- By the rational root theorem, the only possible rational roots of $X^3 + 4X - 1$ are $\pm 1$.
      have h_no_rational_roots : ¬∃ r : ℚ, Polynomial.eval r (Polynomial.X ^ 3 + Polynomial.C 4 * Polynomial.X - Polynomial.C 1) = 0 := by
        ...
        rw [ ← @Rat.num_div_den r ] at hr; norm_num [ h_div ] at hr; norm_cast at hr; have := ( show r.num ≤ 1 by nlinarith [ sq_nonneg ( r.num^2 ) ] ) ; have := ( show r.num ≥ -1 by nlinarith [ sq_nonneg ( r.num^2 ) ] ) ; interval_cases r.num <;> trivial;
      -- Since $X^3 + 4X - 1$ is a cubic polynomial with no rational roots, it is irreducible over $\mathbb{Q}$.
      have h_irred : ∀ p q : Polynomial ℚ, p.degree > 0 → q.degree > 0 → Polynomial.X ^ 3 + Polynomial.C 4 * Polynomial.X - Polynomial.C 1 = p * q → False := by
        intros p q hp hq h_factor
        have h_deg : p.degree + q.degree = 3 := by
          rw [ ← Polynomial.degree_mul, ← h_factor, Polynomial.degree_sub_C ] <;> erw [ Polynomial.degree_add_eq_left_of_degree_lt ] <;> norm_num;
        ...
        · exact h_no_rational_roots <| by obtain ⟨ r, hr ⟩ := Polynomial.exists_root_of_degree_eq_one h_deg_p; exact ⟨ r, by aesop ⟩ ;
        · exact h_no_rational_roots <| by obtain ⟨ r, hr ⟩ := Polynomial.exists_root_of_degree_eq_one h_deg_q; exact ⟨ r, by aesop ⟩ ;
      constructor;
      · exact fun h => absurd ( Polynomial.degree_eq_zero_of_isUnit h ) ( by erw [ Polynomial.degree_sub_C ] <;> erw [ Polynomial.degree_add_eq_left_of_degree_lt ] <;> norm_num );
      · contrapose! h_irred;
...
private lemma resolvent_identity {K : Type*} [Field K] (t w v : K)
  ...
  have step2 : (v - w) ^ 2 = t ^ 4 + 4 := by
    ...
    rw [← h1, h2] at this; linear_combination this
  rw [step2] at step1
...
private lemma resolvent_root_from_cubic_root (α γ : q₄.SplittingField)
  ...
  have h1 : t ^ 2 = w + v := by simp only [ht_def, hw_def, hv_def]; ring
  have h2 : w * v = -1 := by
    simp only [hw_def, hv_def]
    linear_combination α * hγ + (1 - hα)
  have h3 : t * (v - w) = 1 := by
    simp only [ht_def, hv_def, hw_def]; linear_combination hγ
    ...
    ¬ ∃ (γ : IntermediateField.adjoin ℚ {α}),
      γ.val ^ 3 + α * γ.val ^ 2 + α ^ 2 * γ.val + (α ^ 3 - 1) = 0 := by
        ...
        have h_resolvent : ((α + γ.val) ^ 2) ^ 3 + 4 * (α + γ.val) ^ 2 - 1 = 0 := by
          ...
          · unfold q₄ at hα; norm_num at hα; linear_combination' hα;
          · exact hγ;
        -- Since $y = (α + γ)^2$ is in $\mathbb{Q}(α)$, we have $\mathbb{Q}(y) \subseteq \mathbb{Q}(α)$.
        ...
        have h_deg_y : Module.finrank ℚ (↥(IntermediateField.adjoin ℚ {((α + γ.val) ^ 2)})) = 3 := by
          ...
          · have h_minpoly : minpoly ℚ ((α + γ.val) ^ 2) = Polynomial.X ^ 3 + Polynomial.C 4 * Polynomial.X - Polynomial.C 1 := by
              refine' Eq.symm ( minpoly.eq_of_irreducible_of_monic _ _ _ );
              · convert resolvent_irreducible using 1;
              · aesop;
              · erw [ Polynomial.Monic, Polynomial.leadingCoeff, Polynomial.natDegree_sub_C, Polynomial.natDegree_add_eq_left_of_natDegree_lt ] <;> norm_num;
        ...
        have h_deg_α : Module.finrank ℚ (↥(IntermediateField.adjoin ℚ {α})) = 4 := by
          ...
          have h_deg_α : minpoly ℚ α = q₄ := by
            ...
            · convert q₄_irreducible using 1;
            · exact hα;
            · erw [ Polynomial.Monic, Polynomial.leadingCoeff, Polynomial.natDegree_sub_C, Polynomial.natDegree_sub_eq_left_of_natDegree_lt ] <;> norm_num [ Polynomial.coeff_one, Polynomial.coeff_X ]
          have h_deg_α : Module.finrank ℚ (↥(IntermediateField.adjoin ℚ {α})) = Polynomial.natDegree (minpoly ℚ α) := by
            rw [ IntermediateField.adjoin.finrank ] ; aesop;
          rw [h_deg_α, h_deg_α] at *; simp_all +decide [ q₄_natDegree ] ;
        have h_div : Module.finrank ℚ (↥(IntermediateField.adjoin ℚ {((α + γ.val) ^ 2)})) ∣ Module.finrank ℚ (↥(IntermediateField.adjoin ℚ {α})) := by
          exact finrank_dvd_of_le_right h_sub;
        simp_all +decide only [Nat.dvd_prime]
...
private lemma three_dvd_card_gal : 3 ∣ Nat.card q₄.Gal := by
  ...
  obtain ⟨α, hα⟩ : ∃ α : q₄.SplittingField, Polynomial.aeval α q₄ = 0 := by
    ...
    · erw [ Polynomial.degree_map, Polynomial.degree_sub_C ] <;> erw [ Polynomial.degree_sub_eq_left_of_degree_lt ] <;> norm_num
  have h_deg : 4 ∣ (Module.finrank ℚ (IntermediateField.adjoin ℚ {α})) := by
    have h_deg : minpoly ℚ α = Polynomial.C (1 / 1) * q₄ := by
      ...
      · unfold q₄; erw [ Polynomial.Monic, Polynomial.leadingCoeff ] ; erw [ Polynomial.natDegree_sub_C ] ; erw [ Polynomial.natDegree_sub_eq_left_of_natDegree_lt ] <;> norm_num [ Polynomial.coeff_one, Polynomial.coeff_X ] ;
    ...
    obtain ⟨β, hβ⟩ : ∃ β : q₄.SplittingField, β ^ 3 + α * β ^ 2 + α ^ 2 * β + (α ^ 3 - 1) = 0 := by
      obtain ⟨β, hβ⟩ : ∃ β : q₄.SplittingField, β ≠ α ∧ Polynomial.aeval β q₄ = 0 := by
        ...
        have h_linear : q₄.map (algebraMap ℚ q₄.SplittingField) = (Polynomial.X - Polynomial.C α) ^ 4 := by
          have h_linear : q₄.map (algebraMap ℚ q₄.SplittingField) = Polynomial.C (1 : q₄.SplittingField) * Multiset.prod (Multiset.map (fun β => Polynomial.X - Polynomial.C β) (Polynomial.roots (q₄.map (algebraMap ℚ q₄.SplittingField)))) := by
            convert Polynomial.Splits.eq_prod_roots _;
            · unfold q₄; erw [ Polynomial.leadingCoeff_map_of_leadingCoeff_ne_zero ] <;> norm_num;
              · erw [ Polynomial.leadingCoeff, Polynomial.natDegree_sub_C, Polynomial.natDegree_sub_eq_left_of_natDegree_lt ] <;> norm_num [ Polynomial.coeff_one, Polynomial.coeff_X ];
              · exact ne_of_apply_ne ( Polynomial.eval 0 ) ( by norm_num );
            · exact Polynomial.SplittingField.splits _;
          rw [ h_linear ];
          rw [ show Polynomial.roots ( Polynomial.map ( algebraMap ℚ q₄.SplittingField ) q₄ ) = Multiset.replicate 4 α from ?_ ] ; norm_num [ ← pow_mul ];
          · ring;
          · refine' Multiset.eq_replicate.mpr ⟨ _, _ ⟩;
            · replace h_linear := congr_arg Polynomial.natDegree h_linear ; norm_num [ Polynomial.natDegree_map ] at h_linear ⊢;
              exact h_linear ▸ q₄_natDegree;
            · exact fun x hx => Classical.not_not.1 fun hx' => h_contra x hx' <| by simpa [ Polynomial.eval_map ] using Polynomial.isRoot_of_mem_roots hx;
        simp_all +decide [ q₄ ];
        have := congr_arg ( Polynomial.derivative ) h_linear; norm_num at this;
        replace this := congr_arg ( Polynomial.eval 0 ) this ; norm_num [ Polynomial.derivative_pow ] at this;
        grind;
      use β;
      unfold q₄ at *; norm_num at *; exact mul_left_cancel₀ ( sub_ne_zero_of_ne hβ.1 ) <| by linear_combination hβ.2 - hα;
    -- Since $g$ is irreducible over $\mathbb{Q}(\alpha)$, the minimal polynomial of $\beta$ over $\mathbb{Q}(\alpha)$ has degree 3.
    have h_min_poly_deg : Polynomial.natDegree (minpoly (IntermediateField.adjoin ℚ {α}) β) = 3 := by
      have h_deg2 : minpoly (IntermediateField.adjoin ℚ {α}) β = Polynomial.X ^ 3 + Polynomial.C (⟨α, IntermediateField.subset_adjoin ℚ {α} (Set.mem_singleton α)⟩ : IntermediateField.adjoin ℚ {α}) * Polynomial.X ^ 2 + Polynomial.C (⟨α ^ 2, by
        exact Subalgebra.pow_mem _ ( IntermediateField.mem_adjoin_simple_self ℚ α ) 2⟩ : IntermediateField.adjoin ℚ {α}) * Polynomial.X + Polynomial.C (⟨α ^ 3 - 1, by
        exact Subalgebra.sub_mem _ ( Subalgebra.pow_mem _ ( IntermediateField.mem_adjoin_simple_self ℚ α ) _ ) ( Subalgebra.one_mem _ )⟩ : IntermediateField.adjoin ℚ {α}) := by
        refine' Eq.symm ( minpoly.eq_of_irreducible_of_monic _ _ _ );
        · have h_irred : ∀ (p : Polynomial (IntermediateField.adjoin ℚ {α})), p.degree = 3 → (∀ (γ : IntermediateField.adjoin ℚ {α}), p.eval γ ≠ 0) → Irreducible p := by
            intros p hp_deg hp_no_roots
            have h_irred : ∀ (f g : Polynomial (IntermediateField.adjoin ℚ {α})), f.degree > 0 → g.degree > 0 → p = f * g → False := by
              intros f g hf hg hp_eq
              have h_deg_f : f.degree = 1 ∨ g.degree = 1 := by
                have := congr_arg Polynomial.degree hp_eq; norm_num [ hp_deg ] at this; rw [ Polynomial.degree_eq_natDegree ( by aesop_cat ), Polynomial.degree_eq_natDegree ( by aesop_cat ) ] at *; norm_cast at *; omega;
              rcases h_deg_f with ( h | h ) <;> obtain ⟨ x, hx ⟩ := Polynomial.exists_root_of_degree_eq_one h <;> simp_all +decide;
            constructor <;> contrapose! h_irred;
            · exact absurd ( Polynomial.degree_eq_zero_of_isUnit h_irred ) ( by aesop );
            · obtain ⟨ a, b, rfl, ha, hb ⟩ := h_irred; exact ⟨ a, b, not_le.mp fun ha' => ha <| Polynomial.isUnit_iff_degree_eq_zero.mpr <| le_antisymm ha' <| le_of_not_gt fun ha'' => by aesop, not_le.mp fun hb' => hb <| Polynomial.isUnit_iff_degree_eq_zero.mpr <| le_antisymm hb' <| le_of_not_gt fun hb'' => by aesop, rfl, trivial ⟩ ;
          apply h_irred;
          ...
          · intro γ hγ
            have h_contra : γ.val ^ 3 + α * γ.val ^ 2 + α ^ 2 * γ.val + (α ^ 3 - 1) = 0 := by
              convert congr_arg Subtype.val hγ using 1 ; norm_num;
            exact cubic_factor_irreducible α hα ⟨ γ, h_contra ⟩;
        · convert hβ using 1;
          simp +decide [ Polynomial.aeval_def ];
        · erw [ Polynomial.Monic, Polynomial.leadingCoeff, Polynomial.natDegree_add_C, Polynomial.natDegree_add_eq_left_of_natDegree_lt ] <;> erw [ Polynomial.natDegree_add_eq_left_of_natDegree_lt ] <;> by_cases h : α = 0 <;> simp +decide [ h ];
          all_goals simp_all +decide [ Subtype.ext_iff ]; all_goals unfold q₄ at hα; norm_num at hα;
      rw [ h_deg2, Polynomial.natDegree_add_C, Polynomial.natDegree_add_eq_left_of_natDegree_lt ] <;> rw [ Polynomial.natDegree_add_eq_left_of_natDegree_lt ] <;> norm_num;
      · by_cases h : ( C ⟨ α, IntermediateField.subset_adjoin ℚ { α } ( Set.mem_singleton α ) ⟩ : Polynomial ( IntermediateField.adjoin ℚ { α } ) ) = 0 <;> simp +decide [ h ];
      · exact lt_of_le_of_lt ( Polynomial.natDegree_C_mul_le _ _ ) ( by norm_num );
      · by_cases h : ( C ⟨ α, IntermediateField.subset_adjoin ℚ { α } ( Set.mem_singleton α ) ⟩ : Polynomial ( IntermediateField.adjoin ℚ { α } ) ) = 0 <;> simp +decide [ h ];
    have := Module.finrank_mul_finrank ( IntermediateField.adjoin ℚ { α } ) ( IntermediateField.adjoin ( IntermediateField.adjoin ℚ { α } ) { β } ) q₄.SplittingField;
    have h_min_poly_deg : Module.finrank (IntermediateField.adjoin ℚ {α}) (IntermediateField.adjoin (IntermediateField.adjoin ℚ {α}) {β}) = 3 := by
  ...
  have h_deg_beta : 3 ∣ (Module.finrank ℚ q₄.SplittingField) := by
    exact dvd_trans h_deg_beta ( by simpa using Module.finrank_mul_finrank ℚ ( IntermediateField.adjoin ℚ { α } ) q₄.SplittingField ▸ dvd_mul_left _ _ );
  convert h_deg_beta using 1;
  convert Polynomial.Gal.card_of_separable ( q₄_irreducible.separable )
...
private lemma conj_support_card_eq_two :
    (Polynomial.Gal.galActionHom q₄ ℂ
      (Polynomial.Gal.restrict q₄ ℂ (Complex.conjAe.restrictScalars ℚ))).support.card = 2 := by
  have key := Polynomial.Gal.card_complex_roots_eq_card_real_add_card_not_gal_inv q₄
  ...
  have hℝ : (q₄.rootSet ℝ).toFinset.card = 2 := by
    rw [Set.toFinset_card]; exact q₄_real_roots
  omega
...
private lemma card_gal_ne_12 : Nat.card q₄.Gal ≠ 12 := by
  intro h_card
  have h_ker : (Nat.card (MonoidHom.ker (Equiv.Perm.sign.comp (Polynomial.Gal.galActionHom q₄ ℂ))) = 6) := by
    have h_ker : (Nat.card (MonoidHom.range (Equiv.Perm.sign.comp (Polynomial.Gal.galActionHom q₄ ℂ))) = 2) := by
      rw [ Nat.card_eq_two_iff ];
      simp +decide [ Set.ext_iff ];
      refine' ⟨ 1, Polynomial.Gal.restrict q₄ ℂ ( Complex.conjAe.restrictScalars ℚ ), _, _ ⟩ <;> norm_num;
      · have := conj_support_card_eq_two;
        rw [ Equiv.Perm.card_support_eq_two ] at this;
        obtain ⟨ x, y, hxy, h ⟩ := this; simp +decide [ h, hxy ] ;
      · intro a; cases' Int.units_eq_one_or ( Equiv.Perm.sign ( Polynomial.Gal.galActionHom q₄ ℂ a ) ) with h h <;> cases' Int.units_eq_one_or ( Equiv.Perm.sign ( Polynomial.Gal.galActionHom q₄ ℂ ( Polynomial.Gal.restrict q₄ ℂ ( Complex.conjAe.restrictScalars ℚ ) ) ) ) with h' h' <;> simp +decide only [h, h'] ;
        have := conj_support_card_eq_two; simp_all +decide [ Equiv.Perm.card_support_eq_two ] ;
        obtain ⟨ x, y, hxy, h ⟩ := this; simp_all +decide [ Equiv.Perm.sign_swap ] ;
    have := Subgroup.card_mul_index ( MonoidHom.ker ( Equiv.Perm.sign.comp ( Polynomial.Gal.galActionHom q₄ ℂ ) ) ) ; simp_all +decide [ Nat.mul_comm ] ;
    rw [ Subgroup.index_ker ] at this ; simp_all +decide [ Fintype.card_subtype ] ;
    linarith;
  obtain ⟨S, hS⟩ : ∃ S : Finset (Equiv.Perm (q₄.rootSet ℂ)), S.card = 6 ∧ (∀ x ∈ S, Equiv.Perm.sign x = 1) ∧ (1 ∈ S) ∧ (∀ a ∈ S, ∀ b ∈ S, a * b ∈ S) := by
    refine' ⟨ Finset.image ( fun x : ↥ ( MonoidHom.ker ( Equiv.Perm.sign.comp ( Gal.galActionHom q₄ ℂ ) ) ) => ( Gal.galActionHom q₄ ℂ ) x ) ( Finset.univ ), _, _, _, _ ⟩ <;> simp_all +decide [ Finset.card_image_of_injective, Function.Injective ];
    · rw [ Finset.card_image_of_injective _ fun x y hxy => by simpa using Gal.galActionHom_injective q₄ ℂ hxy, Finset.card_univ ] ; aesop;
  ...
  refine' no_order_six_subgroup_A4 ( Finset.image ( fun x : Equiv.Perm ( q₄.rootSet ℂ ) => Equiv.permCongr e x ) S ) _ _ _ _ <;> simp_all +decide [ Finset.card_image_of_injective, Function.Injective ];
  · exact ⟨ 1, hS.2.2.1, by aesop ⟩;
  · exact fun a ha b hb => ⟨ a * b, hS.2.2.2 a ha b hb, by simp +decide [ Equiv.Perm.ext_iff ] ⟩
...
theorem IsInverseGalois.perm_fin_four : IsInverseGalois (Equiv.Perm (Fin 4)) := by
  ...
  · have h_galois : Nat.card q₄.Gal = 24 := by
      convert card_gal_eq_24;
    refine' ⟨ _ ⟩;
    ...
    have h_iso : Function.Bijective (h_iso.choose : Gal(q₄.SplittingField/ℚ) → Equiv.Perm (Fin 4)) := by
      have h_iso : Fintype.card (Gal(q₄.SplittingField/ℚ)) = Fintype.card (Equiv.Perm (Fin 4)) := by
        ...
        exact h_galois;
      exact ⟨ ‹∃ f : Gal(q₄.SplittingField/ℚ) →* Equiv.Perm (Fin 4), Function.Injective f›.choose_spec, Fintype.bijective_iff_injective_and_card _ |>.2 ⟨ ‹∃ f : Gal(q₄.SplittingField/ℚ) →* Equiv.Perm (Fin 4), Function.Injective f›.choose_spec, h_iso ⟩ |>.2 ⟩;
