...
private lemma a₄_irreducible : Irreducible a₄ := by
  ...
  have h_coeff : ∃ a b d : ℚ, b + d - a^2 = 0 ∧ a * (d - b) = 8 ∧ b * d = 12 := by
    ...
    obtain ⟨a, b, c, d, e, f', ha⟩ :
      ...
      · simp +arith +decide [Polynomial.natDegree_eq_of_degree_eq_some hf_deg, Finset.sum_range_succ']
      · simp +arith +decide [Polynomial.natDegree_eq_of_degree_eq_some hg_deg, Finset.sum_range_succ'] 
    generalize_proofs at *
    (
    ...
    have h_iso_subgroup : ∃ (f : a₄.Gal →* Equiv.Perm (a₄.rootSet ℂ)), Function.Injective f := by
      have h_iso_subgroup : ∃ (f : a₄.Gal →* Equiv.Perm (a₄.rootSet a₄.SplittingField)), Function.Injective f := by
        ...
        · rw [Polynomial.mem_rootSet] at *
          refine ⟨?_, ?_⟩
          · exact ne_of_apply_ne (Polynomial.eval 0) (by norm_num [a₄])
          · simpa [Polynomial.aeval_def, Polynomial.eval₂_eq_sum_range] using
              congr_arg (fun p => σ p) (Polynomial.mem_rootSet.mp x.2 |>.2)
        · intro x hx
          use σ.symm x
