...
private lemma f_a5_Z_monic : f_a5_Z.Monic := by
  erw [ Polynomial.Monic, Polynomial.leadingCoeff, Polynomial.natDegree_add_C, Polynomial.natDegree_add_eq_left_of_natDegree_lt ] <;> norm_num [ f_a5_Z ]
...
private lemma f_a5_Z_irreducible : Irreducible f_a5_Z := by
  have h_irred_mod3 : Irreducible (Polynomial.X^5 + 2 * Polynomial.X + 1 : Polynomial (ZMod 3)) := by
    have h_no_factorization : ∀ p q : Polynomial (ZMod 3), p.degree = 2 → q.degree = 3 → p * q ≠ Polynomial.X ^ 5 + 2 * Polynomial.X + 1 := by
      intros p q hp hq h_eq
      have h_coeff : p.coeff 2 * q.coeff 3 = 1 ∧ p.coeff 2 * q.coeff 2 + p.coeff 1 * q.coeff 3 = 0 ∧ p.coeff 2 * q.coeff 1 + p.coeff 1 * q.coeff 2 + p.coeff 0 * q.coeff 3 = 0 ∧ p.coeff 2 * q.coeff 0 + p.coeff 1 * q.coeff 1 + p.coeff 0 * q.coeff 2 = 0 ∧ p.coeff 1 * q.coeff 0 + p.coeff 0 * q.coeff 1 = 2 ∧ p.coeff 0 * q.coeff 0 = 1 := by
        rw [ Polynomial.as_sum_range_C_mul_X_pow p, Polynomial.as_sum_range_C_mul_X_pow q ] at h_eq; norm_num [ Finset.sum_range_succ', Polynomial.natDegree_eq_of_degree_eq_some hp, Polynomial.natDegree_eq_of_degree_eq_some hq ] at h_eq; simp_all +decide [ Polynomial.ext_iff ] ;
        have H0 := h_eq 0; have H1 := h_eq 1; have H2 := h_eq 2; have H3 := h_eq 3; have H4 := h_eq 4; have H5 := h_eq 5; simp_all +decide [ Polynomial.coeff_one, Polynomial.coeff_X, mul_assoc, add_mul, pow_succ ] ;
      have h_coeff_cases : ∀ a b c d e f g : ZMod 3, a * g = 1 → a * f + b * g = 0 → a * e + b * f + c * g = 0 → a * d + b * e + c * f = 0 → b * d + c * e = 2 → c * d = 1 → False := by
        native_decide +revert;
      exact h_coeff_cases _ _ _ _ _ _ _ h_coeff.1 h_coeff.2.1 h_coeff.2.2.1 h_coeff.2.2.2.1 h_coeff.2.2.2.2.1 h_coeff.2.2.2.2.2;
    constructor;
    · exact fun h => absurd ( Polynomial.degree_eq_zero_of_isUnit h ) ( by erw [ Polynomial.degree_add_C ] <;> repeat ( first | erw [ Polynomial.degree_add_eq_left_of_degree_lt ] | erw [ Polynomial.degree_C ] | simp +decide ) );
    · intro p q hpq
      by_contra h_contra
      push_neg at h_contra
      have h_deg : p.degree + q.degree = 5 := by
        rw [ ← Polynomial.degree_mul, ← hpq, Polynomial.degree_add_eq_left_of_degree_lt ] <;> erw [ Polynomial.degree_add_eq_left_of_degree_lt ] <;> norm_num;
      ...
      have h_deg_cases : p.degree = 1 ∧ q.degree = 4 ∨ p.degree = 4 ∧ q.degree = 1 ∨ p.degree = 2 ∧ q.degree = 3 ∨ p.degree = 3 ∧ q.degree = 2 := by
        ...
        rw [ Polynomial.degree_eq_natDegree ( show p ≠ 0 from fun h => by subst h; exact absurd hpq <| by exact ne_of_apply_ne ( Polynomial.eval 0 ) <| by simp +decide ), Polynomial.degree_eq_natDegree ( show q ≠ 0 from fun h => by subst h; exact absurd hpq <| by exact ne_of_apply_ne ( Polynomial.eval 0 ) <| by simp +decide ) ] at * ; norm_cast at * ; omega
      rcases h_deg_cases with (⟨hp, hq⟩ | ⟨hp, hq⟩ | ⟨hp, hq⟩ | ⟨hp, hq⟩) <;> simp_all +decide [ Polynomial.isUnit_iff_degree_eq_zero ];
      · obtain ⟨a, ha⟩ : ∃ a : ZMod 3, p.eval a = 0 := by
          exact Polynomial.exists_root_of_degree_eq_one hp;
        replace hpq := congr_arg ( Polynomial.eval a ) hpq ; simp_all +decide;
        fin_cases a <;> contradiction;
      · obtain ⟨r, hr⟩ : ∃ r : ZMod 3, q.eval r = 0 := by
          exact Polynomial.exists_root_of_degree_eq_one hq;
        replace hpq := congr_arg ( Polynomial.eval r ) hpq ; simp_all +decide;
  ...
  · unfold f_a5_Z; norm_num [ Polynomial.ext_iff ] ;
    erw [ Polynomial.Monic, Polynomial.leadingCoeff, Polynomial.natDegree_add_C, Polynomial.natDegree_add_eq_left_of_natDegree_lt ] <;> norm_num [ Polynomial.coeff_X ];
...
private lemma five_dvd_card_gal : 5 ∣ Nat.card f_a5.Gal := by
  have := f_a5_natDegree ▸ natDegree_dvd_card f_a5_irreducible
  exact this
...
private lemma three_dvd_card_gal : 3 ∣ Nat.card f_a5.Gal := by
  ...
  have h3_div_order : 3 ∣ orderOf (Polynomial.Gal.galActionHom (Polynomial.map (Int.castRingHom ℚ) f_a5_Z) ℂ σ) := by
    have h3_div_order : 3 ∈ (Polynomial.Gal.galActionHom (Polynomial.map (Int.castRingHom ℚ) f_a5_Z) ℂ σ).cycleType := by
...
private lemma card_gal_dvd_60 : Nat.card f_a5.Gal ∣ 60 := by
  ...
  have := Nat.le_of_dvd ( by decide ) ‹Nat.card f_a5.Gal ∣ Nat.factorial 5›; interval_cases _ : Nat.card f_a5.Gal <;> simp_all +decide only ;
...
private lemma A5_no_subgroup_order_30 :
  ...
  have h_card_A5 : Nat.card (alternatingGroup (Fin 5)) = 60 := by
    rw [Nat.card_eq_fintype_card, card_alternatingGroup]; norm_num
  have h_index : H.index = 2 := by
    have := H.index_mul_card; rw [hH, h_card_A5] at this; omega
  have h_normal := Subgroup.normal_of_index_eq_two h_index
  rcases IsSimpleGroup.eq_bot_or_eq_top_of_normal H h_normal with h | h
  · simp [h] at hH
  · rw [h, Subgroup.card_top, h_card_A5] at hH; omega
...
private lemma sixty_dvd_card_gal : 60 ∣ Nat.card f_a5.Gal := by
  ...
  have h_div : Nat.card f_a5.Gal ∈ ({15, 30, 60} : Set ℕ) := by
    ...
    have := Nat.le_of_dvd ( by decide ) h_div.2; interval_cases Nat.card f_a5.Gal <;> simp +decide at h_div ⊢;
  ...
  have h_inj : ∃ H : Subgroup (Equiv.Perm (f_a5.rootSet ℂ)), Nat.card H = Nat.card f_a5.Gal := by
    ...
    obtain ⟨ f, hf ⟩ := h_inj; use f.range; simp +decide [ hf, Nat.card_eq_fintype_card ] ;
  ...
  obtain ⟨ H', hH' ⟩ := h_iso_perm; have := Perm_Fin5_no_subgroup_order_15 H'; have := Perm_Fin5_no_subgroup_order_30 H'; aesop;
...
private lemma gal_iso_alt5 :
    Nonempty (f_a5.Gal ≃* (alternatingGroup (Fin 5))) := by
      have h_subgroup : ∃ (f : f_a5.Gal →* Equiv.Perm (f_a5.rootSet ℂ)), Function.Injective f ∧ (Nat.card (MonoidHom.range f)) = 60 := by
        obtain ⟨f, hf⟩ : ∃ f : f_a5.Gal →* Equiv.Perm (f_a5.rootSet (f_a5.SplittingField)), Function.Injective f := by
          ...
          · rw [ Polynomial.mem_rootSet ] at *;
            have := x.2; rw [ Polynomial.mem_rootSet ] at this;
            rw [ aeval_def, Polynomial.eval₂_eq_sum_range ] at *;
            exact ⟨ this.1, by simpa [ map_sum, map_mul, map_pow ] using congr_arg ( σ : f_a5.SplittingField → f_a5.SplittingField ) this.2 ⟩;
          · intro a ha;
          ...
          · aesop;
          · aesop;
          · intro a₁ a₂ h; ext x; by_cases hx : x ∈ f_a5.rootSet f_a5.SplittingField <;> aesop;
      ...
      have h_image : MonoidHom.range f = alternatingGroup (f_a5.rootSet ℂ) := by
        have h_unique : ∀ (H : Subgroup (Equiv.Perm (f_a5.rootSet ℂ))), Nat.card H = 60 → H = alternatingGroup (f_a5.rootSet ℂ) := by
          intros H hH_card
          have h_index : H.index = 2 := by
