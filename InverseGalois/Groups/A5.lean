  ...
  have h_irred_mod3 : Irreducible (Polynomial.X^5 + 2 * Polynomial.X + 1 : Polynomial (ZMod 3)) := by
    have h_no_factorization : ∀ p q : Polynomial (ZMod 3),
      ...
      have h_coeff : p.coeff 2 * q.coeff 3 = 1 ∧
        ...
        have H4 := h_eq 4
        have H5 := h_eq 5
        simp_all [Polynomial.coeff_one, Polynomial.coeff_X, mul_assoc, add_mul, pow_succ]
    ...
    · intro p q hpq
      ...
      have h_deg_cases : p.degree = 1 ∧ q.degree = 4 ∨ p.degree = 4 ∧ q.degree = 1 ∨
        ...
        have hp0 : p ≠ 0 := by
          rintro rfl
          refine absurd hpq (ne_of_apply_ne (Polynomial.eval 0) ?_)
          simp
        have hq0 : q ≠ 0 := by
          rintro rfl
          refine absurd hpq (ne_of_apply_ne (Polynomial.eval 0) ?_)
      ...
      · obtain ⟨a, ha⟩ : ∃ a : ZMod 3, p.eval a = 0 :=
          Polynomial.exists_root_of_degree_eq_one hp
        replace hpq := congr_arg (Polynomial.eval a) hpq
        simp_all
        fin_cases a <;> contradiction
      · obtain ⟨r, hr⟩ : ∃ r : ZMod 3, q.eval r = 0 :=
          Polynomial.exists_root_of_degree_eq_one hq
        replace hpq := congr_arg (Polynomial.eval r) hpq
        simp_all
        fin_cases r <;> contradiction
      · exact h_no_factorization p q hp hq rfl
      · refine h_no_factorization q p hq hp ?_
        rw [mul_comm]
...
private lemma three_dvd_card_gal : 3 ∣ Nat.card f_a5.Gal := by
  have hirr : Irreducible (f_a5_Z.map (Int.castRingHom ℚ)) := by
    convert f_a5_irreducible using 1
    exact f_a5_eq_map.symm
  obtain ⟨σ, hσ⟩ := dedekind_theorem f_a5_Z f_a5_Z_monic hirr 7 f_a5_mod7_squarefree
  have : Fact (((f_a5_Z.map (Int.castRingHom ℚ)).map (algebraMap ℚ ℂ)).Splits) :=
    ⟨IsAlgClosed.splits _⟩
  have h3_div_order : 3 ∣ orderOf (Gal.galActionHom (Polynomial.map (Int.castRingHom ℚ) f_a5_Z) ℂ σ) := by
    rw [← Equiv.Perm.lcm_cycleType, hσ]
    exact Multiset.dvd_lcm f_a5_mod7_factorizationType
  convert ((h3_div_order.trans (orderOf_map_dvd _ _)).trans (orderOf_dvd_natCard σ)) using 1
...
private lemma card_gal_dvd_60 : Nat.card f_a5.Gal ∣ 60 := by
  ...
  have h_dvd_120 : Nat.card f_a5.Gal ∣ Nat.factorial 5 := by
...
private lemma sixty_dvd_card_gal : 60 ∣ Nat.card f_a5.Gal := by
  ...
  have h_div : Nat.card f_a5.Gal ∈ ({15, 30, 60} : Set ℕ) := by
    have h_bounds : 15 ∣ Nat.card f_a5.Gal ∧ Nat.card f_a5.Gal ∣ 60 :=
      ⟨Nat.lcm_dvd three_dvd_card_gal five_dvd_card_gal, card_gal_dvd_60⟩
    have := Nat.le_of_dvd (by decide) h_bounds.2
    interval_cases Nat.card f_a5.Gal <;> simp at h_bounds ⊢
  ...
  have h_inj : ∃ H : Subgroup (Equiv.Perm (f_a5.rootSet ℂ)), Nat.card H = Nat.card f_a5.Gal := by
    have h_inj_f : ∃ f : f_a5.Gal →* Equiv.Perm (f_a5.rootSet ℂ), Function.Injective f := by
  ...
  -- Transfer to Perm(Fin 5) via Equiv.permCongr with an equivalence rootSet ℂ ≃ Fin 5.
  have e : f_a5.rootSet ℂ ≃ Fin 5 := by
...
private lemma gal_iso_alt5 :
    Nonempty (f_a5.Gal ≃* (alternatingGroup (Fin 5))) := by
      have h_subgroup : ∃ (f : f_a5.Gal →* Equiv.Perm (f_a5.rootSet ℂ)),
        ...
        have h_gal60 : Nat.card f_a5.Gal = 60 := card_gal_a5
        have h_rangecard : Nat.card (MonoidHom.range f) = Nat.card f_a5.Gal :=
          (Nat.card_congr (Equiv.ofInjective _ hf)).symm
        have h_permiso : Nonempty (Equiv.Perm (f_a5.rootSet (f_a5.SplittingField)) ≃* Equiv.Perm (f_a5.rootSet ℂ)) := by
      ...
      have h_galiso : Nonempty (f_a5.Gal ≃* alternatingGroup (f_a5.rootSet ℂ)) := by
        refine ⟨?_⟩
        refine' { Equiv.ofBijective (fun x ↦ ⟨f x, _⟩) ⟨fun x y hxy ↦ _, fun x ↦ _⟩ with .. }
        all_goals simp_all [SetLike.ext_iff]
