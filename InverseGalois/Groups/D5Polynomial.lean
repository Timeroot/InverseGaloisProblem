...
lemma f_d5_irreducible : Irreducible f_d5 := by
  ...
  have h_eisenstein : Irreducible (Polynomial.X ^ 5 - 10 * Polynomial.X ^ 4 + 40 * Polynomial.X ^ 3 - 80 * Polynomial.X ^ 2 + 75 * Polynomial.X - 10 : Polynomial ℤ) := by
    ...
    · erw [ Polynomial.leadingCoeff, Polynomial.natDegree_sub_C ] ; norm_num [ Polynomial.natDegree_add_eq_left_of_natDegree_lt, Polynomial.natDegree_sub_eq_left_of_natDegree_lt ];
      norm_num [ Polynomial.coeff_X ];
    · intro n hn; erw [ Polynomial.natDegree_sub_C ] at hn; norm_num [ Polynomial.natDegree_add_eq_left_of_natDegree_lt, Polynomial.natDegree_sub_eq_left_of_natDegree_lt ] at hn; interval_cases n <;> norm_num [ Polynomial.coeff_one, Polynomial.coeff_X ] ;
    · ring_nf;
    ...
    · refine' Polynomial.Monic.isPrimitive _;
      ...
      norm_num [ Polynomial.coeff_X, Polynomial.natDegree_add_eq_left_of_natDegree_lt, Polynomial.natDegree_sub_eq_left_of_natDegree_lt ];
  -- Since $f(X-2)$ is irreducible over $\mathbb{Z}$, it is also irreducible over $\mathbb{Q}$.
  have h_irred_Q : Irreducible (Polynomial.map (algebraMap ℤ ℚ) (Polynomial.X ^ 5 - 10 * Polynomial.X ^ 4 + 40 * Polynomial.X ^ 3 - 80 * Polynomial.X ^ 2 + 75 * Polynomial.X - 10 : Polynomial ℤ)) := by
    -- Since the polynomial is primitive, we can apply Gauss's Lemma to conclude that it is irreducible over ℚ.
    have h_primitive : Polynomial.IsPrimitive (Polynomial.X ^ 5 - 10 * Polynomial.X ^ 4 + 40 * Polynomial.X ^ 3 - 80 * Polynomial.X ^ 2 + 75 * Polynomial.X - 10 : Polynomial ℤ) := by
      intro p;
      rw [ Polynomial.C_dvd_iff_dvd_coeff ] ; norm_num;
      intro h; specialize h 5; norm_num [ Polynomial.coeff_one, Polynomial.coeff_X ] at h; exact isUnit_of_dvd_one h;
  ...
  have h_irred_Q : Irreducible (f_d5.comp (Polynomial.X - 2) : Polynomial ℚ) := by
    convert h_irred_Q using 1 ; norm_num [ f_d5 ] ; ring;
    exact Polynomial.funext fun x => by norm_num; ring;
  rw [ irreducible_iff ] at *;
  refine' ⟨ _, _ ⟩;
  · exact fun h => absurd ( Polynomial.degree_eq_zero_of_isUnit h ) ( by erw [ Polynomial.degree_add_C ] <;> repeat ( first | erw [ Polynomial.degree_add_eq_left_of_degree_lt ] | simp +decide ) );
  · intro a b hab; specialize h_irred_Q; have := @h_irred_Q.2 ( a.comp ( Polynomial.X - 2 ) ) ( b.comp ( Polynomial.X - 2 ) ) ; simp_all +decide ;
    contrapose! h_irred_Q;
    exact fun _ => ⟨ _, _, rfl, fun h => h_irred_Q.1 <| Polynomial.isUnit_iff_degree_eq_zero.mpr <| by have := Polynomial.degree_eq_zero_of_isUnit h; erw [ Polynomial.degree_eq_natDegree ( by aesop_cat ) ] at *; erw [ Polynomial.natDegree_comp, Polynomial.natDegree_X_sub_C ] at *; aesop_cat, fun h => h_irred_Q.2 <| Polynomial.isUnit_iff_degree_eq_zero.mpr <| by have := Polynomial.degree_eq_zero_of_isUnit h; erw [ Polynomial.degree_eq_natDegree ( by aesop_cat ) ] at *; erw [ Polynomial.natDegree_comp, Polynomial.natDegree_X_sub_C ] at *; aesop_cat ⟩
...
lemma disc_value_d5 (v : Fin 5 ≃ (f_d5.rootSet f_d5.SplittingField)) :
    ...
    algebraMap ℚ _ (8000 ^ 2) := by
      have h_factor : (Polynomial.map (algebraMap ℚ (f_d5.SplittingField)) f_d5) = Finset.prod Finset.univ fun i => Polynomial.X - Polynomial.C (v i : f_d5.SplittingField) := by
        convert Polynomial.Splits.eq_prod_roots _;
        any_goals try infer_instance;
        · rw [ show ( Polynomial.map ( algebraMap ℚ f_d5.SplittingField ) f_d5 |> Polynomial.roots ) = Multiset.map ( fun x : f_d5.rootSet f_d5.SplittingField => ( x : f_d5.SplittingField ) ) ( Multiset.map ( fun x : Fin 5 => ( v x : f_d5.rootSet f_d5.SplittingField ) ) Finset.univ.val ) from ?_ ];
          · erw [ Polynomial.leadingCoeff_map, Polynomial.leadingCoeff, Polynomial.natDegree_add_C, Polynomial.natDegree_sub_eq_left_of_natDegree_lt ] <;> norm_num [ f_d5 ];
            conv_rhs => rw [ ← Equiv.prod_comp v ] ;
          · have hv : (f_d5.map (algebraMap ℚ (f_d5.SplittingField))).roots = Multiset.map (fun x => (x : f_d5.SplittingField)) (f_d5.rootSet f_d5.SplittingField).toFinset.val := by
              ...
              unfold Polynomial.rootSet; norm_num [ Polynomial.aroots_def ] ;
      ...
      have h_power_sums : ∑ i : Fin 5, (v i : f_d5.SplittingField) ^ 0 = 5 ∧ ∑ i : Fin 5, (v i : f_d5.SplittingField) ^ 1 = 0 ∧ ∑ i : Fin 5, (v i : f_d5.SplittingField) ^ 2 = 0 ∧ ∑ i : Fin 5, (v i : f_d5.SplittingField) ^ 3 = 0 ∧ ∑ i : Fin 5, (v i : f_d5.SplittingField) ^ 4 = 20 := by
      ...
      have h_power_sums_5 : ∑ i : Fin 5, (v i : f_d5.SplittingField) ^ 5 = -60 := by
        have h_power_sums_5 : ∀ i : Fin 5, (v i : f_d5.SplittingField) ^ 5 = 5 * (v i : f_d5.SplittingField) - 12 := by
          ...
          have h_root : (v i : f_d5.SplittingField) ^ 5 - 5 * (v i : f_d5.SplittingField) + 12 = 0 := by
            replace h_factor := congr_arg ( Polynomial.eval ( v i : f_d5.SplittingField ) ) h_factor ; simp_all +decide [ Finset.prod_eq_prod_diff_singleton_mul ( Finset.mem_univ i ) ] ;
          linear_combination' h_root;
        simp_all +decide;
        rw [ ← Finset.mul_sum _ _ _, h_power_sums.1 ] ; norm_num
      ...
      have h_power_sums_7 : ∑ i : Fin 5, (v i : f_d5.SplittingField) ^ 7 = 0 := by
        have h_power_sums_7 : ∀ i : Fin 5, (v i : f_d5.SplittingField) ^ 7 = 5 * (v i : f_d5.SplittingField) ^ 3 - 12 * (v i : f_d5.SplittingField) ^ 2 := by
          intro i
          have h_root : (v i : f_d5.SplittingField) ^ 5 - 5 * (v i : f_d5.SplittingField) + 12 = 0 := by
            replace h_factor := congr_arg ( Polynomial.eval ( v i : f_d5.SplittingField ) ) h_factor ; simp_all +decide [ Finset.prod_eq_prod_diff_singleton_mul ( Finset.mem_univ i ) ] ;
      ...
      have h_power_sums_8 : ∑ i : Fin 5, (v i : f_d5.SplittingField) ^ 8 = 100 := by
        simp_all +decide [ Fin.sum_univ_five, pow_succ' ];
        grind +ring;
      convert gram_det_value_d5 ( fun i => ( v i : f_d5.SplittingField ) ) _ _ _ _ _ _ _ _ using 1 <;> norm_num [ h_power_sums, h_power_sums_5, h_power_sums_6, h_power_sums_7, h_power_sums_8 ];
...
lemma f_d5_nonreal_root :
  ...
  have h_real_roots : Fintype.card (f_d5.rootSet ℂ) ≤ Fintype.card (f_d5.rootSet ℝ) := by
    ...
    have h_bij : (f_d5.rootSet ℂ).toFinset ⊆ Finset.image (fun x : ℝ => x : ℝ → ℂ) (f_d5.rootSet ℝ).toFinset := by
      ...
      exact fun z h1 h2 => ⟨ z.re, by simpa [ Complex.ext_iff, pow_succ, h z h2 ] using h2, by simp [ Complex.ext_iff, h z h2] ⟩;
    have := Finset.card_le_card h_bij; simp_all [ Finset.card_image_of_injective, Function.Injective ] ;
  -- However, we have that $f_d5$ has at most 3 distinct real roots.
  have h_real_roots_card : Fintype.card (f_d5.rootSet ℝ) ≤ 3 := by
    have h_real_roots_card : Fintype.card (f_d5.rootSet ℝ) ≤ Fintype.card ((Polynomial.derivative f_d5).rootSet ℝ) + 1 := by
