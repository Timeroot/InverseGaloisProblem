...
theorem cubic_mod7_irreducible :
    Irreducible (X ^ 3 + C 2 * X ^ 2 + C 5 * X + C 5 : (ZMod 7)[X]) := by
      ...
      have h_irred : ∀ p q : Polynomial (ZMod 7), p.degree > 0 → q.degree > 0 →
          (p * q = Polynomial.X ^ 3 + Polynomial.C 2 * Polynomial.X ^ 2 + Polynomial.C 5 * Polynomial.X
...
theorem f_a5_mod7_squarefree :
    Squarefree (Polynomial.map (Int.castRingHom (ZMod 7))
      (X ^ 5 + C 20 * X + C 16 : ℤ[X])) := by
        have h_gcd : IsCoprime
            (Polynomial.X ^ 5 + Polynomial.C 20 * Polynomial.X + Polynomial.C 16 : Polynomial (ZMod 7))
          ...
          have h_coprime : ∀ x : AlgebraicClosure (ZMod 7),
              ...
              Polynomial.eval x (Polynomial.map (algebraMap (ZMod 7) (AlgebraicClosure (ZMod 7)))
                (Polynomial.C 5 * Polynomial.X ^ 4 + Polynomial.C 6)) ≠ 0 := by
            intro x
            by_contra h_contra
          ...
          · intro z hz hz' hz'' hz'''
            ...
            obtain ⟨y, hy⟩ := hz''
            obtain ⟨z, hz⟩ := hz'''
            replace hy := congr_arg (Polynomial.map (algebraMap (ZMod 7) (AlgebraicClosure (ZMod 7)))) hy
...
theorem f_a5_mod7_factorizationType :
    3 ∈ factorizationType (Polynomial.map (Int.castRingHom (ZMod 7))
      (X ^ 5 + C 20 * X + C 16 : ℤ[X])) := by
        ...
        have heq : (Polynomial.map (Int.castRingHom (ZMod 7)) (X ^ 5 + C 20 * X + C 16) : (ZMod 7)[X]) =
            (X + 3) * (X + 2) * (X ^ 3 + 2 * X ^ 2 + 5 * X + 5) := by
          ext
          norm_num
          ring_nf
          rename_i n
