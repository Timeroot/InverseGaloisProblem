...
The finite level in which the leftover places are asked to be completely decomposed comes with the
shrinking rather than being chosen here, and it is asked to kill the given lift carried down, which
is what makes the whole decomposition subgroup of such a place die.  It cannot be chosen here: the
larger the level, the more units of the level below become powers in it and the more the
orthogonality asks, so the level making the least demand is the one the carried down lift itself
cuts out — and which lift that is depends on the shrinking.
...
announced in advance, and a surjection onto the number asked for is produced along with the level
and the orthogonality of the naming the values carried across it name. -/
theorem hasKernelPrescription_of_places (N : ℕ) (K : IntermediateField k Ω)
    ...
    (horth : ∀ (F : Gal(Ω/k) →* GenericQuot ℓ U N S (j + 1)) (ι : Type) [Fintype ι]
  ...
  have hacyc' : ∀ μ : ι, ∃ x₀ : ↥(A μ), ∀ x : ↥(A μ),
    ...
    rw [← hm', map_zpow]
  -- the level the orthogonality is read in contains the level the base realization cuts out
  have hEK : E.fixingSubgroup ≤ φ.ker := by
    intro x hx
    rw [← hKker]
    exact hanti K E hKE hx
