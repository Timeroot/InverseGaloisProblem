...
* `InverseGalois.Shafarevich.HasFlatKernelPrescription` — **an equivariant smooth homomorphism into
  ...
  at the named primes or where the given lift kills the whole decomposition subgroup.**
...
* `InverseGalois.Shafarevich.hasFlatPrescription_of_hasFlatKernelPrescription` — **extending a
  ...
  prescription down to the base field**, one shrinking of the operator group paying for the factor
  set of the section.
...
trivial along the finite family, and ramify only at the named primes or where the given lift kills
the whole decomposition subgroup.**
...
The last clause is read of the given lift itself rather than of the lift carried down along a
shrinking, since the prescription spends no shrinking of its own: the number of letters the data is
read at is handed to it, and it answers at that same number.  The shrinking which pays for the
factor set of the section is spent outside, where the number of letters is chosen. -/
  ...
  ∀ (F : Gal(Ω/k) →* GenericQuot ℓ U N S (j + 1)) (ι : Type) [Finite ι]
    ...
    (∀ (μ : ι) (g : Gal(Ω/k)) (x : ↥(A μ)) (hx : g * (x : Gal(Ω/k)) * g⁻¹ ∈ A μ),
      ...
      ∃ u : ↥(φ.ker) →* ↥(layerSub ℓ (Generic U N S) j),
        ...
        (∀ (μ : ι) (x : ↥(A μ)) (hx : (x : Gal(Ω/k)) ∈ φ.ker), u ⟨(x : Gal(Ω/k)), hx⟩ = a μ x) ∧
...
carries the flat prescription down to the base field**, one shrinking of the operator group paying
for the factor set of the section.

The number of letters the data is read at is announced first, and it is announced large enough that
a single surjective shrinking of the operator group annihilates any prescribed family of elements of
the layer indexed by ordered pairs of operators.  The factor set of a section of the base
realization is such a family, so whatever homomorphism the prescription answers with, its values on
that factor set are killed by one shrinking, and the extension of the homomorphism along the section
is an honest cocycle at the number of letters asked for.
...
confining the new ramification. -/
theorem hasFlatPrescription_of_hasFlatKernelPrescription (hS : IsPGroup ℓ S)
    ...
    (hpres : ∀ N : ℕ, HasFlatKernelPrescription ℓ U N S j φ D) :
  ...
  obtain ⟨u, ⟨V, hV, hVu⟩, hueq, huD, hua, huram⟩ :=
    hpres N F ι Q A a hFsurj hFsm hFright hQp hQbot hAstab hAker hasm haequiv
  ...
  refine ⟨α, hα, hαsurj, fun g => layerSubMap ℓ α j (u ⟨g * (s (φ g))⁻¹, hr g⟩), ?_, ?_, ?_, ?_,
    ?_⟩
  · intro g h
  ...
  · intro μ x
    ...
    show layerSubMap ℓ α j (u ⟨(x : Gal(Ω/k)) * (s (φ (x : Gal(Ω/k))))⁻¹, hr (x : Gal(Ω/k))⟩)
      = layerSubMap ℓ α j (a μ x)
    rw [hkerv (x : Gal(Ω/k)) hxk, hua μ x hxk]
