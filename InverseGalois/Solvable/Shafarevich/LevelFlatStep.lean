...
/-!
# The step of the ladder in exchange for flat radicands and twisted norms

The flattening the step of the ladder is bought with is made one field up and one named prime at a
time, and there it is assembled out of the conjugates of a single unit of the level the base
realization cuts out for each named prime: one whose order at the place below that prime is prime to
the exponent and which is a local power everywhere the assembly is asked to be trivial.  This file
spends that assembly the way the sharp prescription was spent — a base realization over the
rationals cuts out a finite Galois level, the roots of unity of order the prime lie in it because
the realization is asked to fix those of order its square, and the places of the level below the
finite family are the places the units are asked to be local powers at.

What is left over is two statements.  One is about a number field alone, asking nothing about any
group: given a finite Galois level of it and finitely many places in pairwise distinct orbits, there
is one unit for each place with those local properties.  The other is about the layer alone, asking
nothing about any field: a value carried by a subgroup to its own twisted powers is the product of a
twisted orbit of that subgroup.  Together with the orthogonality of the naming, which is already a
theorem, the two carry the whole step of the ladder for every odd prime.

## Main definitions

* `Shafarevich.FlatRadicandsEP` — **every level carries, for each of finitely many places in
  pairwise distinct orbits, a unit of order there prime to the exponent and a local power at the
  places the assembly is asked to be trivial at.**

## Main results

* `Shafarevich.flatOrbitPrescriptionEP_of_flatRadicandsEP` — **the flat radicands and the twisted
  norms buy the flattening made one field up and one named prime at a time.**
* `Shafarevich.flatPrescriptionEP_of_flatRadicandsEP` — **the flat radicands and the twisted norms
  buy the flattening.**
* `Shafarevich.genericLevelStepEPRoots_of_flatRadicandsEP` — **the step of the ladder, in exchange
  for the flat radicands and the twisted norms.**
...
/-- **Every level carries, for each of finitely many places in pairwise distinct orbits, a unit of
order there prime to the exponent and a local power at the places the assembly is asked to be
trivial at.**
...
Nothing is asked of a unit under the automorphisms of the level, the conjugates of it being spent
separately in the assembly, one under each coefficient of a twisted orbit of the layer.  What makes
the prescription cost one unit per named prime rather than one per coordinate of the layer is that
the coefficients are asked for along the automorphisms fixing the place, which is exactly the amount
of equivariance the decomposition subgroup of a prime above that place sees. -/
def FlatRadicandsEP (ℓ : ℕ) [NeZero ℓ] : Prop :=
  ∀ (k Ω : Type) [Field k] [NumberField k] [Field Ω] [Algebra k Ω] [IsAlgClosed Ω] [IsGalois k Ω]
      ...
      (ζ : ↥K), IsPrimitiveRoot ζ ℓ → HasFlatRadicands ℓ K

/-! ### The flattening and the step -/

/-- **The flat radicands and the twisted norms buy the flattening made one field up and one named
prime at a time.**
...
theorem flatOrbitPrescriptionEP_of_flatRadicandsEP (ℓ : ℕ) [Fact ℓ.Prime] [NeZero ℓ]
    (htw : TwistedNormEP ℓ) (h : FlatRadicandsEP ℓ) : FlatOrbitPrescriptionEP ℓ := by
  ...
  · haveI : IsAlgClosure ℚ Ω := ⟨inferInstance, inferInstance⟩
    ...
    exact hasFlatOrbitPrescription_of_places K hKker hζ hkd hPrp hPrbot hDPr
      (exists_smul_placeUnder_of_mem K hPrp hPrbot hcovP) htw (h ℚ Ω K ⟨z, hzK⟩ hζ)
  · refine ⟨n, ?_⟩
...
/-- **The flat radicands and the twisted norms buy the flattening.** -/
theorem flatPrescriptionEP_of_flatRadicandsEP (ℓ : ℕ) [Fact ℓ.Prime] [NeZero ℓ]
    (htw : TwistedNormEP ℓ) (h : FlatRadicandsEP ℓ) : FlatPrescriptionEP ℓ :=
  flatPrescriptionEP_of_flatOrbitPrescriptionEP ℓ
    (flatOrbitPrescriptionEP_of_flatRadicandsEP ℓ htw h)

/-- **The step of the ladder, in exchange for the flat radicands and the twisted norms** — one
piece of arithmetic and one piece of group homology, which is all the climb rests on for an odd
prime. -/
theorem genericLevelStepEPRoots_of_flatRadicandsEP (ℓ : ℕ) [Fact ℓ.Prime] [NeZero ℓ]
    (hodd : 2 < ℓ) (htw : TwistedNormEP ℓ) (h : FlatRadicandsEP ℓ) : GenericLevelStepEPRoots ℓ :=
  genericLevelStepEPRoots_of_flatPrescriptionEP ℓ hodd
    (flatPrescriptionEP_of_flatRadicandsEP ℓ htw h)
