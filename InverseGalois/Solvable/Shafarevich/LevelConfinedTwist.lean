...
They are asked in addition to be trivial along the inertia subgroup of their prime, which is what
makes the local classes naming them unramified there; it costs nothing, the lift being corrected
already carrying the whole of inertia on the generator the correction is normalised at.
...
theorem hasSplitCyclicRepair_of_hasConfinedPrescription (hS : IsPGroup ℓ S) (hj : 1 ≤ j)
  ...
  have hstep : ∀ μ : {μ : Fin s // RamifiesAt φ f (Pr μ)}, ∃ (A : Subgroup Gal(Ω/k))
      ...
      A ≤ stabilizer Gal(Ω/k) (Pr (μ : Fin s)) ∧ A ≤ φ.ker ∧
        ...
        (∀ x : ↥A, (x : Gal(Ω/k)) ∈ Ideal.inertia Gal(Ω/k) (Pr (μ : Fin s)) → a x = 1) ∧
    ...
    refine ⟨stabilizer Gal(Ω/k) (Pr μ), a, le_rfl, hQker ⟨μ, hμ⟩, rfl, ?_, hacyc, ?_, ?_⟩
    · obtain ⟨C, hC, hCf⟩ :=
        isSmooth₁_comp (continuous_subtype (stabilizer Gal(Ω/k) (Pr μ))) hfs
      exact ⟨C, hC, fun x m hm => hafac (x * m) x (hCf x m hm)⟩
    · intro x hx
      obtain ⟨i, hi⟩ := Subgroup.mem_zpowers_iff.1 (hinert (x : Gal(Ω/k)) hx)
      have hcoe : (((⟨τ, hτA⟩ : ↥(stabilizer Gal(Ω/k) (Pr μ))) ^ i :
          ↥(stabilizer Gal(Ω/k) (Pr μ))) : Gal(Ω/k)) = τ ^ i :=
        _root_.map_zpow (stabilizer Gal(Ω/k) (Pr μ)).subtype _ i
      have hfeq : f (((⟨τ, hτA⟩ : ↥(stabilizer Gal(Ω/k) (Pr μ))) ^ i :
          ↥(stabilizer Gal(Ω/k) (Pr μ))) : Gal(Ω/k)) = f (x : Gal(Ω/k)) := by
        rw [hcoe, _root_.map_zpow, hi]
      rw [← hafac _ _ hfeq, _root_.map_zpow, haτ, one_zpow]
    · intro β hβ Ψ hΨ _
  ...
  choose A a hAstab hAker hAcase hasm hacyc hainert hAkey using hstep
  ...
  obtain ⟨α₂, hα₂, hα₂surj, c, hc, hcs, hcD, hca, hcram⟩ :=
    hpres hφopen f {μ : Fin s // RamifiesAt φ f (Pr μ) ∧
      ...
      (fun μ => hacyc ⟨μ.1, μ.2.1⟩) (fun μ => hainert ⟨μ.1, μ.2.1⟩) havoid
