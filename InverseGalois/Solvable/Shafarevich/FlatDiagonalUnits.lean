...
* `Shafarevich.genericLevelStepEPRoots`: **the step of the ladder**, with nothing left to assume.
...
/-- **The step of the ladder, in exchange for the units.** -/
theorem genericLevelStepEPRoots_of_flatDiagonalUnitsEP (ℓ : ℕ) [Fact ℓ.Prime] [NeZero ℓ]
    (hunits : FlatDiagonalUnitsEP ℓ) : GenericLevelStepEPRoots ℓ :=
  genericLevelStepEPRoots_of_confinedDiagonalPlacesEP ℓ
    (confinedDiagonalPlacesEP_of_flatDiagonalUnitsEP ℓ hunits)

/-- **The step of the ladder**, with nothing left to assume: a place reachable in the bigger level
carries the unit the diagonal wants there, the diagonal makes every system of orders at the chosen
places the system of orders of a confined unit, and the correction room the second reading of those
units needs is bounded by the refined class group of the level. -/
theorem genericLevelStepEPRoots (ℓ : ℕ) [Fact ℓ.Prime] [NeZero ℓ] :
    GenericLevelStepEPRoots ℓ :=
  genericLevelStepEPRoots_of_flatDiagonalUnitsEP ℓ (flatDiagonalUnitsEP ℓ)
