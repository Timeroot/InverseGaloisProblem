...
one element which is inertia, the local field is asked to contain the roots of unity the layer
needs, and adjoining a further root of a uniformizer produces the local lift.
...
* `InverseGalois.Shafarevich.IsSplitTotallyRamified` — **at a prime where the solution ramifies
  over the base realization, the base realization splits completely, the solution is cyclic and
  totally ramified, and the local field carries the roots of unity the next layer calls for.**
...
and there the clauses transport one by one, the order of the generator of the local image only
dropping to a divisor. -/
