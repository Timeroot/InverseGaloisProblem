...
* `Shafarevich.genericLevelStepEPRoots_of_flatTensorEP` — **the step of the ladder, in exchange
  for the tensor alone.**
* `Shafarevich.flatTensorEP_of_invariantUnitTensorEP` — the invariant tensor of units buys the
  prescribed tensor.
* `Shafarevich.genericLevelStepEPRoots_of_invariantUnitTensorEP` — **the step of the ladder, in
  exchange for the invariant tensor of units.**
...
theorem flatPrescriptionEP_of_flatTensorEP (ℓ : ℕ) [Fact ℓ.Prime] [NeZero ℓ]
  ...
  · haveI : IsAlgClosure ℚ Ω := ⟨inferInstance, inferInstance⟩
    ...
    exact hasFlatKernelPrescription_of_tensorPlaces hS K hKker hζ hkd hPrp hPrbot hDPr
      (exists_smul_placeUnder_of_mem K hPrp hPrbot hcovP) Tz hTz
      (fun m => hreach ℚ Ω S U φ m j K ⟨_, hζ⟩ hKker ↑Tz Tz.finite_toSet) (h Ω K _ hζ)
  · refine ⟨0, ?_⟩
...
/-- **The step of the ladder, in exchange for the tensor alone** — the arithmetic read as a single
invariant object, the level reaching every place bought by the shrinking. -/
theorem genericLevelStepEPRoots_of_flatTensorEP (ℓ : ℕ) [Fact ℓ.Prime] [NeZero ℓ]
    (h : FlatTensorEP ℓ) : GenericLevelStepEPRoots ℓ :=
  genericLevelStepEPRoots_of_flatPrescriptionEP ℓ
...
/-- **The step of the ladder, in exchange for the invariant tensor of units** — the arithmetic of
the climb resting on one statement about the units of a number field. -/
theorem genericLevelStepEPRoots_of_invariantUnitTensorEP (ℓ : ℕ) [Fact ℓ.Prime] [NeZero ℓ]
    (h : InvariantUnitTensorEP ℓ) : GenericLevelStepEPRoots ℓ :=
  genericLevelStepEPRoots_of_flatTensorEP ℓ (flatTensorEP_of_invariantUnitTensorEP ℓ h)
