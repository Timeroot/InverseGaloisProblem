...
theorem exists_prime_family {ι : Type*} [Fintype ι] (n : ι → ℕ) (hn : ∀ i, n i ≠ 0) :
  ...
  | empty => exact ⟨fun _ => 2, by simp, by simp, by simp⟩
  | @insert a s ha ih =>
    ...
    · intro i hi
      ...
      · rw [Function.update_of_ne (ne_of_mem_of_not_mem his ha)]; exact hp_prime i his
    · intro i hi
      rcases Finset.mem_insert.mp hi with rfl | his
      · rw [Function.update_self]
        exact (Nat.modEq_iff_dvd' hq1).mp hq_mod.symm
      · rw [Function.update_of_ne (ne_of_mem_of_not_mem his ha)]; exact hp_dvd i his
    · intro x hx y hy hxy
      ...
      · rfl
      · rw [Function.update_self, Function.update_of_ne (ne_of_mem_of_not_mem hys ha)] at hxy
        exact absurd hxy.symm (Nat.ne_of_lt (hlt y hys))
      · rw [Function.update_self, Function.update_of_ne (ne_of_mem_of_not_mem hxs ha)] at hxy
        exact absurd hxy (Nat.ne_of_lt (hlt x hxs))
      · rw [Function.update_of_ne (ne_of_mem_of_not_mem hxs ha),
          Function.update_of_ne (ne_of_mem_of_not_mem hys ha)] at hxy
...
theorem IsInverseGalois.of_finite_commGroup (G : Type*) [CommGroup G] [Finite G] :
  ...
  have hn0 : ∀ i, n i ≠ 0 := fun i => by have := hn1 i; omega
  obtain ⟨p, hp_prime, hp_dvd, hp_inj⟩ := exists_prime_family n hn0
  set m : ℕ := ∏ i, p i with hm
  have hm0 : NeZero m := ⟨by
    rw [hm]; exact Finset.prod_ne_zero_iff.mpr fun i _ => (hp_prime i).ne_zero⟩
  ...
  have ecyc : ∀ i, (ZMod (p i))ˣ ≃* Multiplicative (ZMod (p i - 1)) := by
    intro i
    haveI := Fact.mk (hp_prime i)
    have hcard : Nat.card (ZMod (p i))ˣ = p i - 1 := by
      rw [Nat.card_eq_fintype_card, ZMod.card_units_eq_totient, Nat.totient_prime (hp_prime i)]
    rw [← hcard]
