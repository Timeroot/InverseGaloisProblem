...
lemma two_le_natDegree_choose {d k : ℕ} (hk : 1 ≤ k) (hk' : k < d) : 2 ≤ d.choose k := by
  induction hk' <;> simp_all +arith +decide [ Nat.choose ];
...
lemma esymm_map_ringHom {A B : Type*} [CommRing A] [CommRing B] (φ : A →+* B)
  ...
  induction' ( Multiset.powersetCard n s ) using Multiset.induction_on with a s ih <;> simp_all [ mul_assoc, map_multiset_prod ]
...
lemma wval_map {A B : Type*} [CommRing A] [CommRing B] (φ : A →+* B) (k : ℕ) (lam : Fin k → ℤ)
  ...
  unfold wval; simp [ *, Finset.mul_sum ] ;
...
lemma resolventProduct_map {A B : Type*} [CommRing A] [CommRing B] (φ : A →+* B) (k : ℕ)
  ...
  simp [resolventProduct, Polynomial.map_multiset_prod, Multiset.map_map, Multiset.powersetCard_map, Polynomial.map_sub, Polynomial.map_X, Polynomial.map_C];
  simp [wval_map]
...
lemma resolventProduct_univ_isSymmetric (d k : ℕ) (lam : Fin k → ℤ) (i : ℕ) :
  ...
  intro σ; simp [ resolventProduct ] ;
  -- By definition of `resolventProduct`, we know that its coefficients are symmetric.
  have h_symm : (MvPolynomial.rename σ).toRingHom ((resolventProduct k lam (Multiset.map (MvPolynomial.X : Fin d → MvPolynomial (Fin d) ℤ) Finset.univ.val)).coeff i) = ((resolventProduct k lam (Multiset.map (MvPolynomial.X : Fin d → MvPolynomial (Fin d) ℤ) Finset.univ.val)).map (MvPolynomial.rename σ).toRingHom).coeff i := by
    rw [ Polynomial.coeff_map ];
  convert h_symm using 1;
  · unfold resolventProduct; aesop;
  · rw [ resolventProduct_map ];
    rw [ show ( Multiset.map ( ⇑ ( MvPolynomial.rename ⇑σ ).toRingHom ) ( Multiset.map MvPolynomial.X Finset.univ.val ) ) = Multiset.map MvPolynomial.X ( Finset.univ.val.map σ ) from ?_ ];
...
lemma exists_esymm_lift (d : ℕ) (U : Polynomial (MvPolynomial (Fin d) ℤ))
  ...
  have h_coeff : ∀ i, ∃ p : Polynomial (MvPolynomial (Fin d) ℤ), Polynomial.map (MvPolynomial.aeval (fun i : Fin d => MvPolynomial.esymm (Fin d) ℤ (i.val + 1))).toRingHom p = Polynomial.C (U.coeff i) := by
    intro i;
    have := hU i;
  ...
  ext i; simp [ Polynomial.coeff_sum, Polynomial.coeff_monomial ] ;
...
lemma exists_fin_map_eq {A : Type*} (r : Multiset A) (n : ℕ) (h : r.card = n) :
  ...
  rcases r with ⟨ l ⟩ ; simp_all [ Multiset.map_coe ] ;
  refine' ⟨ fun i => l.get ⟨ i, by linarith [ Fin.is_lt i ] ⟩, _ ⟩;
  rw [ List.ofFn_eq_map ] ; aesop;
...
lemma resolvent_identity (F : Polynomial (Polynomial ℤ)) (hF : F.Monic)
  ...
  have hP : ∀ {A : Type} [Field A] (ev : ℤ[X] →+* A), (F.map ev).natDegree = d → (F.map ev).roots.card = d → (P.map ev) = resolventProduct k lam (F.map ev).roots := by
    ...
    have h_ring_hom : ev.comp cval = aρ.comp ((MvPolynomial.aeval (fun i : Fin d => MvPolynomial.esymm (Fin d) ℤ (i.val + 1))).toRingHom) := by
      ext i; simp [cval, aρ];
      have h_vieta : p.coeff (d - (i.val + 1)) = 1 * (-1 : A) ^ (i.val + 1) * r.esymm (i.val + 1) := by
        have h_vieta_i : p = Polynomial.C (p.leadingCoeff) * Multiset.prod (Multiset.map (fun x => Polynomial.X - Polynomial.C x) r) := by
        ...
        · rw [ hcard ];
          rw [ Nat.sub_sub_self ( by linarith [ Fin.is_lt i ] ) ] ; ring;
      ...
      rw [ ← hρ ] ; norm_num [ MvPolynomial.aeval_esymm_eq_multiset_esymm ] ; ring;
      norm_num [ pow_mul' ];
  ...
  exact absurd ( h_contra P ) ( by push_neg; tauto )
...
lemma exists_resolvent_poly (F : Polynomial (Polynomial ℤ)) (hF : F.Monic)
  ...
  · -- Let $A$ be an algebraically closed field of characteristic $0$, such as $\mathbb{C}$.
    ...
    obtain ⟨ev,hev⟩ : ∃ ev : Polynomial ℤ →+* AlgebraicClosure A, Function.Injective ev := by
      exact ⟨ ( algebraMap A ( AlgebraicClosure A ) ).comp ( algebraMap ( Polynomial ℤ ) A ), by exact ( algebraMap A ( AlgebraicClosure A ) ).injective.comp ( IsFractionRing.injective _ _ ) ⟩;
    have hP_monic : (P.map ev).Monic := by
      ...
      rw [ Polynomial.splits_iff_card_roots ] at hP_monic ; aesop;
    rw [ Polynomial.Monic, Polynomial.leadingCoeff_map_of_injective hev ] at hP_monic ; aesop;
  · obtain ⟨A, hA⟩ : ∃ A : Type, ∃ (inst : Field A), ∃ (ev : Polynomial ℤ →+* A), Function.Injective ev ∧ (F.map ev).natDegree = F.natDegree ∧ (F.map ev).roots.card = F.natDegree := by
      obtain ⟨A, hA⟩ : ∃ A : Type, ∃ (inst : Field A), ∃ (ev : Polynomial ℤ →+* A), Function.Injective ev ∧ (F.map ev).Splits := by
        use AlgebraicClosure (FractionRing (Polynomial ℤ));
        refine' ⟨ inferInstance, _, _, _ ⟩;
        exact ( algebraMap ( Polynomial ℤ ) ( FractionRing ( Polynomial ℤ ) ) ).comp ( algebraMap ( Polynomial ℤ ) ( Polynomial ℤ ) ) |> RingHom.comp ( algebraMap ( FractionRing ( Polynomial ℤ ) ) ( AlgebraicClosure ( FractionRing ( Polynomial ℤ ) ) ) );
        · exact fun x y hxy => by simpa using hxy;
        · exact IsAlgClosed.splits_codomain _;
      obtain ⟨ inst, ev, hev₁, hev₂ ⟩ := hA; use A, inst, ev; simp_all [ Polynomial.splits_iff_card_roots ] ;
    obtain ⟨ inst, ev, hev, hF₁, hF₂ ⟩ := hA; specialize hP ev hF₁ hF₂; replace hP := congr_arg Polynomial.natDegree hP; simp_all [ Polynomial.natDegree_map ] ;
    rw [ ← hF₂, resolventProduct_natDegree ] at *;
    rw [ ← Polynomial.natDegree_map_eq_of_injective hev ] ; aesop;
...
lemma exists_esymm_notMem {K L : Type} [Field K] [Field L] [Algebra K L]
  ...
  by_contra! h_contra;
  obtain ⟨gK, hgK⟩ : ∃ gK : Polynomial K, gK.Monic ∧ gK.map (algebraMap K L) = Multiset.prod (s.map (fun β => Polynomial.X - Polynomial.C β)) ∧ gK.natDegree = k := by
    have h_lift : ∃ gK : Polynomial K, gK.map (algebraMap K L) = Multiset.prod (s.map (fun β => Polynomial.X - Polynomial.C β)) ∧ gK.natDegree = k := by
      have h_lift : ∃ gK : Polynomial K, gK.map (algebraMap K L) = Multiset.prod (s.map (fun β => Polynomial.X - Polynomial.C β)) := by
        have h_lift : ∀ j : ℕ, j ≤ k → (Multiset.prod (s.map (fun β => Polynomial.X - Polynomial.C β))).coeff j ∈ Set.range (algebraMap K L) := by
          intro j hj; by_cases hj' : j = k <;> simp_all [ Multiset.prod_X_sub_C_coeff ] ;
          · exact ⟨ 1, by simp [ Multiset.esymm ] ⟩;
          · obtain ⟨ y, hy ⟩ := h_contra ⟨ k - j - 1, by omega ⟩ ; use ( -1 ) ^ ( k - j ) * y; simp_all [ Nat.sub_sub, add_comm ] ;
            rw [ show 1 + ( k - ( j + 1 ) ) = k - j by omega ];
        choose! g hg using h_lift;
        use ∑ j ∈ Finset.range (k + 1), g j • Polynomial.X ^ j;
        ext j; simp [ Polynomial.coeff_sum, Polynomial.coeff_X_pow, hg ] ;
        split_ifs <;> simp_all [ Polynomial.coeff_eq_zero_of_natDegree_lt ];
      obtain ⟨ gK, hgK ⟩ := h_lift; use gK; have := congr_arg Polynomial.natDegree hgK; rw [ Polynomial.natDegree_map, Polynomial.natDegree_multiset_prod ] at this <;> simp_all ;
      exact fun x hx => Polynomial.X_sub_C_ne_zero x;
    obtain ⟨ gK, hgK₁, hgK₂ ⟩ := h_lift; use gK; simp_all [ Polynomial.Monic.def ] ;
    replace hgK₁ := congr_arg Polynomial.leadingCoeff hgK₁ ; simp_all [ Polynomial.leadingCoeff_multiset_prod ] ;
  have h_div : gK ∣ f := by
    have h_div : Multiset.prod (s.map (fun β => Polynomial.X - Polynomial.C β)) ∣ Polynomial.map (algebraMap K L) f := by
      have h_div : Multiset.map (fun β => Polynomial.X - Polynomial.C β) s ≤ Multiset.map (fun β => Polynomial.X - Polynomial.C β) (Polynomial.roots (Polynomial.map (algebraMap K L) f)) := by
    ...
    aesop;
  obtain ⟨ q, rfl ⟩ := h_div; simp_all [ irreducible_mul_iff ] ;
...
lemma exists_generic_lam {K L : Type} [Field K] [Field L] [Algebra K L] [CharZero L]
  ...
  have h_linear_maps : ∀ s ∈ (f.map (algebraMap K L)).roots.powersetCard k, ∃ Φ_s : M →ₗ[ℚ] L, ∀ μ : M, Φ_s μ = ∑ j : Fin k, algebraMap ℚ L (μ j) * s.esymm (↑j + 1) := by
    intro s hs;
    refine' ⟨ _, _ ⟩;
    ...
    · simp +zetaDelta at *;
      exact fun x y => by rw [ ← Finset.sum_add_distrib ] ; exact Finset.sum_congr rfl fun _ _ => by ring;
    · simp +zetaDelta at *;
  ...
  obtain ⟨μ, hμ⟩ : ∃ μ : M, ∀ s ∈ (f.map (algebraMap K L)).roots.powersetCard k, Φ s μ ∉ Set.range (algebraMap K L) := by
    have h_submodule : ∀ s ∈ (f.map (algebraMap K L)).roots.powersetCard k, ∃ p : Submodule ℚ M, p ≠ ⊤ ∧ ∀ μ : M, μ ∈ p ↔ Φ s μ ∈ Set.range (algebraMap K L) := by
      intro s hs
      obtain ⟨p, hp⟩ : ∃ p : Submodule ℚ M, ∀ μ : M, μ ∈ p ↔ Φ s μ ∈ Set.range (algebraMap K L) := by
        have h_submodule : ∃ p : Submodule ℚ L, ∀ x : L, x ∈ p ↔ x ∈ Set.range (algebraMap K L) := by
          ...
          intro x; exact ⟨fun hx => by
            ...
            obtain ⟨ c, rfl ⟩ := hx; simp [ Finsupp.sum ] ;
            use ∑ x ∈ c.support, c x • x; simp [ Algebra.smul_def ] ;
            simp [ Algebra.smul_def, Rat.smul_def ], fun hx => by
            exact Submodule.subset_span hx⟩;
        exact ⟨ Submodule.comap ( Φ s ) h_submodule.choose, fun μ => by simpa using h_submodule.choose_spec ( Φ s μ ) ⟩;
      have := exists_esymm_notMem f hf_irr hf_monic hsplit k hk hk' s hs; simp_all [ Submodule.eq_top_iff' ] ;
      refine' ⟨ p, _, hp ⟩;
      obtain ⟨ j, hj ⟩ := this; use fun i => if i = j then 1 else 0; simp_all [ Finset.sum_ite, Finset.filter_eq', Finset.filter_ne' ] ;
      simp_all [ Finset.sum_eq_single j, Fin.val_inj ];
  ...
  obtain ⟨n, hn_ne_zero, lam, hlam⟩ : ∃ n : ℤ, n ≠ 0 ∧ ∃ lam : Fin k → ℤ, ∀ j : Fin k, (lam j : ℚ) = n * μ j := by
    ...
    obtain ⟨n, hn⟩ : ∃ n : ℕ, n ≠ 0 ∧ ∀ j : Fin k, (μ j).den ∣ n := by
      exact ⟨ ∏ j, ( μ j |> Rat.den ), Finset.prod_ne_zero_iff.mpr fun j _ => Nat.cast_ne_zero.mpr <| Rat.den_nz _, fun j => Finset.dvd_prod_of_mem _ <| Finset.mem_univ _ ⟩;
    refine' ⟨ n, mod_cast hn.1, fun j => ( n * μ j |> Rat.num ), fun j => _ ⟩ ; simp [ Rat.cast_def, mul_div_assoc, hn.2 j ];
    obtain ⟨ m, hm ⟩ := hn.2 j; simp [ hm, mul_assoc, mul_left_comm, Rat.mul_den, Rat.mul_num ] ;
  ...
  have h_wval : ∀ s ∈ (f.map (algebraMap K L)).roots.powersetCard k, wval k lam s = (n : L) * Φ s μ := by
    intro s hs; simp [ hΦ s hs, wval, Finset.mul_sum _ _ _, mul_assoc, mul_left_comm, hlam ] ;
    exact Finset.sum_congr rfl fun _ _ => by rw [ show ( lam _ : L ) = n * μ _ from mod_cast hlam _ ] ; ring;
  refine' ⟨ lam, fun s hs => _ ⟩;
  contrapose! hμ;
  obtain ⟨ x, hx ⟩ := hμ;
  refine' ⟨ s, hs, ⟨ x / n, _ ⟩ ⟩;
  simp_all [ mul_comm, mul_assoc, mul_left_comm, div_eq_mul_inv ]
...
lemma isRoot_resolventProduct_of_mem {A : Type*} [CommRing A] (k : ℕ) (lam : Fin k → ℤ)
  ...
  simp [ hs, Polynomial.eval_multiset_prod ];
...
lemma esymm_roots_map_mem_range {R A : Type*} [CommRing R] [CommRing A] [IsDomain A]
  ...
  · have h_coeff : Polynomial.coeff (Polynomial.map φ g) (Polynomial.natDegree g - m) = (-1 : A) ^ m * (Polynomial.roots (Polynomial.map φ g)).esymm m := by
      convert Polynomial.coeff_eq_esymm_roots_of_card ( show Multiset.card ( Polynomial.roots ( Polynomial.map φ g ) ) = Polynomial.natDegree ( Polynomial.map φ g ) from ?_ ) ?_ using 1;
      ...
      · rw [ hcard, Polynomial.natDegree_map_of_leadingCoeff_ne_zero ] ; aesop;
...
lemma wval_roots_map_mem_range {R A : Type*} [CommRing R] [CommRing A] [IsDomain A]
  ...
  convert SetLike.mem_coe.mpr ( Subring.sum_mem _ fun j ( hj : j ∈ ( Finset.univ : Finset ( Fin k ) ) ) => Subring.mul_mem _ ?_ ?_ );
  rotate_left;
  exact φ.range;
  · exact ⟨ lam j, by simp ⟩;
