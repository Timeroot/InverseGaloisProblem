...
theorem vander_isAlternating : IsAlternating (vander n) := by
  ...
  simp [vander]
  let v : Fin n → MvPolynomial (Fin n) ℚ := fun i ↦ X i
  ...
  have hv : (Matrix.vandermonde v).det = ∏ i : Fin n, ∏ j ∈ Finset.Ioi i, (v j - v i) :=
    Matrix.det_vandermonde _
  rw [hv', hv] at det_vandermonde_perm
...
theorem isRelPrime_X_sub_X_of_ne {i j k l : Fin n} (hij : i < j) (hkl : k < l)
  ...
  · have hvk : v k = ((k : ℕ) : ℚ) := by simp [hv, ne_of_lt hkl]
    rw [hvj, hvk] at hmap
    rw [hcast i k hmap] at hne
    exact hne rfl
...
theorem X_sub_X_dvd_of_isAlternating {i j : Fin n} (hij : i ≠ j)
  ...
  have hswap : rename (Equiv.swap i j) p = -p := by
    simpa [Equiv.Perm.sign_swap hij] using hp (Equiv.swap i j)
  ...
  have hφp : MvPolynomial.aeval g p = 0 :=
    add_self_eq_zero.mp (neg_eq_iff_add_eq_zero.mp h1)
  ...
  have hp0 : Ideal.Quotient.mkₐ ℚ I p = 0 := hc.symm
...
theorem vander_dvd_of_isAlternating {p : MvPolynomial (Fin n) ℚ} (hp : IsAlternating p) :
  ...
  · intro x hx y hy hxy
    ...
    refine isRelPrime_X_sub_X_of_ne hx hy ?_
    simpa using hxy
  · intro x hx
    obtain ⟨i, j⟩ := x
...
theorem exists_symmetric_of_isAlternating {p : MvPolynomial (Fin n) ℚ} (hp : IsAlternating p) :
  ...
  · rw [MvPolynomial.IsSymmetric]
    ...
    rw [hq, map_mul, hvander, Algebra.smul_mul_assoc] at hp'
    have hsign_ne : (↑↑(Equiv.Perm.sign σ) : ℚ) ≠ 0 := by
      rcases Int.units_eq_one_or (Equiv.Perm.sign σ) with h | h <;> simp [h]
    have hp'' : vander n * rename σ q = vander n * q :=
      smul_right_injective _ hsign_ne hp'
    have hvander_ne : vander n ≠ 0 := by
      ...
      apply Finset.prod_ne_zero_iff.mpr
      intro j hj
      exact (prime_X_sub_X (Finset.mem_Ioi.mp hj).ne').ne_zero
...
theorem exists_symm_add_alternating (hn : 2 ≤ n) {p : MvPolynomial (Fin n) ℚ}
  ...
  obtain ⟨τ₀, hτ₀⟩ : ∃ σ : Equiv.Perm (Fin n), Equiv.Perm.sign σ = -1 := by
    use Equiv.swap ⟨0, by linarith⟩ ⟨1, by linarith⟩
    simp
  let s := (1/2 : ℚ) • (p + rename τ₀ p)
  let a := (1/2 : ℚ) • (p - rename τ₀ p)
  have hcancel {ρ μ : Equiv.Perm (Fin n)} (h : rename (↑ρ⁻¹) (rename μ p) = p) :
      rename μ p = rename ρ p := by
    conv_rhs => rw [← h, rename_rename]
    simp
  refine ⟨s, a, ?_, ?_, ?_⟩
  · intro σ
    simp only [s]
    ...
    · have hσ_odd : Equiv.Perm.sign σ = -1 := by
        rcases Int.units_eq_one_or (Equiv.Perm.sign σ) with h | h <;> simp_all [Equiv.Perm.mem_alternatingGroup]
  ...
  · intro σ
    ...
    · have hsign : Equiv.Perm.sign σ = 1 := by
        rwa [Equiv.Perm.mem_alternatingGroup] at hσ
      rw [hsign]
      simp
      have h1 : rename σ p = p := hp σ hσ
