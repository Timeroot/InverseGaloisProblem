...
theorem Perm_Fin5_no_subgroup_order_15 :
  ...
  obtain ⟨g, hg⟩ : ∃ g : H, orderOf g = 15 := by
    ...
    obtain ⟨g3, hg3⟩ : ∃ g3 : H, orderOf g3 = 3 := by
      ...
      swap;
      exacts [ Fintype.ofFinite _, by rw [ Fintype.card_eq_nat_card ] ; exact h_card.symm ▸ by decide ]
    obtain ⟨g5, hg5⟩ : ∃ g5 : H, orderOf g5 = 5 := by
      ...
      have hP_card : Nat.card P = 5 := by
        ...
        exact this.trans ( by rw [ h_card ] ; native_decide );
      ...
      have hP_order : orderOf g = 5 := by
        rw [ orderOf_eq_card_of_forall_mem_zpowers hg ] ; aesop;
    ...
    have h_comm : g3 * g5 = g5 * g3 := by
      have h_comm : Subgroup.Normal (Subgroup.zpowers g3) ∧ Subgroup.Normal (Subgroup.zpowers g5) := by
        have h_sylow : ∀ p : ℕ, Nat.Prime p → p ∣ Nat.card H → Nat.card (Sylow p H) = 1 := by
          ...
          have h_sylow : Nat.card (Sylow p H) ≡ 1 [MOD p] ∧ Nat.card (Sylow p H) ∣ Nat.card H := by
            ...
            have h_sylow_div : Nat.card (Sylow p H) ∣ Nat.card H := by
              have h_orbit : Nat.card (Sylow p H) = Nat.card (H ⧸ Subgroup.normalizer (Sylow.toSubgroup (Classical.arbitrary (Sylow p H)))) := by
          ...
          · have := Nat.le_of_dvd ( by decide ) h_sylow.2; interval_cases Fintype.card ( Sylow 3 H ) <;> trivial;
          · have := Nat.le_of_dvd ( by decide ) h_sylow.2; interval_cases Fintype.card ( Sylow 5 H ) <;> trivial;
          · have := Nat.le_of_dvd ( by decide ) hp_div; interval_cases _ : p + 6 <;> simp_all +decide ;
        have h_sylow_normal : ∀ p : ℕ, Nat.Prime p → p ∣ Nat.card H → ∀ P : Sylow p H, Subgroup.Normal (P.toSubgroup) := by
        ...
        have h_sylow_g3 : ∃ P : Sylow 3 H, Subgroup.zpowers g3 = P.toSubgroup := by
          have h_sylow_g3 : IsPGroup 3 (Subgroup.zpowers g3) := by
            ...
            obtain ⟨ k, hk ⟩ := x.2;
            simp_all +decide [← hg3];
            rw [ ← Subtype.coe_inj ] ; simp +decide [← hk];
          obtain ⟨ P, hP ⟩ := IsPGroup.exists_le_sylow h_sylow_g3;
          have hP_card : Nat.card (Subgroup.zpowers g3) = Nat.card P.toSubgroup := by
            have := P.card_eq_multiplicity; simp_all +decide [ Nat.Prime.dvd_iff_one_le_factorization ] ;
        ...
        have h_sylow_g5 : ∃ P : Sylow 5 H, Subgroup.zpowers g5 = P.toSubgroup := by
          have h_sylow_g5 : IsPGroup 5 (Subgroup.zpowers g5) := by
            intro x;
            use 1;
            rcases x with ⟨ x, hx ⟩ ; obtain ⟨ k, rfl ⟩ := Subgroup.mem_zpowers_iff.mp hx; simp +decide [← hg5] ;
          ...
          have hP_card : Nat.card (Subgroup.zpowers g5) = Nat.card P.toSubgroup := by
            have hP_card : Nat.card P.toSubgroup = 5 := by
              ...
              · rw [ h_card ] ; native_decide;
              · exact ⟨ by norm_num ⟩;
            simp_all +decide [Nat.card_eq_fintype_card];
          have hP_eq : Subgroup.zpowers g5 = P.toSubgroup := by
            exact SetLike.ext' ( Set.eq_of_subset_of_ncard_le hP hP_card.ge )
          use P;
        exact ⟨ by obtain ⟨ P, hP ⟩ := h_sylow_g3; exact hP ▸ h_sylow_normal 3 Nat.prime_three ( by rw [ h_card ] ; decide ) P, by obtain ⟨ P, hP ⟩ := h_sylow_g5; exact hP ▸ h_sylow_normal 5 ( by decide ) ( by rw [ h_card ] ; decide ) P ⟩;
      ...
      have h_comm : Subgroup.zpowers g3 ⊓ Subgroup.zpowers g5 = ⊥ := by
        ...
        have h_comm : Nat.card (↥(Subgroup.zpowers g3 ⊓ Subgroup.zpowers g5)) ∣ Nat.gcd 3 5 := by
          exact Nat.dvd_gcd ( by simpa [ h_comm ] using Subgroup.card_dvd_of_le ( show Subgroup.zpowers g3 ⊓ Subgroup.zpowers g5 ≤ Subgroup.zpowers g3 from inf_le_left ) ) ( by simpa [ h_comm ] using Subgroup.card_dvd_of_le ( show Subgroup.zpowers g3 ⊓ Subgroup.zpowers g5 ≤ Subgroup.zpowers g5 from inf_le_right ) );
        simp_all +decide [ Subgroup.eq_bot_iff_card ];
      simp_all +decide [ mul_inv_eq_iff_eq_mul ];
    use g3 * g5;
    have h_order : ∀ {a b : H}, a * b = b * a → Nat.gcd (orderOf a) (orderOf b) = 1 → orderOf (a * b) = orderOf a * orderOf b := by
...
lemma normal_subgroup_Perm_Fin5_trichotomy
  ...
  · -- If $M$ is trivial, then $N$ has at most two elements.
    have hN_le_two : N ≤ Subgroup.center (Equiv.Perm (Fin 5)) := by
      ...
      have h_comm : ∀ h : Equiv.Perm (Fin 5), h * g * h⁻¹ * g⁻¹ ∈ M := by
        ...
        cases Int.units_eq_one_or ( Equiv.Perm.sign g ) <;> cases Int.units_eq_one_or ( Equiv.Perm.sign h ) <;> simp_all +decide [ mul_assoc ];
      simp_all +decide [ Subgroup.mem_center_iff ];
      intro h; replace hM_def := SetLike.ext_iff.mp hM_def ( h * g * h⁻¹ * g⁻¹ ) ; simp_all +decide [ mul_inv_eq_iff_eq_mul ] ;
    have hN_card : Nat.card N ≤ 2 := by
      exact le_trans ( Subgroup.card_le_of_le hN_le_two ) ( by rw [ Nat.card_eq_fintype_card ] ; native_decide ) ;
    have hN_card_cases : Nat.card N = 1 ∨ Nat.card N = 2 := by
      interval_cases _ : Nat.card N <;> simp_all +decide only [Nat.card_eq_zero];
      exact absurd ( ‹IsEmpty ↥N ∨ Infinite ↥N›.resolve_left <| by exact not_isEmpty_iff.mpr <| by exact ⟨ 1, N.one_mem ⟩ ) <| by exact not_infinite_iff_finite.mpr <| by exact Set.Finite.to_subtype <| Set.toFinite _;
    ...
    generalize_proofs at *; (
    exact absurd h ( by rw [ show N = ⊥ from eq_bot_iff.mpr hN_center ] ; simp +decide )));
  · -- If $M = A_5$, then $N$ contains $A_5$ and thus has index 1 or 2 in $S_5$.
    ...
    have := Subgroup.index_mul_card N; simp_all +decide ;
    cases hN_index <;> simp_all +decide [ Fintype.card_perm ];
    exact Or.inr <| Or.inl <| by norm_num [ Nat.factorial ] at this; linarith;
...
theorem Perm_Fin5_no_subgroup_order_30 :
  ...
  have h_index : H.index = 4 := by
    have := Subgroup.index_mul_card H ; simp_all +decide [ Fintype.card_perm ];
  ...
  obtain ⟨ϕ, hϕ⟩ : ∃ ϕ : Equiv.Perm (Fin 5) →* Equiv.Perm (Equiv.Perm (Fin 5) ⧸ H), ϕ.ker ≤ H := by
    ...
    intro x hx; simp_all +decide [ MonoidHom.mem_ker, Equiv.Perm.ext_iff ] ;
    specialize hx ( QuotientGroup.mk 1 ) ; simp_all +decide [ QuotientGroup.eq ] ;
  ...
  have h_kernel_nontrivial : Nat.card (ϕ.ker) ≠ 1 := by
    ...
    have h_inj : Function.Injective ϕ := by
      ...
      exact ( MonoidHom.ker_eq_bot_iff _ ).mp ( eq_bot_iff.mpr fun x hx => by have := h_card_ker_one.1.elim ⟨ x, hx ⟩ ⟨ 1, by simp +decide ⟩ ; aesop );
    have h_card : Nat.card (Equiv.Perm (Equiv.Perm (Fin 5) ⧸ H)) = 24 := by
      simp_all +decide [ Subgroup.index ];
      have h_card : Nat.card (Equiv.Perm (Equiv.Perm (Fin 5) ⧸ H)) = Nat.factorial (Nat.card (Equiv.Perm (Fin 5) ⧸ H)) := by
        exact Nat.card_perm;
...
theorem Perm_Fin5_no_subgroup_order_40 :
      ...
      have h_sylow : ∃ P : Subgroup (Equiv.Perm (Fin 5)), P ≤ H ∧ Nat.card P = 5 := by
        obtain ⟨g, hg⟩ : ∃ g : H, orderOf g = 5 := by
          have h_cauchy : ∀ {p : ℕ}, Nat.Prime p → p ∣ Nat.card H → ∃ g : H, orderOf g = p := by
            intro p pp dp; haveI := Fact.mk pp; exact exists_prime_orderOf_dvd_card' p dp;
          exact h_cauchy ( by decide ) ( hH_card.symm ▸ by decide );
        use Subgroup.zpowers (g : Equiv.Perm (Fin 5));
        simp +zetaDelta at *;
        rw [ Fintype.card_zpowers ] ; aesop;
      obtain ⟨ P, hP₁, hP₂ ⟩ := h_sylow; use P; simp_all +decide [ IsPGroup.iff_card ] ;
      exists 1;
  ...
  have h_n5_bound : (Nat.card (Sylow 5 (Equiv.Perm (Fin 5)))) = 6 := by
    have h_card_sylow : (Nat.card (Sylow 5 (Equiv.Perm (Fin 5)))) ∣ 24 := by
      have h_card_sylow : (Nat.card (Sylow 5 (Equiv.Perm (Fin 5)))) = Nat.card (Equiv.Perm (Fin 5) ⧸ Subgroup.normalizer (P.toSubgroup)) := by
      ...
      have h_card_sylow : (Nat.card (P.toSubgroup)) = 5 := by
        ...
        rw [ Nat.card_eq_fintype_card ] ; native_decide;);
        exact ⟨ by norm_num ⟩;
      have h_card_sylow : (Nat.card (Equiv.Perm (Fin 5) ⧸ Subgroup.normalizer (P.toSubgroup))) ∣ (Nat.card (Equiv.Perm (Fin 5) ⧸ P.toSubgroup)) := by
        convert Subgroup.index_dvd_of_le ( Subgroup.le_normalizer : ( P : Subgroup ( Equiv.Perm ( Fin 5 ) ) ) ≤ Subgroup.normalizer ( P : Subgroup ( Equiv.Perm ( Fin 5 ) ) ) ) using 1;
      have := Subgroup.card_eq_card_quotient_mul_card_subgroup ( P.toSubgroup ) ; simp_all +decide [ Nat.card_eq_fintype_card ] ;
      exact h_card_sylow.trans ( by rw [ show Nat.card ( Equiv.Perm ( Fin 5 ) ⧸ ( P : Subgroup ( Equiv.Perm ( Fin 5 ) ) ) ) = 24 by { rw [ show Fintype.card ( Equiv.Perm ( Fin 5 ) ) = 120 by native_decide ] at this; linarith } ] )
    have h_card_sylow_mod : (Nat.card (Sylow 5 (Equiv.Perm (Fin 5)))) ≡ 1 [MOD 5] := by
      haveI := Fact.mk ( show Nat.Prime 5 by decide ) ; exact card_sylow_modEq_one 5 (Equiv.Perm (Fin 5));
    have h_card_sylow_ne_one : (Nat.card (Sylow 5 (Equiv.Perm (Fin 5)))) ≠ 1 := by
      ...
      have h_normal : P.toSubgroup.Normal := by
        ...
        constructor;
        intro n hn g; specialize h_unique ( g • P ) ; simp_all +decide [ Sylow.smul_eq_iff_mem_normalizer ] ;
      ...
      have h_all_5cycles : ∀ g : Equiv.Perm (Fin 5), orderOf g = 5 → g ∈ P.toSubgroup := by
        ...
        have h_conj : ∃ h : Equiv.Perm (Fin 5), h * g * h⁻¹ ∈ P.toSubgroup := by
          have h_conj : ∀ g : Equiv.Perm (Fin 5), orderOf g = 5 → ∃ Q : Sylow 5 (Equiv.Perm (Fin 5)), g ∈ Q.toSubgroup := by
            intro g hg_order
            have h_conj : ∃ Q : Subgroup (Equiv.Perm (Fin 5)), IsPGroup 5 Q ∧ g ∈ Q := by
              use Subgroup.zpowers g;
              simp +decide [IsPGroup];
              intro m; use 1; simp +decide [← hg_order] ;
            ...
            have h_conj : ∃ Q : Sylow 5 (Equiv.Perm (Fin 5)), g ∈ Q.toSubgroup := by
              ...
              exact ⟨ Q, hQ hQ_g ⟩
            exact h_conj;
          obtain ⟨ Q, hQ ⟩ := h_conj g hg_order; have := Subgroup.card_mul_index Q.toSubgroup; simp_all +decide ;
          rw [ Fintype.card_eq_one_iff ] at h_card_sylow_one ; aesop;
        obtain ⟨ h, hh ⟩ := h_conj; have := h_normal.conj_mem _ hh h⁻¹; simp_all +decide [ mul_assoc ] ;
    ...
    have h_card_sylow_eq_six : (Nat.card (Sylow 5 (Equiv.Perm (Fin 5)))) = 6 := by
      have := Nat.le_of_dvd ( by decide ) h_card_sylow; interval_cases Nat.card ( Sylow 5 ( Equiv.Perm ( Fin 5 ) ) ) <;> trivial;
    exact h_card_sylow_eq_six
  -- Now, show |N_{S₅}(P₅)| = 20
  have h_normalizer_card : Nat.card (Subgroup.normalizer P.toSubgroup) = 120 / 6 := by
    have := Fact.mk ( show Nat.Prime 5 by decide ) ; have := Sylow.card_eq_index_normalizer P; simp_all +decide [ Nat.card_eq_fintype_card ] ;
    have := Subgroup.index_mul_card ( Subgroup.normalizer ( P : Subgroup ( Equiv.Perm ( Fin 5 ) ) ) ) ; simp_all +decide [ Nat.card_eq_fintype_card ] ;
    exact Eq.symm ( Nat.div_eq_of_eq_mul_left ( by linarith ) ( by rw [ show Fintype.card ( Equiv.Perm ( Fin 5 ) ) = 120 by native_decide ] at this; linarith ) )
    ...
    have h_contradiction : H ≤ Subgroup.normalizer P.toSubgroup := by
      have h_normalizer : (Nat.card (Sylow 5 H)) = 1 := by
        have h_unique : Nat.card (Sylow 5 H) ≡ 1 [MOD 5] ∧ Nat.card (Sylow 5 H) ∣ 40 := by
          haveI := Fact.mk ( by decide : Nat.Prime 5 ) ; have := card_sylow_modEq_one 5 H; simp_all +decide [ ← ZMod.natCast_eq_natCast_iff ] ;
          rw [ ← hH_card ] ; have := Sylow.card_eq_index_normalizer ( Classical.arbitrary ( Sylow 5 H ) ) ; simp_all +decide ;
          rw [ ← hH_card ] ; have := Subgroup.index_dvd_card ( Subgroup.normalizer ( Classical.arbitrary ( Sylow 5 H ) |> Sylow.toSubgroup ) ) ; aesop;
        have := Nat.le_of_dvd ( by decide ) h_unique.2; interval_cases Nat.card ( Sylow 5 H ) <;> trivial;
      ...
      have h_conj : ∀ g : Equiv.Perm (Fin 5), g ∈ H → g • P = P := by
        intro g hg; specialize h_unique ( g • P |> Sylow.subtype <| by
          intro x hx; obtain ⟨ y, hy, rfl ⟩ := hx; simp_all +decide [ Subgroup.mul_mem_cancel_left, Subgroup.mul_mem_cancel_right ] ;
          exact hP hy ) ; simp_all +decide [ Sylow.smul_eq_iff_mem_normalizer ] ;
...
theorem dvd_20_of_constraints (n : ℕ) (hn_pos : 0 < n)
    ...
    n ∣ 20 := by
  have := Nat.le_of_dvd (by decide) h120; interval_cases n <;> trivial