...
* `Rigidity.RET.exists_primitive_of_separating` — a function of moderate growth separating the
  fibres over the complement of a finite set generates the function field, and its equation is
  irreducible and separable over that complement.
...
generates the function field of the covering, and its equation is irreducible.**

Multiplied by the leading coefficient of the equation it satisfies, the function becomes a function
`w` satisfying a *monic* equation `P` of degree the order `n` of the deck group, still separating a
fibre — the leading coefficient vanishes only over the punctures.  Over the rational functions of
the base, the minimal polynomial of `w` divides `P`, so has degree at most `n`; and the `n`
translates of `w` under the deck group are distinct elements of the function field, each a root of
that minimal polynomial, so its degree is at least `n`.  Hence the minimal polynomial is `P`, which
is therefore irreducible, and `w` generates a subextension of degree `n` — all of the function
field, whose degree over the base is the order of the deck group. -/
theorem exists_primitive_of_separating (hf : IsLocalHomeomorph f)
    (hrange : Set.range f = (↑S : Set ℂ)ᶜ) [IsOverBase H f]
    (htrans : ∀ y y' : Y, f y = f y' → ∃ b : H, y' = b • y)
    (hne : ∀ a : H, a ≠ 1 → ∃ F ∈ coverRing hf S, ∃ y : Y, F (a • y) ≠ F y)
    {S₁ : Finset ℂ} (hS₁ : S ⊆ S₁) {F : Y → ℂ} (hF : F ∈ coverRing hf S)
    (hsepF : ∀ y : Y, f y ∉ (S₁ : Set ℂ) → ∀ c : H, c ≠ 1 → F (c • y) ≠ F y) :
    letI := coverRatFuncAlgebra hf hrange
    ∃ P : Polynomial ℂ[X], P.Monic ∧ P.natDegree = Nat.card H ∧
      ...
      ∃ α : FractionRing ↥(coverRing hf S),
        aeval α (P.map (algebraMap ℂ[X] (RatFunc ℂ))) = 0 ∧
  ...
  obtain ⟨bc, d, hd, hdS, heq⟩ := exists_integral_of_growth (H := H) hf
  ...
  have hsepW : ∀ y : Y, f y ∉ (S₁ : Set ℂ) → ∀ c : H, c ≠ 1 → W (c • y) ≠ W y := by
    ...
    exact fun h => hsepF y hy c hc (mul_left_cancel₀ (hdne _ hy) h)
  ...
  refine ⟨monicPoly bc n, monicPoly_monic bc n, ?_, hsepz, hmodel, hirr, α, hQα, ?_⟩
...
separating the points of a fibre over the complement of a finite set, and such a function
generates. -/
theorem exists_algebraic_model_primitive (hf : IsLocalHomeomorph f)
  ...
  exact ⟨P, S₁, hS₁, hP, hdeg, hsepz, hmodel, hirr, FractionRing ↥(coverRing hf S), inferInstance,
    inferInstance, isGalois_ratFunc_coverRing hf hrange htrans hne,
    ⟨mulEquivAlgEquiv_ratFunc_coverRing hf hrange htrans hne⟩, α, hα, hgen⟩
