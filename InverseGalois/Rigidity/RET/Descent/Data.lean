...
structure GeomTower (G : Type) [Group G] [Finite G] (cert : RigidData G) where
  ...
  φ_pres : ∀ x : Rigidity.RET.SphereGroup cert.r,
    φ (pres x) = Rigidity.RET.sphereHom base base_mem.2.1 x
  /-- the modulus through which the tame inertia is read: the number of roots of unity the
  cyclotomic character of the tower sees. -/
  rootOrder : ℕ
  [rootOrder_neZero : NeZero rootOrder]
  /-- every monodromy generator's order — the ramification index at that branch point — divides the
  modulus. -/
  order_dvd : ∀ i, orderOf (base i) ∣ rootOrder
  /-- the **cyclotomic character** of the arithmetic fundamental group, valued in the units of
  `ZMod rootOrder`: `χ (e)` is the power to which `e` raises the `rootOrder`-th roots of unity. -/
  cycloChar : E →* (ZMod rootOrder)ˣ
  /-- the geometric group acts trivially on roots of unity: they are constants, and all constants
  lie in the geometric base. -/
  cycloChar_N : ∀ n : N, cycloChar (n : E) = 1
  /-- **The branch-cycle formula** (Module C's tame-inertia + cyclotomic-character input, carried on
  the tower).  For each `e : E`, conjugation by `e` twists each geometric generator `pres (xᵢ)` —
  through `φ` — into the cyclotomic power class `Cᵢ^{χ(e)}`, the exponent being the value of the
  cyclotomic character.  This is the arithmetic-geometry content Fried's branch-cycle argument
  ...
  branchCycleχ : ∀ (e : E) (i : Fin cert.r),
    ConjClasses.mk (φ (Rigidity.RET.conjN N e (pres (PresentedGroup.of i)))) =
      ConjClasses.powClass ((cycloChar e : ZMod rootOrder).val) (cert.C i)
  ...
  /-- the geometric base is generated by the constants.  Together with `const_le_geomBase` this pins
  `geomBase` to the constant-field base `k₀(T)`. -/
...
attribute [instance] GeomTower.groupE GeomTower.normalN GeomTower.fieldΩ GeomTower.algΩ
  ...
  GeomTower.rootOrder_neZero

/-- **The geometric base of the tower is exactly the constant-field base** `k₀(T)`: it contains all
constants (`const_le_geomBase`) and is contained in the field they generate
(`geomBase_le_constFieldBase`). -/
theorem GeomTower.geomBase_eq_constFieldBase {G : Type} [Group G] [Finite G] {cert : RigidData G}
    (tw : GeomTower G cert) : tw.geomBase = constFieldBase tw.Ω :=
  le_antisymm tw.geomBase_le_constFieldBase
    (IntermediateField.adjoin_le_iff.mpr fun _ hx => tw.const_le_geomBase hx)

/-- **The branch-cycle formula in its loose form**: the cyclotomic exponent is *some* natural number
coprime to every monodromy generator's order.  A unit of `ZMod rootOrder` has value coprime to
`rootOrder` (`ZMod.val_coe_unit_coprime`), and every generator's order divides `rootOrder`. -/
theorem GeomTower.branchCycle {G : Type} [Group G] [Finite G] {cert : RigidData G}
    (tw : GeomTower G cert) (e : tw.E) :
    ∃ k : ℕ, (∀ i, Nat.Coprime k (orderOf (tw.base i))) ∧
      ∀ i, ConjClasses.mk (tw.φ (Rigidity.RET.conjN tw.N e (tw.pres (PresentedGroup.of i)))) =
        ConjClasses.mk (tw.base i ^ k) := by
  refine ⟨(tw.cycloChar e : ZMod tw.rootOrder).val, fun i =>
    (ZMod.val_coe_unit_coprime (tw.cycloChar e)).coprime_dvd_right (tw.order_dvd i), fun i => ?_⟩
  rw [tw.branchCycleχ e i, ← tw.base_mem.1 i, ConjClasses.powClass_mk]
