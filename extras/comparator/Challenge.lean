...
Everything below the definitions is `sorry`; `Solution.lean` repeats the definitions verbatim
and discharges the statements from `Rigidity.RET.geomRET`.
-/
import Mathlib
...
theorem geometric_igp (G : Type) [Group G] [Finite G] :
    ∃ (L : Type) (_ : Field L) (_ : Algebra (RatFunc Qbar) L)
      ...
      Nonempty ((L ≃ₐ[RatFunc Qbar] L) ≃* G) := by
  sorry
